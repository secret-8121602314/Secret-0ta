...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div className={`fixed inset-y-0 left-0 z-50 w-80 bg-gradient-to-b from-surface to-background border-r border-surface-light/20 transform transition-transform duration-300 ease-in-out lg:translate-x-0 lg:static lg:inset-0 ${
        ...
        <div className="flex flex-col h-full">
          ...
          {/* Conversation List */}
          <div className="flex-1 overflow-y-auto px-6 pb-6 custom-scrollbar" style={{ maxHeight: 'calc(100vh - 200px)' }}>
            <div className="space-y-3">
              {conversationList.map((conversation) => (
                <div
                  key={conversation.id}
                  className={`conversation-tab cursor-pointer transition-all duration-300 ${
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      ...
                      className="opacity-0 group-hover:opacity-100 p-2 text-text-muted hover:text-red-400 transition-all duration-200 rounded-lg hover:bg-red-500/10"
                    >
