...
3. **📚 Cache History Tracking**: Avoids repetitive news over 7-day period
...
private async hasRecentSimilarContent(promptKey: string): Promise<boolean> {
  try {
    ...
    if (historyData && historyData.cacheData && historyData.cacheData.history) {
      ...
      // Check for content from the last week
      const recentContent = history.filter(entry => entry.timestamp > oneWeekAgo);
      
      if (recentContent.length > 0) {
        console.log(`📰 Found ${recentContent.length} recent cache entries for ${promptKey} - avoiding repetition`);
        return true;
      }
    }
  } catch (error) {
...
2. **System checks**: Recent similar content found (within 7 days)
...
- **Smart Logic**: Prevents repetitive content within 7 days
...
✅ **Smart Deduplication**: 7-day repetition prevention  
