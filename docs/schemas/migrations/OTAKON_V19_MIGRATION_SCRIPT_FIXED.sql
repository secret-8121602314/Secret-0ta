-- 🚀 OTAKON V19 SAFE MIGRATION SCRIPT (FIXED VERSION)
...
-- FIXED: Removed CONCURRENTLY from CREATE INDEX to avoid transaction block issues
...
-- =====================================================
-- STEP 12: CREATE INDEXES (SAFE - IF NOT EXISTS, NO CONCURRENTLY)
-- =====================================================

-- Player profiles indexes
CREATE INDEX IF NOT EXISTS idx_player_profiles_user_id ON player_profiles(user_id);
CREATE INDEX IF NOT EXISTS idx_player_profiles_hint_style ON player_profiles(hint_style);
CREATE INDEX IF NOT EXISTS idx_player_profiles_player_focus ON player_profiles(player_focus);

-- Game contexts indexes
CREATE INDEX IF NOT EXISTS idx_game_contexts_user_id ON game_contexts(user_id);
CREATE INDEX IF NOT EXISTS idx_game_contexts_game_id ON game_contexts(game_id);
CREATE INDEX IF NOT EXISTS idx_game_contexts_user_game ON game_contexts(user_id, game_id);
CREATE INDEX IF NOT EXISTS idx_game_contexts_last_session ON game_contexts(last_session_date);

-- Build snapshots indexes
CREATE INDEX IF NOT EXISTS idx_build_snapshots_game_context ON build_snapshots(game_context_id);
CREATE INDEX IF NOT EXISTS idx_build_snapshots_timestamp ON build_snapshots(timestamp);
...
CREATE INDEX IF NOT EXISTS idx_enhanced_insights_conversation ON enhanced_insights(conversation_id);
CREATE INDEX IF NOT EXISTS idx_enhanced_insights_user ON enhanced_insights(user_id);
...
CREATE INDEX IF NOT EXISTS idx_proactive_triggers_processed ON proactive_triggers(processed) WHERE processed = FALSE;
CREATE INDEX IF NOT EXISTS idx_proactive_triggers_created ON proactive_triggers(created_at);
...
CREATE INDEX IF NOT EXISTS idx_proactive_insights_unread ON proactive_insights(is_read) WHERE is_read = FALSE;
...
BEGIN
    ...
    RAISE NOTICE '';
    RAISE NOTICE '⚠️  NOTE: Indexes were created without CONCURRENTLY due to transaction block limitations.';
    RAISE NOTICE '   For production databases with heavy load, consider recreating indexes with CONCURRENTLY';
    RAISE NOTICE '   during maintenance windows using the provided index recreation script.';
END $$;
