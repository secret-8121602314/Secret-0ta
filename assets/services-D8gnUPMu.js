...
`,Ue=(o,e,t,s,r)=>{var d,f;const a=((d=o.subtabs)==null?void 0:d.filter(S=>S.status==="loaded"&&S.content).map(S=>`### ${S.title} (ID: ${S.id})
${S.content}`).join(`

`))||"No subtabs available yet.",n=o.messages.slice(-10).map(S=>`${S.role==="user"?"User":"Otagon"}: ${S.content}`).join(`
`),c=o.contextSummary?`**Historical Context (Previous Sessions):**
${o.contextSummary}

`:"",i=r||$.getDefaultProfile(),u=$.buildProfileContext(i);return`
...
`},Ge=(o,e,t,s)=>{const r=s||$.getDefaultProfile();return`
...
*Content loading...*`}}const pt=new Me;class De{constructor(){g(this,"STORAGE_KEY","otakon_used_suggested_prompts");g(this,"LAST_RESET_KEY","otakon_suggested_prompts_last_reset");g(this,"RESET_INTERVAL_MS",1440*60*1e3);g(this,"usedPrompts",new Set);this.loadUsedPrompts(),this.checkAndResetIfNeeded()}loadUsedPrompts(){try{const e=localStorage.getItem(this.STORAGE_KEY);if(e){const t=JSON.parse(e);this.usedPrompts=new Set(t)}}catch(e){console.warn("Failed to load used suggested prompts:",e),this.usedPrompts=new Set}}saveUsedPrompts(){try{const e=Array.from(this.usedPrompts);localStorage.setItem(this.STORAGE_KEY,JSON.stringify(e))}catch(e){console.warn("Failed to save used suggested prompts:",e)}}checkAndResetIfNeeded(){try{const e=localStorage.getItem(this.LAST_RESET_KEY),t=Date.now();(!e||t-parseInt(e)>=this.RESET_INTERVAL_MS)&&(console.log("🔄 24 hours passed - resetting suggested prompts for fresh daily news"),this.resetUsedPrompts(),localStorage.setItem(this.LAST_RESET_KEY,t.toString()))}catch(e){console.warn("Failed to check/reset suggested prompts:",e)}}markPromptAsUsed(e){this.usedPrompts.add(e),this.saveUsedPrompts()}isPromptUsed(e){return this.usedPrompts.has(e)}getUnusedPrompts(e){return e.filter(t=>!this.isPromptUsed(t))}areAllPromptsUsed(e){return e.every(t=>this.isPromptUsed(t))}resetUsedPrompts(){this.usedPrompts.clear(),localStorage.removeItem(this.STORAGE_KEY),console.log("🔄 Suggested prompts reset - all 4 prompts will be available again")}getUsedCount(){return this.usedPrompts.size}getTimeUntilNextReset(){try{const e=localStorage.getItem(this.LAST_RESET_KEY);if(!e)return 0;const t=parseInt(e)+this.RESET_INTERVAL_MS;return Math.max(0,t-Date.now())}catch(e){return console.warn("Failed to calculate time until next reset:",e),0}}getStaticNewsPrompts(){return we}processAISuggestions(e){if(console.log("🔍 [SuggestedPromptsService] Input suggestions:",e,"Type:",typeof e),!e)return console.log("🔍 [SuggestedPromptsService] No suggestions provided"),[];let t=[];if(Array.isArray(e))console.log("🔍 [SuggestedPromptsService] Suggestions is already an array"),t=e;else if(typeof e=="string"){console.log("🔍 [SuggestedPromptsService] Suggestions is a string, attempting to parse");let r=e.trim();r.startsWith('["')&&!r.endsWith('"]')&&(r.endsWith('"')||(r+='"'),r.endsWith("]")||(r+="]"),console.log("🔍 [SuggestedPromptsService] Fixed malformed JSON array:",r));try{const a=JSON.parse(r);Array.isArray(a)?(t=a,console.log("🔍 [SuggestedPromptsService] Successfully parsed as JSON array")):(t=[e],console.log("🔍 [SuggestedPromptsService] Parsed as single value, wrapped in array"))}catch{console.log("🔍 [SuggestedPromptsService] JSON parsing failed, trying delimiter splitting"),r.includes('", "')||r.includes(`",
"`)?(t=r.split(/",\s*"/).map(n=>n.replace(/^["\s]+|["\s]+$/g,"")).filter(n=>n.length>0),console.log("🔍 [SuggestedPromptsService] Split by comma delimiters")):r.includes(`
`)?(t=r.split(`
`).map(n=>n.replace(/^["\s]+|["\s]+$/g,"")).filter(n=>n.length>0),console.log("🔍 [SuggestedPromptsService] Split by quote delimiters")):(t=[e],console.log("🔍 [SuggestedPromptsService] Treated as single suggestion"))}}console.log("🔍 [SuggestedPromptsService] Raw suggestions array:",t);const s=t.filter(r=>r&&typeof r=="string"&&r.trim().length>0).map(r=>r.trim()).slice(0,3);return console.log("🔍 [SuggestedPromptsService] Final processed suggestions:",s),s}getFallbackSuggestions(e,t){return t===!0||e==="game-hub"||e==="everything-else"?this.getStaticNewsPrompts():["What should I do next in this area?","Tell me about the story so far","Give me some tips for this game","What are the key mechanics I should know?"]}}const ft=new De;class Fe{async generatePlayingSessionSummary(e){const t=e.messages.filter(n=>n.role==="assistant"&&n.content.includes("Hint:")),s=this.extractKeyPoints(t),r=this.extractObjectives(t),a=`Playing session summary for ${e.gameTitle}:
...
Strategic Notes:
${t.slice(-3).map(n=>`- ${n.content.substring(0,100)}...`).join(`
`)}`;return{mode:"planning",gameTitle:e.gameTitle||"Unknown Game",conversationId:e.id,summary:a,keyPoints:s,objectives:r,timestamp:Date.now()}}extractKeyPoints(e){const t=[];return e.forEach(s=>{(s.content.includes("defeated")||s.content.includes("completed"))&&t.push("Achievement unlocked or objective completed"),(s.content.includes("found")||s.content.includes("discovered"))&&t.push("New item or location discovered"),(s.content.includes("unlocked")||s.content.includes("gained"))&&t.push("New ability or feature unlocked")}),t.length>0?t:["Session progress recorded"]}extractObjectives(e){const t=[];return e.forEach(s=>{(s.content.includes("objective")||s.content.includes("goal"))&&t.push("Continue current objective"),(s.content.includes("next")||s.content.includes("should"))&&t.push("Follow recommended next steps"),(s.content.includes("explore")||s.content.includes("investigate"))&&t.push("Explore new areas or investigate leads")}),t.length>0?t:["Continue game progression"]}async storeSessionSummary(e,t){console.log("Storing session summary for conversation:",e,t)}async getLatestSessionSummary(e){return console.log("Getting latest session summary for conversation:",e),null}}const yt=new Fe,k=class k{static getInstance(){return k.instance||(k.instance=new k),k.instance}async getSubtabs(e){return this.getSubtabsFromTable(e)}async setSubtabs(e,t){{console.error(`🔄 [SubtabsService] Writing ${t.length} subtabs to BOTH table AND JSONB for conversation:`,e);const s=await this.setSubtabsInTable(e,t);console.error("  ✅ Table write:",s?"SUCCESS":"FAILED");const r=await this.setSubtabsInJsonb(e,t);return console.error("  ✅ JSONB write:",r?"SUCCESS":"FAILED"),s&&r}}async addSubtab(e,t){{const s=await this.addSubtabToTable(e,t);return await this.addSubtabToJsonb(e,t),s}}async updateSubtab(e,t,s){{const r=await this.updateSubtabInTable(t,s),a=await this.updateSubtabInJsonb(e,t,s);return r&&a}}async deleteSubtab(e,t){return this.deleteSubtabFromTable(t)}async getSubtabsFromTable(e){try{const{data:t,error:s}=await l.from("subtabs").select("*").eq("conversation_id",e).order("order_index",{ascending:!0});return s?(console.error("Error getting subtabs from table:",s),[]):(t||[]).map(r=>{const a=typeof r.metadata=="object"&&r.metadata!==null?r.metadata:{};return{id:r.id,title:r.title,content:r.content||"",type:r.tab_type,isNew:a.isNew||!1,status:a.status||"loaded",instruction:a.instruction}})}catch(t){return console.error("Error getting subtabs from table:",t),[]}}async setSubtabsInTable(e,t){try{const{error:s}=await l.from("subtabs").delete().eq("conversation_id",e);if(s)return console.error("Error deleting existing subtabs:",s),!1;if(t.length>0){const r=t.map((n,c)=>({id:n.id,conversation_id:e,game_id:null,title:n.title,content:n.content,tab_type:n.type,order_index:c,metadata:{isNew:n.isNew,status:n.status,instruction:n.instruction}})),{error:a}=await l.from("subtabs").insert(r);if(a)return console.error("Error inserting subtabs:",a),!1}return!0}catch(s){return console.error("Error setting subtabs in table:",s),!1}}async addSubtabToTable(e,t){var s;try{const{data:r}=await l.from("conversations").select("game_id").eq("id",e).single(),a=(r==null?void 0:r.game_id)||"",{data:n}=await l.from("subtabs").select("order_index").eq("conversation_id",e).order("order_index",{ascending:!1}).limit(1),c=((s=n==null?void 0:n[0])==null?void 0:s.order_index)??-1,{data:i,error:u}=await l.from("subtabs").insert({id:t.id,conversation_id:e,game_id:a,title:t.title,content:t.content,tab_type:t.type,order_index:c+1,metadata:{isNew:t.isNew,status:t.status,instruction:t.instruction}}).select().single();return u?(console.error("Error adding subtab to table:",u),null):{id:i.id,title:i.title,content:Te(i.content),type:i.tab_type,isNew:typeof i.metadata=="object"&&i.metadata!==null&&!Array.isArray(i.metadata)&&i.metadata.isNew||!1,status:(typeof i.metadata=="object"&&i.metadata!==null&&!Array.isArray(i.metadata)?i.metadata.status:void 0)||"loaded",instruction:typeof i.metadata=="object"&&i.metadata!==null&&!Array.isArray(i.metadata)?i.metadata.instruction:void 0}}catch(r){return console.error("Error adding subtab to table:",r),null}}async updateSubtabInTable(e,t){try{const s={};if(t.title!==void 0&&(s.title=t.title),t.content!==void 0&&(s.content=t.content),t.type!==void 0&&(s.tab_type=t.type),t.isNew!==void 0||t.status!==void 0||t.instruction!==void 0){const{data:a}=await l.from("subtabs").select("metadata").eq("id",e).single(),n=typeof(a==null?void 0:a.metadata)=="object"&&(a==null?void 0:a.metadata)!==null?a.metadata:{};s.metadata={...n,...t.isNew!==void 0&&{isNew:t.isNew},...t.status!==void 0&&{status:t.status},...t.instruction!==void 0&&{instruction:t.instruction}}}const{error:r}=await l.from("subtabs").update(s).eq("id",e);return r?(console.error("Error updating subtab in table:",r),!1):!0}catch(s){return console.error("Error updating subtab in table:",s),!1}}async deleteSubtabFromTable(e){try{const{error:t}=await l.from("subtabs").delete().eq("id",e);return t?(console.error("Error deleting subtab from table:",t),!1):!0}catch(t){return console.error("Error deleting subtab from table:",t),!1}}async getSubtabsFromJsonb(e){try{const{data:t,error:s}=await l.from("conversations").select("subtabs").eq("id",e).single();return s?(console.error("Error getting subtabs from JSONB:",s),[]):(t==null?void 0:t.subtabs)||[]}catch(t){return console.error("Error getting subtabs from JSONB:",t),[]}}async setSubtabsInJsonb(e,t){try{const{error:s}=await l.from("conversations").update({subtabs:t,subtabs_order:t.map(r=>r.id)}).eq("id",e);return s?(console.error("Error setting subtabs in JSONB:",s),!1):!0}catch(s){return console.error("Error setting subtabs in JSONB:",s),!1}}async addSubtabToJsonb(e,t){try{const r=[...await this.getSubtabsFromJsonb(e),t];return await this.setSubtabsInJsonb(e,r)?t:null}catch(s){return console.error("Error adding subtab to JSONB:",s),null}}async updateSubtabInJsonb(e,t,s){try{const a=(await this.getSubtabsFromJsonb(e)).map(n=>n.id===t?{...n,...s}:n);return await this.setSubtabsInJsonb(e,a)}catch(r){return console.error("Error updating subtab in JSONB:",r),!1}}async deleteSubtabFromJsonb(e,t){try{const r=(await this.getSubtabsFromJsonb(e)).filter(a=>a.id!==t);return await this.setSubtabsInJsonb(e,r)}catch(s){return console.error("Error deleting subtab from JSONB:",s),!1}}async migrateConversationSubtabs(e){try{const t=await this.getSubtabsFromJsonb(e);return t.length===0?!0:await this.setSubtabsInTable(e,t)}catch(t){return console.error("Error migrating subtabs:",t),!1}}async rollbackConversationSubtabs(e){try{const t=await this.getSubtabsFromTable(e);return t.length===0?!0:await this.setSubtabsInJsonb(e,t)}catch(t){return console.error("Error rolling back subtabs:",t),!1}}async migrateAllSubtabs(){try{const{data:e,error:t}=await l.from("conversations").select("id, subtabs").not("subtabs","is",null);if(t)return console.error("Error fetching conversations:",t),{success:0,failed:0};let s=0,r=0;const a=(e||[]).filter(c=>c.subtabs&&Array.isArray(c.subtabs)&&c.subtabs.length>0).map(c=>this.migrateConversationSubtabs(c.id));return(await Promise.allSettled(a)).forEach(c=>{c.status==="fulfilled"&&c.value?s++:r++}),{success:s,failed:r}}catch(e){return console.error("Error in batch migration:",e),{success:0,failed:0}}}};g(k,"instance");let ee=k;const j=ee.getInstance();function B(){var o;return((o=globalThis.crypto)==null?void 0:o.randomUUID())||"xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx".replace(/[xy]/g,e=>{const t=Math.random()*16|0;return(e==="x"?t:t&3|8).toString(16)})}class Le{async createGameTab(e){var a,n,c;console.log("🎮 [GameTabService] Creating game tab:",e);const t=await y.getConversation(e.conversationId);if(t){if(console.log("🎮 [GameTabService] Tab already exists:",t.title),e.aiResponse&&((a=t.subtabs)!=null&&a.some(i=>i.status==="loading"||i.content==="Loading..."))){console.log("🎮 [GameTabService] Updating loading subtabs with new AI insights");const i=this.extractInsightsFromAIResponse(e.aiResponse,t.subtabs);return await y.updateConversation(t.id,{subtabs:i,updatedAt:Date.now()}),{...t,subtabs:i}}return t}console.log("🎮 [GameTabService] Creating new tab for:",e.gameTitle);let s=[];if(e.isUnreleased)console.error("🎮 [GameTabService] Creating unreleased game tab (no subtabs, Discuss mode only)");else if(e.aiResponse)if(console.error("🎮 [GameTabService] Extracting subtabs from AI response"),(n=e.aiResponse.gamePillData)!=null&&n.wikiContent&&Object.keys(e.aiResponse.gamePillData.wikiContent).length>0)console.error("🎮 [GameTabService] Found gamePillData.wikiContent with",Object.keys(e.aiResponse.gamePillData.wikiContent).length,"tabs"),s=Object.entries(e.aiResponse.gamePillData.wikiContent).map(([i,u])=>({id:B(),title:this.formatTabTitle(i),type:this.determineTabType(i),content:u,isNew:!1,status:"loaded"})),console.error("🎮 [GameTabService] Created",s.length,"subtabs from gamePillData.wikiContent");else if(e.aiResponse.progressiveInsightUpdates&&e.aiResponse.progressiveInsightUpdates.length>0)console.error("🎮 [GameTabService] Found progressiveInsightUpdates with",e.aiResponse.progressiveInsightUpdates.length,"updates"),s=e.aiResponse.progressiveInsightUpdates.map(i=>({id:B(),title:i.title,type:this.determineTabType(i.tabId),content:i.content,isNew:!1,status:"loaded"})),console.error("🎮 [GameTabService] Created",s.length,"subtabs from progressiveInsightUpdates");else{const i=this.extractInsightsFromAIResponse(e.aiResponse,[]);i.length>0?(s=i,console.error("🎮 [GameTabService] Created",s.length,"subtabs from INSIGHT_UPDATE tags")):(s=this.generateInitialSubTabs(e.genre||"Default",e.playerProfile),console.error("🎮 [GameTabService] Created",s.length,"template subtabs (will populate via background AI using conversation context)"))}else s=this.generateInitialSubTabs(e.genre||"Default",e.playerProfile),console.error("🎮 [GameTabService] Created",s.length,"initial template subtabs (no AI response)");const r={id:e.conversationId,title:e.gameTitle,messages:[],createdAt:Date.now(),updatedAt:Date.now(),isActive:!1,gameId:e.gameTitle.toLowerCase().replace(/\s+/g,"-"),gameTitle:e.gameTitle,genre:e.genre,subtabs:s,subtabsOrder:s.map(i=>i.id),isActiveSession:!1,activeObjective:"",gameProgress:0,isUnreleased:e.isUnreleased||!1};return await y.addConversation(r),s.length>0?(console.error("🎮 [GameTabService] Saving",s.length,"subtabs for conversation:",r.id),console.error("🎮 [GameTabService] Subtabs:",JSON.stringify(s,null,2)),await j.setSubtabs(r.id,s)):console.error("🎮 [GameTabService] No subtabs to save for conversation:",r.id),e.aiResponse?(c=r.subtabs)!=null&&c.some(u=>u.content==="Loading...")&&this.generateInitialInsights(r,e.playerProfile,e.aiResponse).catch(u=>console.error("Background insight generation failed:",u)):this.generateInitialInsights(r,e.playerProfile,e.aiResponse).catch(i=>console.error("Background insight generation failed:",i)),r}generateInitialSubTabs(e,t,s){let a=(oe[e]||oe.Default).map(n=>({...n,priority:"medium",isProfileSpecific:!1}));if(t){console.error("🎮 [GameTabService] Generating profile-specific tabs for:",t.playerFocus);const n=$.generateProfileSpecificTabs(t,s);a=[...a,...n],a=$.prioritizeTabsForProfile(a,t)}return a.map(n=>({id:B(),title:n.title,type:n.type,content:"Loading...",isNew:!0,status:"loading",instruction:n.instruction}))}extractInsightsFromAIResponse(e,t){console.error("🤖 [GameTabService] Extracting dynamic insights from AI response");const s=e.otakonTags.get("INSIGHT_UPDATE");if(s){if(console.error("🤖 [GameTabService] Found INSIGHT_UPDATE:",s),t.find(a=>a.id===s.id))return t.map(a=>a.id===s.id?{...a,content:s.content,isNew:!0,status:"loaded"}:a);{const a={id:B(),title:this.formatTabTitle(s.id),type:this.determineTabType(s.id),content:s.content,isNew:!0,status:"loaded"};return[...t,a]}}return t}formatTabTitle(e){return e.split("_").map(t=>t.charAt(0).toUpperCase()+t.slice(1)).join(" ")}determineTabType(e){return e.includes("story")?"story":e.includes("character")?"characters":e.includes("strategy")||e.includes("tips")?"tips":e.includes("boss")?"strategies":e.includes("quest")||e.includes("walkthrough")?"walkthrough":e.includes("item")?"items":"chat"}async generateInitialInsights(e,t,s){var n,c,i;const r=e.id,a=e.gameTitle;console.error(`🤖 [GameTabService] 🔄 [${r}] Generating initial insights for: ${a}`);try{if(!(await y.getConversations(!0))[r]){console.error(`🤖 [GameTabService] [${r}] ⚠️ Conversation no longer exists, aborting insight generation`);return}let d="";s!=null&&s.content?(d=`AI Analysis: ${s.content}`,console.error(`🤖 [GameTabService] [${r}] Using AI response as context (${s.content.length} chars)`)):e.messages.length>0?(d=e.messages.map(h=>`${h.role==="user"?"User":"AI"}: ${h.content}`).join(`

`),console.error(`🤖 [GameTabService] [${r}] Using messages as context (${e.messages.length} msgs)`)):console.error(`🤖 [GameTabService] [${r}] ⚠️ No context available`),console.error(`🤖 [GameTabService] [${r}] 🚀 Calling AI generateInitialInsights...`);const f=await ue.generateInitialInsights(a||"Unknown Game",e.genre||"Action RPG",t,d);if(console.error(`🤖 [GameTabService] [${r}] 📥 AI returned:`,Object.keys(f).length,"insights"),!(await y.getConversations(!0))[r]){console.error(`🤖 [GameTabService] [${r}] ⚠️ Conversation deleted during AI call, discarding results`);return}const x=f&&Object.keys(f).length>0;x?console.error(`🤖 [GameTabService] [${r}] ✅ Got ${Object.keys(f).length} insights:`,Object.keys(f)):console.error(`🤖 [GameTabService] [${r}] ❌ Empty insights, using fallback`);const ae=(await y.getConversations(!0))[r];if(!ae){console.error(`🤖 [GameTabService] [${r}] ⚠️ Conversation not found, may have been deleted`);return}const fe={story:"story_so_far",walkthrough:"quest_log",strategies:"build_optimization",tips:"hidden_paths"};console.error("🤖 [GameTabService] Building content mapping for subtabs...");const V=((n=ae.subtabs)==null?void 0:n.map(h=>{let b="";const z=fe[h.type];if(h.type==="strategies"&&h.title.includes("Boss")){const C="boss_strategy";x&&f[C]&&(b=f[C],console.error(`🤖 [GameTabService] Subtab "${h.title}" using AI content from key "${C}" (${b.length} chars)`))}if(!b&&x&&z&&f[z]&&(b=f[z],console.error(`🤖 [GameTabService] Subtab "${h.title}" using AI content from key "${z}" (${b.length} chars)`)),!b){let C=d;if(h.type==="story"&&d.includes("Lore:")){const E=d.match(/Lore:(.*?)(?=\n\n|\n[A-Z]|$)/s);C=E?E[1].trim():d}else if(h.type==="strategies"&&d.includes("Analysis:")){const E=d.match(/Analysis:(.*?)(?=\n\n|\n[A-Z]|$)/s);C=E?E[1].trim():d}else if(h.type==="tips"&&d.includes("Hint:")){const E=d.match(/Hint:(.*?)(?=\n\n|\n[A-Z]|$)/s);C=E?E[1].trim():d}b=`## ${h.title}

${C}`,console.error(`🤖 [GameTabService] Subtab "${h.title}" using fallback content from AI response (${b.length} chars)`),console.error("🤖 [GameTabService] Preview:",b.substring(0,150)+"...")}return{...h,content:b,isNew:!1,status:"loaded"}}))||[];console.error("🤖 [GameTabService] Updating subtabs with content...");const ye=V.map(h=>{var b;return{id:h.id,title:h.title,status:h.status,contentLength:((b=h.content)==null?void 0:b.length)||0,isNew:h.isNew}});console.error("🤖 [GameTabService] Subtabs to save:",ye),console.error("🤖 [GameTabService] ALL statuses:",V.map(h=>h.status)),console.error("🤖 [GameTabService] 🗑️ Clearing cache BEFORE subtabs write..."),y.clearCache(),await j.setSubtabs(e.id,V),console.error("🤖 [GameTabService] ✅ Subtabs dual-write complete (table + JSONB)"),y.clearCache(),await new Promise(h=>setTimeout(h,500));const ne=(await y.getConversations(!0))[e.id];ne?console.error("🤖 [GameTabService] 🔍 VERIFICATION: Read back subtabs after write:",((c=ne.subtabs)==null?void 0:c.map(h=>({title:h.title,status:h.status})))||"NO SUBTABS"):console.error("🤖 [GameTabService] ⚠️ VERIFICATION: Could not find conversation after write!"),await y.updateConversation(e.id,{updatedAt:Date.now()}),console.error("🤖 [GameTabService] ✅ Conversation metadata updated")}catch(u){console.error("🤖 [GameTabService] ❌ Failed to generate initial insights:",u),G.warning("Failed to load game insights. You can still chat about the game!");try{const f=(await y.getConversations(!0))[e.id];if(!f){console.error("🤖 [GameTabService] Conversation not found for error update:",e.id);return}const S=((i=f.subtabs)==null?void 0:i.map(x=>({...x,content:`Failed to load ${x.title} content. Please try again later.`,isNew:!1,status:"error"})))||[];await j.setSubtabs(e.id,S),await y.updateConversation(e.id,{updatedAt:Date.now()})}catch(d){console.error("🤖 [GameTabService] Failed to update error state:",d)}}}async updateSubTabContent(e,t,s){console.error("📝 [GameTabService] Updating sub-tab content:",{conversationId:e,subTabId:t});try{const a=(await y.getConversations())[e];if(!a||!a.subtabs)throw new Error("Conversation or sub-tabs not found");const n=a.subtabs.map(c=>c.id===t?{...c,content:s,isNew:!1,status:"loaded"}:c);await j.setSubtabs(e,n),await y.updateConversation(e,{updatedAt:Date.now()})}catch(r){throw console.error("Failed to update sub-tab content:",r),r}}async getGameTab(e){try{const s=(await y.getConversations())[e];return!s||!s.gameTitle?null:{id:s.id,title:s.title,gameId:s.gameId||s.gameTitle.toLowerCase().replace(/\s+/g,"-"),gameTitle:s.gameTitle,genre:s.genre||"Unknown",subtabs:s.subtabs||[],createdAt:s.createdAt,updatedAt:s.updatedAt,isActiveSession:s.isActiveSession||!1}}catch(t){return console.error("Failed to get game tab:",t),G.error("Failed to load game tab."),null}}isGameTab(e){return!e.isGameHub&&!!e.gameTitle}generateGameConversationId(e){return`game-${e.toLowerCase().replace(/[^a-z0-9\s-]/g,"").replace(/\s+/g,"-")}`}async updateSubTabsFromAIResponse(e,t){console.error(`📝 [GameTabService] [${e}] Updating subtabs from AI response:`,t.length);try{const r=(await y.getConversations(!0))[e];if(!r||!r.subtabs){console.error(`📝 [GameTabService] [${e}] ⚠️ Conversation or subtabs not found, aborting update`);return}let a=0;const n=r.subtabs.map(c=>{const i=t.find(u=>u.tabId===c.id);if(i){a++,console.error(`📝 [GameTabService] [${e}] Updating subtab: ${c.id} - ${i.title}`);const d=`

---
**Updated: `+new Date().toLocaleString()+`**

`,S=c.content&&c.content.trim().length>0&&c.content!=="Loading..."&&c.status==="loaded"?c.content+d+i.content:i.content;return{...c,title:i.title||c.title,content:S,isNew:!0,status:"loaded"}}return c});if(a===0){console.error(`📝 [GameTabService] [${e}] ⚠️ No subtabs matched for update`);return}await y.updateConversation(e,{subtabs:n,updatedAt:Date.now()}),console.error(`📝 [GameTabService] [${e}] ✅ Updated ${a} subtabs successfully`)}catch(s){throw console.error(`📝 [GameTabService] [${e}] ❌ Failed to update subtabs:`,s),s}}}const St=new Le;class bt{static getCurrentUser(){return O.get(I.USER,null)}static setCurrentUser(e){O.set(I.USER,e)}static createUser(e,t=ve.FREE){const s=Date.now(),r=ie[t];return{id:`user_${s}`,authUserId:`user_${s}`,email:e,tier:t,hasProfileSetup:!1,hasSeenSplashScreens:!1,hasSeenHowToUse:!1,hasSeenFeaturesConnected:!1,hasSeenProFeatures:!1,pcConnected:!1,pcConnectionSkipped:!1,onboardingCompleted:!1,hasWelcomeMessage:!1,isNewUser:!0,hasUsedTrial:!1,lastActivity:s,preferences:{},textCount:0,imageCount:0,textLimit:r.text,imageLimit:r.image,totalRequests:0,lastReset:s,usage:{textCount:0,imageCount:0,textLimit:r.text,imageLimit:r.image,totalRequests:0,lastReset:s,tier:t},appState:{},profileData:{},onboardingData:{},behaviorData:{},feedbackData:{},usageData:{},createdAt:s,updatedAt:s}}static updateUser(e){const t=this.getCurrentUser();if(!t)return;const s={...t,...e,updatedAt:Date.now()};this.setCurrentUser(s)}static updateUsage(e){const t=this.getCurrentUser();t&&this.updateUser({usage:{...t.usage,...e}})}static resetUsage(){const e=this.getCurrentUser();if(!e)return;const t=ie[e.tier];this.updateUsage({textCount:0,imageCount:0,totalRequests:0,lastReset:Date.now(),textLimit:t.text,imageLimit:t.image})}static canMakeRequest(e){const t=this.getCurrentUser();if(!t)return!1;const{usage:s}=t;return e==="text"?s.textCount<s.textLimit:s.imageCount<s.imageLimit}static incrementUsage(e){const t=this.getCurrentUser();if(!t)return;const s={totalRequests:t.usage.totalRequests+1};e==="text"?s.textCount=t.usage.textCount+1:s.imageCount=t.usage.imageCount+1,this.updateUsage(s)}static logout(){O.remove(I.USER)}static async getCurrentUserAsync(){try{const e=O.get(I.USER,null),{data:{user:t},error:s}=await l.auth.getUser();if(s||!t)return e;const{data:r,error:a}=await l.from("users").select("*").eq("auth_user_id",t.id).single();if(a||!r)return console.error("Failed to fetch user from Supabase:",a),e;const n={id:r.id,authUserId:r.auth_user_id,email:r.email,tier:r.tier,textCount:r.text_count||0,imageCount:r.image_count||0,textLimit:q(r.text_limit),imageLimit:q(r.image_limit),totalRequests:r.total_requests||0,lastReset:M(r.last_reset),hasProfileSetup:r.has_profile_setup||!1,hasSeenSplashScreens:r.has_seen_splash_screens||!1,hasSeenHowToUse:r.has_seen_how_to_use||!1,hasSeenFeaturesConnected:r.has_seen_features_connected||!1,hasSeenProFeatures:r.has_seen_pro_features||!1,pcConnected:r.pc_connected||!1,pcConnectionSkipped:r.pc_connection_skipped||!1,onboardingCompleted:r.onboarding_completed||!1,hasWelcomeMessage:r.has_welcome_message||!1,isNewUser:r.is_new_user||!1,hasUsedTrial:r.has_used_trial||!1,lastActivity:M(r.updated_at),preferences:N(r.preferences),usage:{textCount:r.text_count||0,imageCount:r.image_count||0,textLimit:q(r.text_limit),imageLimit:q(r.image_limit),totalRequests:r.total_requests||0,lastReset:M(r.last_reset),tier:r.tier},appState:N(r.app_state),profileData:N(r.profile_data),onboardingData:N(r.onboarding_data),behaviorData:N(r.behavior_data),feedbackData:N(r.feedback_data),usageData:N(r.usage_data),createdAt:M(r.created_at),updatedAt:M(r.updated_at)};return O.set(I.USER,n),n}catch(e){return console.error("Error in getCurrentUserAsync:",e),O.get(I.USER,null)}}static async setCurrentUserAsync(e){try{O.set(I.USER,e);const{error:t}=await l.from("users").update({tier:e.tier,text_count:e.textCount,image_count:e.imageCount,text_limit:e.textLimit,image_limit:e.imageLimit,total_requests:e.totalRequests,last_reset:new Date(e.lastReset).toISOString(),has_profile_setup:e.hasProfileSetup,has_seen_splash_screens:e.hasSeenSplashScreens,has_seen_how_to_use:e.hasSeenHowToUse,has_seen_features_connected:e.hasSeenFeaturesConnected,has_seen_pro_features:e.hasSeenProFeatures,pc_connected:e.pcConnected,pc_connection_skipped:e.pcConnectionSkipped,onboarding_completed:e.onboardingCompleted,has_welcome_message:e.hasWelcomeMessage,has_used_trial:e.hasUsedTrial,preferences:e.preferences,profile_data:e.profileData,app_state:e.appState,onboarding_data:e.onboardingData,behavior_data:e.behaviorData,feedback_data:e.feedbackData,usage_data:e.usageData,updated_at:new Date().toISOString()}).eq("auth_user_id",e.authUserId);t&&console.error("Failed to sync user to Supabase:",t)}catch(t){console.error("Error in setCurrentUserAsync:",t)}}static async updateUsageAsync(e){const t=await this.getCurrentUserAsync();if(!t)return;const s={...t,usage:{...t.usage,...e},textCount:e.textCount??t.textCount,imageCount:e.imageCount??t.imageCount,totalRequests:e.totalRequests??t.totalRequests,lastReset:e.lastReset??t.lastReset,updatedAt:Date.now()};await this.setCurrentUserAsync(s)}}class We{hasTabCommand(e){return/^@\w+/.test(e.trim())}parseTabCommand(e,t){const s=e.trim();if(!this.hasTabCommand(s))return null;const r=s.match(/^@(\w+)\s*(\\modify|\\delete)?\s*(.*)$/);if(!r)return null;const[,a,n,c]=r,i=this.findMatchingTab(a,t.subtabs||[]);if(!i)return console.warn(`Tab "${a}" not found in conversation`),null;let u;return n==="\\delete"?u="delete":n==="\\modify"?u="modify":u="update",{type:u,tabId:i.id,tabName:i.title,instruction:c.trim()}}findMatchingTab(e,t){const s=this.normalizeTabName(e);let r=t.find(a=>this.normalizeTabName(a.id)===s||this.normalizeTabName(a.title)===s);return r||(r=t.find(a=>this.normalizeTabName(a.id).includes(s)||this.normalizeTabName(a.title).includes(s)),r)?r:(r=t.find(a=>s.includes(this.normalizeTabName(a.id))||s.includes(this.normalizeTabName(a.title))),r||null)}normalizeTabName(e){return e.toLowerCase().replace(/[_\s-]+/g,"").replace(/[^a-z0-9]/g,"")}getAvailableTabNames(e){return!e.subtabs||e.subtabs.length===0?[]:e.subtabs.map(t=>({id:t.id,title:t.title,variations:[t.id,t.title,t.id.replace(/_/g," "),t.title.toLowerCase()]})).map(t=>t.id)}formatTabSuggestion(e,t){return`@${e}`}getCommandHelp(){return`
**Tab Commands:**
...
Example: @story_so_far The player defeated the first boss
    `.trim()}validateCommand(e){switch(e.type){case"update":if(!e.instruction)return{valid:!1,error:"Update command requires content. Example: @story_so_far The player..."};break;case"modify":if(!e.instruction)return{valid:!1,error:"Modify command requires instructions. Example: @tips \\modify Change to combat strategies"};break}return{valid:!0}}describeCommand(e){switch(e.type){case"update":return`Updating "${e.tabName}" with: ${e.instruction}`;case"modify":return`Modifying "${e.tabName}": ${e.instruction}`;case"delete":return`Deleting "${e.tabName}"`}}}const wt=new We;let m,U=[],ce=!1,W="",L=null,A=null,v=null,te=!1;const He="otakonSpeechRate",re=async()=>{try{"wakeLock"in navigator&&(L=await navigator.wakeLock.request("screen"),L.addEventListener("release",()=>{console.log("Wake Lock released - attempting to reacquire..."),m&&m.speaking&&!te&&re()}))}catch(o){console.warn("Wake Lock not supported or failed:",o)}},de=async()=>{try{L!==null&&(await L.release(),L=null)}catch(o){console.warn("Wake Lock release failed:",o)}},ze=()=>{try{A||(A=new(window.AudioContext||window.webkitAudioContext)),v||(v=new Audio,v.src="data:audio/wav;base64,UklGRigAAABXQVZFZm10IBIAAAABAAEARKwAAIhYAQACABAAAABkYXRhAgAAAAEA",v.loop=!0,v.volume=.01,A&&A.createMediaElementSource(v).connect(A.destination))}catch(o){console.warn("Audio Context initialization failed:",o)}},ge=async()=>{try{v&&A&&(A.state==="suspended"&&await A.resume(),await v.play())}catch(o){console.warn("Silent audio playback failed:",o)}},me=()=>{try{v&&(v.pause(),v.currentTime=0)}catch(o){console.warn("Silent audio stop failed:",o)}},qe=()=>new Promise((o,e)=>{if(!m)return e(new Error("Speech synthesis not initialized."));if(U=m.getVoices(),U.length>0){o();return}m.onvoiceschanged=()=>{U=m.getVoices(),o()},setTimeout(()=>{U.length===0&&(console.warn("TTS voices did not load within a reasonable time."),U=m.getVoices()),o()},1e3)}),H=()=>{m&&m.speaking&&m.cancel(),W="","mediaSession"in navigator&&navigator.mediaSession.playbackState!=="none"&&(navigator.mediaSession.playbackState="paused"),de(),me(),window.dispatchEvent(new CustomEvent("otakon:ttsStopped"))},Ke=()=>{m&&m.speaking&&!m.paused&&(m.pause(),"mediaSession"in navigator&&(navigator.mediaSession.playbackState="paused"),window.dispatchEvent(new CustomEvent("otakon:ttsPaused")))},je=()=>{m&&m.paused&&(m.resume(),"mediaSession"in navigator&&(navigator.mediaSession.playbackState="playing"),window.dispatchEvent(new CustomEvent("otakon:ttsResumed")))},Be=async()=>{W&&(H(),await pe(W))},Ye=()=>m?m.speaking:!1,le=()=>{H(),window.dispatchEvent(new CustomEvent("otakon:disableHandsFree"))},Je=()=>{"mediaSession"in navigator&&(navigator.mediaSession.setActionHandler("play",()=>{}),navigator.mediaSession.setActionHandler("pause",le),navigator.mediaSession.setActionHandler("stop",le))},Ve=async()=>{document.hidden?(te=!0,console.log("App backgrounded - maintaining TTS audio"),m&&m.speaking&&await ge()):(te=!1,console.log("App foregrounded - resuming normal operation"),m&&m.speaking&&await re())},Xe=async()=>{if(typeof window<"u"&&"speechSynthesis"in window){if(ce)return;ce=!0,m=window.speechSynthesis,await qe(),Je(),ze(),document.addEventListener("visibilitychange",Ve),m.getVoices().length===0&&m.speak(new SpeechSynthesisUtterance(""))}else console.warn("Speech Synthesis not supported in this browser.")},he=()=>U.filter(o=>o.lang.startsWith("en-")),pe=async o=>new Promise((e,t)=>{try{if(!m)return console.error("Text-to-Speech is not available on this browser."),t(new Error("Text-to-Speech is not available on this browser."));if(!o.trim())return e();H(),W=o;const s=new SpeechSynthesisUtterance(o),r=localStorage.getItem(He);s.rate=r?parseFloat(r):.94;const a=localStorage.getItem("otakonPreferredVoiceURI"),n=he();let c;if(a&&(c=n.find(i=>i.voiceURI===a)),!c&&n.length>0){const i=n.find(u=>u.name.toLowerCase().includes("female"));i?c=i:c=n[0]}c&&(s.voice=c),s.onstart=async()=>{await re(),await ge(),"serviceWorker"in navigator&&navigator.serviceWorker.controller&&navigator.serviceWorker.controller.postMessage({type:"TTS_STARTED"}),"mediaSession"in navigator&&(navigator.mediaSession.playbackState="playing",navigator.mediaSession.metadata=new MediaMetadata({title:o.length>50?o.substring(0,50)+"...":o,artist:"Your AI Gaming Companion",album:"Otakon",artwork:[{src:"/Otagon/icon-192.png",sizes:"192x192",type:"image/png"},{src:"/Otagon/icon-512.png",sizes:"512x512",type:"image/png"}]})),window.dispatchEvent(new CustomEvent("otakon:ttsStarted"))},s.onend=()=>{W="","mediaSession"in navigator&&(navigator.mediaSession.playbackState="paused"),"serviceWorker"in navigator&&navigator.serviceWorker.controller&&navigator.serviceWorker.controller.postMessage({type:"TTS_STOPPED"}),de(),me(),window.dispatchEvent(new CustomEvent("otakon:ttsStopped")),e()},s.onerror=i=>{console.error("SpeechSynthesis Utterance Error",i),H(),t(i)},m.speak(s)}catch(s){console.error("TTS Error:",s),t(s)}}),vt={init:Xe,getAvailableVoices:he,speak:pe,cancel:H,pause:Ke,resume:je,restart:Be,isSpeaking:Ye};class Tt{static async migrateMessagesAtomic(e,t,s){const r=await y.getConversations(!1),a=r[t],n=r[s];if(!a)throw console.error("📦 [MessageRouting] Source conversation not found:",t),console.error("📦 [MessageRouting] Available conversations:",Object.keys(r)),new Error(`Source conversation ${t} not found`);if(!n)throw console.error("📦 [MessageRouting] Destination conversation not found:",s),console.error("📦 [MessageRouting] Available conversations:",Object.keys(r)),new Error(`Destination conversation ${s} not found`);const c=a.messages.filter(d=>e.includes(d.id));if(c.length===0){console.warn("⚠️ [MessageRouting] No messages found to migrate");return}const i=c.filter(d=>!n.messages.some(f=>f.id===d.id)),u={...r,[s]:{...n,messages:[...n.messages,...i],updatedAt:Date.now()},[t]:{...a,messages:a.messages.filter(d=>!e.includes(d.id)),updatedAt:Date.now()}};await y.setConversations(u)}static shouldRouteMessage(e,t,s){return!t||e===t?!1:!!(s&&t)}static messageExists(e,t){return e.some(s=>s.id===t)}}class Ze{constructor(){g(this,"MAX_WORDS",300);g(this,"RECENT_MESSAGE_COUNT",8)}countWords(e){return e.trim().split(/\s+/).filter(t=>t.length>0).length}getTotalWordCount(e){return e.reduce((t,s)=>{const r=this.countWords(s.content);return t+r},0)}shouldSummarize(e){if(!e.messages||e.messages.length<=this.RECENT_MESSAGE_COUNT)return!1;const t=this.getTotalWordCount(e.messages);return console.log(`📊 [ContextSummarization] Total words in conversation: ${t}`),t>this.MAX_WORDS*3}splitMessages(e){if(e.length<=this.RECENT_MESSAGE_COUNT)return{toSummarize:[],toKeep:e};const t=e.length-this.RECENT_MESSAGE_COUNT;return{toSummarize:e.slice(0,t),toKeep:e.slice(t)}}async summarizeMessages(e,t,s){console.log(`📝 [ContextSummarization] Summarizing ${e.length} messages`);const r=this.getTotalWordCount(e),a=e.map(i=>`${i.role==="user"?"User":"Assistant"}: ${i.content}`).join(`
...
Provide ONLY the summary, no additional commentary.`;try{const i={id:"temp-summary",title:"Summary Request",messages:[{id:"summary-msg-"+Date.now(),role:"user",content:c,timestamp:Date.now()}],createdAt:Date.now(),updatedAt:Date.now(),isActive:!1,isGameHub:!1},u={id:"system",email:"system@otakon.ai",profileData:null},f=(await ue.getChatResponse(i,u,c,!1,!1)).content.trim(),S=this.countWords(f);return console.log(`✅ [ContextSummarization] Summary generated: ${S} words (reduced from ${r})`),{summary:f,wordCount:S,messagesIncluded:e.length,originalWordCount:r}}catch(i){console.error("❌ [ContextSummarization] Failed to generate summary:",i);const u=e.slice(0,5).map(d=>d.content.substring(0,100)).join(" ... ").substring(0,this.MAX_WORDS*6);return{summary:`[Previous conversation context] ${u}`,wordCount:this.countWords(u),messagesIncluded:e.length,originalWordCount:r}}}async applyContextSummarization(e){if(!this.shouldSummarize(e))return console.log("📊 [ContextSummarization] No summarization needed"),e;console.log("🔄 [ContextSummarization] Applying context summarization...");const{toSummarize:t,toKeep:s}=this.splitMessages(e.messages);if(t.length===0)return e;const r=await this.summarizeMessages(t,e.gameTitle,e.genre),n=[{id:"summary-"+Date.now(),role:"system",content:r.summary,timestamp:t[t.length-1].timestamp,metadata:{isSummary:!0,messagesIncluded:r.messagesIncluded,originalWordCount:r.originalWordCount,summaryWordCount:r.wordCount}},...s],c=r.summary.replace(/!\[.*?\]\(data:image\/.*?\)/g,""),i=c.split(/\s+/).filter(d=>d.length>0),u=i.length>500?i.slice(0,500).join(" ")+"...":c;return console.log(`✅ [ContextSummarization] Context optimized: ${e.messages.length} messages → ${n.length} (${r.originalWordCount} words → ${r.wordCount} + recent)`),{...e,messages:n,contextSummary:u,lastSummarizedAt:Date.now(),updatedAt:Date.now()}}async getOptimizedContext(e){return this.shouldSummarize(e)?(await this.applyContextSummarization(e)).messages:e.messages}willTriggerSummarization(e){return this.getTotalWordCount(e.messages)>this.MAX_WORDS*3*.8}}const Qe=new Ze,Ct=Object.freeze(Object.defineProperty({__proto__:null,contextSummarizationService:Qe},Symbol.toStringTag,{value:"Module"}));export{K as E,Tt as M,O as S,bt as U,lt as W,w as a,gt as b,ut as c,pt as d,ht as e,$ as f,mt as g,vt as h,ft as i,St as j,wt as k,Pe as l,yt as m,ct as n,ot as o,dt as p,Ct as q,it as s,G as t};
//# sourceMappingURL=services-D8gnUPMu.js.map
