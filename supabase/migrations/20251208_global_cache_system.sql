...
CREATE INDEX idx_game_knowledge_igdb_id ON public.game_knowledge_cache(igdb_id);
CREATE INDEX idx_game_knowledge_game_slug ON public.game_knowledge_cache(game_slug);
CREATE INDEX idx_game_knowledge_release_date ON public.game_knowledge_cache(release_date);
CREATE INDEX idx_game_knowledge_is_unreleased ON public.game_knowledge_cache(is_unreleased) WHERE is_unreleased = true;
...
CREATE INDEX idx_news_cache_prompt_hash ON public.news_cache(prompt_hash);
...
-- Update comment
COMMENT ON TABLE public.igdb_game_cache IS 'Global cache for IGDB API responses. 24-hour TTL for unreleased games, permanent for released games.';
...
-- Indexes
CREATE INDEX idx_unreleased_tabs_user ON public.unreleased_game_tabs(auth_user_id);
CREATE INDEX idx_unreleased_tabs_igdb ON public.unreleased_game_tabs(igdb_id);
...
-- Indexes
CREATE INDEX idx_subtab_refresh_user_date ON public.subtab_refresh_usage(auth_user_id, refresh_date);
...
-- Note: games table already exists with columns: title, genre, platform, cover_url,
--       notes, status (playing/completed/backlog/wishlist), progress, playtime_hours,
--       rating, tags, metadata, user_id, auth_user_id
-- ============================================================================
...
-- Note: Using DO block to handle constraint existence gracefully
DO $$ 
BEGIN
    IF NOT EXISTS (
...
-- Indexes
CREATE INDEX idx_screenshots_user ON public.user_screenshots(auth_user_id);
CREATE INDEX idx_screenshots_game ON public.user_screenshots(detected_game_id);
CREATE INDEX idx_screenshots_conversation ON public.user_screenshots(conversation_id);
CREATE INDEX idx_screenshots_captured_at ON public.user_screenshots(captured_at DESC);
...
-- Indexes
CREATE INDEX idx_timeline_user_date ON public.user_timeline(auth_user_id, event_date DESC);
CREATE INDEX idx_timeline_game ON public.user_timeline(igdb_game_id);
CREATE INDEX idx_timeline_event_type ON public.user_timeline(event_type);
...
-- Unreleased Tabs: Users can manage their own
...
-- Subtab Refresh Usage: Users can manage their own
...
-- Note: games table RLS policies already exist in production

-- Screenshots: Users can manage their own
...
-- Timeline: Users can manage their own
...
-- Function: Get or fetch game knowledge
...
-- Function: Check news cache validity
...
-- Function: Count unreleased tabs for user
...
-- Function: Get daily subtab refresh count
...
-- Function: Increment subtab refresh count
...
-- Clean up expired news cache (runs daily at 2 AM)
...
GRANT ALL ON TABLE public.user_game_library TO authenticated;
GRANT ALL ON TABLE public.user_game_library TO service_role;
...
-- Note: games table permissions already exist in production