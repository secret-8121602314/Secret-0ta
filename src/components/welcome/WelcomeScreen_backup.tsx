...
const WelcomeScreen: React.FC<WelcomeScreenProps> = ({ onStartChat, onAddGame: _onAddGame }) => {
  ...
  const tabs = [
    { id: 'overview' as TabType, label: 'Overview', icon: BookOpenIcon },
    { id: 'features' as TabType, label: 'Features', icon: SparklesIcon },
    { id: 'advanced' as TabType, label: 'Advanced', icon: AcademicCapIcon },
    { id: 'hotkeys' as TabType, label: 'PC Hotkeys', icon: ComputerDesktopIcon },
    { id: 'best-practices' as TabType, label: 'Best Practices', icon: LightBulbIcon }
  ];

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-md p-2 sm:p-4">
      {/* Modal Container */}
      <div className="relative w-full max-w-6xl h-[95vh] sm:max-h-[90vh] bg-gradient-to-br from-[#1a1a1a] via-[#1e1e1e] to-[#1a1a1a] rounded-xl sm:rounded-2xl shadow-2xl border border-primary/20 flex flex-col overflow-hidden">
        
        {/* Header */}
        <div className="flex-shrink-0 relative border-b border-surface-light/20 bg-gradient-to-r from-primary/5 via-transparent to-secondary/5">
            ...
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                <div className="p-2 rounded-xl bg-gradient-to-br from-primary/20 to-secondary/20 border border-primary/30">
                  <BookOpenIcon className="w-6 h-6 sm:w-7 sm:h-7 text-primary" />
                </div>
                <div>
                  <h1 className="text-xl sm:text-2xl md:text-3xl font-bold text-white">Otagon Guide</h1>
                  <p className="text-xs sm:text-sm text-text-muted mt-0.5">Your AI Gaming Companion</p>
                </div>
              </div>
              <button
                onClick={onStartChat}
                className="p-2 hover:bg-surface-light/20 rounded-lg transition-colors group"
                aria-label="Close guide"
              >
                <XMarkIcon className="w-6 h-6 text-text-muted group-hover:text-white" />
              </button>
            </div>

            {/* Tabs */}
            <div className="flex gap-1 sm:gap-2 overflow-x-auto pb-0 scrollbar-hide" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
              {tabs.map((tab) => {
                const Icon = tab.icon;
                return (
                  <button
                    ...
                    className={`flex items-center gap-2 px-3 sm:px-4 py-2.5 min-h-[44px] text-xs sm:text-sm font-medium whitespace-nowrap transition-all duration-200 rounded-t-lg flex-shrink-0 ${
                      activeTab === tab.id
                        ? 'bg-surface-light/30 text-white border-b-2 border-primary'
                        : 'text-text-muted hover:text-white hover:bg-surface-light/10'
                    }`}
                  >
        ...
        <div className="flex-1 overflow-y-auto px-3 sm:px-4 md:px-6 py-4 sm:py-6 custom-scrollbar">
        ...
        <div className="flex-shrink-0 py-3 sm:py-4 px-4 sm:px-6 border-t border-surface-light/20 bg-surface/30 backdrop-blur-sm">
          <div className="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center justify-center">
            <button
              onClick={onStartChat}
              className="inline-flex items-center justify-center gap-2 px-6 sm:px-8 py-3 text-sm sm:text-base font-bold text-white bg-gradient-to-r from-primary to-secondary hover:from-primary/90 hover:to-secondary/90 rounded-xl shadow-lg hover:shadow-xl transition-all duration-200 hover:scale-105 active:scale-95 min-h-[48px]"
            >
...
const OverviewTab = () => (
  <div className="space-y-4 sm:space-y-6">
    {/* Welcome Hero Card */}
    <div className="bg-gradient-to-br from-primary/10 via-surface/50 to-secondary/10 border border-primary/30 rounded-2xl p-6 sm:p-8 shadow-xl">
      <div className="flex items-start gap-4 mb-4">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm sm:text-base text-text-secondary leading-relaxed">
            Your intelligent AI gaming companion. Get spoiler-free help, discover secrets, and master any game without leaving your screen.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 mt-6">
        <div className="bg-gradient-to-br from-blue-500/10 to-blue-600/5 border border-blue-500/20 rounded-xl p-4">
          <PhotoIcon className="w-8 h-8 text-blue-400 mb-3" />
          <h3 className="font-semibold text-white mb-2 text-sm sm:text-base">Screenshot Analysis</h3>
          <p className="text-xs sm:text-sm text-text-secondary">Upload any game screenshot for instant tactical insights</p>
        </div>
        <div className="bg-gradient-to-br from-purple-500/10 to-purple-600/5 border border-purple-500/20 rounded-xl p-4">
          <DevicePhoneMobileIcon className="w-8 h-8 text-purple-400 mb-3" />
          <h3 className="font-semibold text-white mb-2 text-sm sm:text-base">Game-Specific Tabs</h3>
          <p className="text-xs sm:text-sm text-text-secondary">Auto-organized tabs for each game you play</p>
        </div>
        <div className="bg-gradient-to-br from-green-500/10 to-green-600/5 border border-green-500/20 rounded-xl p-4">
          <BookOpenIcon className="w-8 h-8 text-green-400 mb-3" />
          <h3 className="font-semibold text-white mb-2 text-sm sm:text-base">Smart Insight Tabs</h3>
          <p className="text-xs sm:text-sm text-text-secondary">AI auto-generates Story, Tips, and Builds tabs</p>
        </div>
        <div className="bg-gradient-to-br from-orange-500/10 to-orange-600/5 border border-orange-500/20 rounded-xl p-4">
          <BoltIcon className="w-8 h-8 text-orange-400 mb-3" />
          <h3 className="font-semibold text-white mb-2 text-sm sm:text-base">Dual Modes</h3>
          <p className="text-xs sm:text-sm text-text-secondary">Switch between Playing and Planning modes</p>
        </div>
      </div>
    </div>

    {/* Quick Start Guide */}
    <div className="bg-surface/50 border border-surface-light/30 rounded-xl p-5 sm:p-6">
      <div className="flex items-center gap-3 mb-4">
        <RocketLaunchIcon className="w-6 h-6 text-primary" />
        <h2 className="text-lg sm:text-xl font-bold text-white">Quick Start Guide</h2>
      </div>
      <ol className="space-y-4">
        <li className="flex items-start gap-3">
          <span className="flex-shrink-0 w-7 h-7 sm:w-8 sm:h-8 bg-gradient-to-r from-primary to-secondary rounded-full flex items-center justify-center text-white font-bold text-sm shadow-lg">1</span>
          <div className="flex-1 pt-0.5">
            <h3 className="font-semibold text-white mb-1 text-sm sm:text-base">Start in Game Hub</h3>
            <p className="text-xs sm:text-sm text-text-secondary">Ask general gaming questions or upload screenshots to get started</p>
          </div>
        </li>
        <li className="flex items-start gap-3">
          <span className="flex-shrink-0 w-7 h-7 sm:w-8 sm:h-8 bg-gradient-to-r from-primary to-secondary rounded-full flex items-center justify-center text-white font-bold text-sm shadow-lg">2</span>
          <div className="flex-1 pt-0.5">
            <h3 className="font-semibold text-white mb-1 text-sm sm:text-base">Create Game Tabs</h3>
            <p className="text-xs sm:text-sm text-text-secondary">Mention a game name or upload its screenshot to create a dedicated tab automatically</p>
          </div>
        </li>
        <li className="flex items-start gap-3">
          <span className="flex-shrink-0 w-7 h-7 sm:w-8 sm:h-8 bg-gradient-to-r from-primary to-secondary rounded-full flex items-center justify-center text-white font-bold text-sm shadow-lg">3</span>
          <div className="flex-1 pt-0.5">
            <h3 className="font-semibold text-white mb-1 text-sm sm:text-base">Explore Insight Tabs</h3>
            <p className="text-xs sm:text-sm text-text-secondary">AI automatically generates specialized tabs (Story, Tips, Builds) as you chat</p>
          </div>
        </li>
        <li className="flex items-start gap-3">
          <span className="flex-shrink-0 w-7 h-7 sm:w-8 sm:h-8 bg-gradient-to-r from-primary to-secondary rounded-full flex items-center justify-center text-white font-bold text-sm shadow-lg">4</span>
          <div className="flex-1 pt-0.5">
            <h3 className="font-semibold text-white mb-1 text-sm sm:text-base">Toggle Modes</h3>
            <p className="text-xs sm:text-sm text-text-secondary">Switch between Playing mode (real-time help) and Planning mode (strategy)</p>
          </div>
        </li>
      </ol>
    </div>
    ...
    {/* Pro Tip */}
    <div className="bg-gradient-to-r from-blue-500/10 to-blue-600/5 border border-blue-500/30 rounded-xl p-4 sm:p-5">
      <div className="flex items-start gap-3">
        <LightBulbIcon className="w-6 h-6 text-blue-400 flex-shrink-0 mt-0.5" />
        <div>
          <h3 className="font-semibold text-white mb-1.5 text-sm sm:text-base">Pro Tip</h3>
          <p className="text-xs sm:text-sm text-text-secondary leading-relaxed">
            Connect the Otagon PC client to capture screenshots with hotkeys without alt-tabbing. 
            Check the PC Hotkeys tab for setup instructions!
          </p>
        </div>
      </div>
    </div>
...
const FeaturesTab = () => {
  const features = [
    ...
    },
    {
      ...
      tips: [
        ...
        'Essential for games released after December 2024',
  ...
  const colorMap: Record<string, { bg: string; border: string; icon: string }> = {
    blue: { bg: 'from-blue-500/10 to-blue-600/5', border: 'border-blue-500/30', icon: 'text-blue-400' },
    purple: { bg: 'from-purple-500/10 to-purple-600/5', border: 'border-purple-500/30', icon: 'text-purple-400' },
    green: { bg: 'from-green-500/10 to-green-600/5', border: 'border-green-500/30', icon: 'text-green-400' },
    orange: { bg: 'from-orange-500/10 to-orange-600/5', border: 'border-orange-500/30', icon: 'text-orange-400' },
    indigo: { bg: 'from-indigo-500/10 to-indigo-600/5', border: 'border-indigo-500/30', icon: 'text-indigo-400' },
    cyan: { bg: 'from-cyan-500/10 to-cyan-600/5', border: 'border-cyan-500/30', icon: 'text-cyan-400' },
    emerald: { bg: 'from-emerald-500/10 to-emerald-600/5', border: 'border-emerald-500/30', icon: 'text-emerald-400' },
    violet: { bg: 'from-violet-500/10 to-violet-600/5', border: 'border-violet-500/30', icon: 'text-violet-400' },
    pink: { bg: 'from-pink-500/10 to-pink-600/5', border: 'border-pink-500/30', icon: 'text-pink-400' },
    sky: { bg: 'from-sky-500/10 to-sky-600/5', border: 'border-sky-500/30', icon: 'text-sky-400' },
    teal: { bg: 'from-teal-500/10 to-teal-600/5', border: 'border-teal-500/30', icon: 'text-teal-400' },
    amber: { bg: 'from-amber-500/10 to-amber-600/5', border: 'border-amber-500/30', icon: 'text-amber-400' },
    yellow: { bg: 'from-yellow-500/10 to-yellow-600/5', border: 'border-yellow-500/30', icon: 'text-yellow-400' },
    rose: { bg: 'from-rose-500/10 to-rose-600/5', border: 'border-rose-500/30', icon: 'text-rose-400' }
  };
    ...
    <div className="grid grid-cols-1 gap-4 sm:gap-5">
      {features.map((feature, index) => {
        ...
        return (
          <div
            key={index}
            className={`bg-gradient-to-br ${colors.bg} border ${colors.border} rounded-xl p-4 sm:p-5 hover:scale-[1.02] transition-all duration-300 shadow-lg hover:shadow-xl`}
          >
            <div className="flex items-start gap-4">
              <div className={`p-3 rounded-lg bg-surface/50 border ${colors.border} flex-shrink-0`}>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  {feature.isPro && (
                    <span className="px-2 py-0.5 bg-gradient-to-r from-primary to-secondary rounded text-[10px] sm:text-xs font-semibold text-white">PRO</span>
                  )}
                </div>
                <p className="text-xs sm:text-sm text-text-secondary mb-3 leading-relaxed">{feature.description}</p>
                <div className="space-y-1.5">
                  {feature.tips.map((tip, tipIndex) => (
                    <div key={tipIndex} className="flex items-start gap-2 text-xs sm:text-sm">
                      <ChevronRightIcon className={`w-4 h-4 ${colors.icon} flex-shrink-0 mt-0.5`} />
