  ...
  return (
    <div className={`relative ${className}`}>
      ...
      {showLabel && (
        <div className="flex items-center justify-between mb-1.5">
          <div className="flex items-center gap-2">
            <svg className="w-3.5 h-3.5 text-[#FF4D4D]" fill="currentColor" viewBox="0 0 20 20">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-bold text-[#FF4D4D] min-w-[36px] text-right">
              {clampedProgress}%
            </span>
          </div>
        </div>
      ...
      <div className="relative h-3 bg-[#1A1A1A] rounded-full overflow-hidden border border-[#333333]/60 shadow-inner">
        ...
        <div
          ...
          style={{ 
            ...
            background: clampedProgress > 0 
              ? 'linear-gradient(90deg, #FF4D4D 0%, #FF6B6B 40%, #FFAB40 70%, #FFD93D 100%)'
              : 'transparent',
            boxShadow: clampedProgress > 0 ? '0 0 12px rgba(255,77,77,0.4), inset 0 1px 0 rgba(255,255,255,0.2)' : 'none'
          }}
        />
        ...
        {clampedProgress > 0 && (
          <div
            className="absolute top-1/2 w-4 h-4 bg-white rounded-full shadow-lg border-2 border-[#FF4D4D] transition-all duration-700 ease-out z-10"
            style={{ 
          ...
          >
            {/* Glow effect */}
            <div className="absolute inset-[-4px] bg-[#FF4D4D]/20 rounded-full blur-sm" />
          </div>
        )}
