...
const ProUpgradeSplashScreen: React.FC<ProUpgradeSplashScreenProps> = ({ 
  ...
  return (
    <div className="fixed inset-0 bg-gradient-to-br from-black/95 to-[#0A0A0A]/95 backdrop-blur-xl flex items-center justify-center p-3 sm:p-4 md:p-6 z-50 animate-fade-in overflow-y-auto">
      <div className="w-full max-w-lg mx-auto animate-scale-in my-2 sm:my-6">
        {/* Card container */}
        <div className="bg-gradient-to-b from-[#1A1A1A]/90 to-[#0D0D0D]/90 border border-[#2A2A2A] rounded-2xl sm:rounded-3xl p-3 sm:p-5 md:p-6 shadow-2xl flex flex-col h-auto">
          
          {/* Mascot with glow - reduced size on mobile */}
          <div className="relative flex justify-center mb-2 sm:mb-4 flex-shrink-0">
            {/* Glow effect */}
          ...
          {/* Features grid - 3 columns on mobile for better space usage */}
          <div className="grid grid-cols-3 sm:grid-cols-2 gap-1.5 sm:gap-3 mb-3 sm:mb-5 flex-shrink-0">
            {features.map((feature) => (
              ...
              >
                <div className="flex flex-col items-center gap-1">
                  ...
                  <h3 className="text-[10px] sm:text-sm font-semibold text-[#F5F5F5] leading-tight text-center">{feature.title}</h3>
                  <p className="text-[8px] sm:text-xs text-[#A3A3A3] leading-tight text-center">{feature.description}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          {/* Pro tips */}
          <div className="bg-[#1E1E1E]/60 border border-[#333333]/60 rounded-lg sm:rounded-xl p-2.5 sm:p-4 mb-3 sm:mb-5 flex-shrink-0">
            <h3 className="text-xs sm:text-sm font-semibold text-[#FFAB40] mb-1.5 sm:mb-2 flex items-center gap-2">
              <svg className="w-3 h-3 sm:w-4 sm:h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" />
              </svg>
              Pro Tips
            </h3>
            <ul className="text-[9px] sm:text-xs text-[#CFCFCF] space-y-0.5 sm:space-y-1">
              <li className="flex items-start gap-1">
                <span className="text-[#FFAB40] mt-0.5 flex-shrink-0">•</span>
                <span>Press <span className="font-mono bg-[#2E2E2E] px-1 py-0.5 rounded text-[#FFAB40] text-[8px] sm:text-xs">F2</span> on PC for batch screenshots</span>
              </li>
              <li className="flex items-start gap-1">
                <span className="text-[#FFAB40] mt-0.5 flex-shrink-0">•</span>
          ...
          {/* Status indicator */}
          <div className="flex items-center justify-center gap-2 mb-3 sm:mb-5 flex-shrink-0">
          ...
          <button
            onClick={onClose}
            className="w-full bg-gradient-to-r from-[#E53A3A] to-[#D98C1F] text-white font-bold py-2.5 sm:py-4 px-6 rounded-xl text-sm sm:text-base transition-all duration-300 active:scale-[0.98] hover:shadow-lg hover:shadow-[#E53A3A]/25 flex-shrink-0"
          >
            Let's Go!
          </button>
        </div>
