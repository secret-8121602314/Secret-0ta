...
`),t=t.replace(/\*\*\s+Release\s+Date\s*:\s*\*\*/gi,"**Release Date:**"),t=t.replace(/\*\*\s+The\s+Verdict\s*:\s*\*\*/gi,"**The Verdict:**"),t=t.replace(/\*\*\s+Key\s+Features\s*:\s*\*\*/gi,"**Key Features:**"),t=t.replace(/\*\*\s+([A-Za-z][A-Za-z\s]+?)(?=\s+(?:is|are|was|were|has|have|and|or|but|the|a|an|of|to|in|on|at|for|with|as|by|from|serves?|often|usually)\s)/gi,"$1"),t=t.replace(/(\b[A-Za-z]+)\s*\*\*(?=,|\s|\.)/g,"$1"),t=t.replace(/\*\*\s*([^*\n]{3,}?)([.!?])(?!\*\*)/g,"$1$2"),(t.match(/\*\*/g)||[]).length%2!==0&&(t=t.replace(/\*\*\s*$/g,""),t=t.replace(/^\s*\*\*/g,""),t=t.replace(/\s\*\*\s+([A-Z])/g," $1"),t=t.replace(/(\w)\*\*(?=[\s,.])/g,"$1")),console.log(`🏷️ [otakonTags] Parsing response (${o.length} chars)...`),["Hint","Lore","Places of Interest"].forEach(h=>{if(o.includes(h)){const g=o.substring(Math.max(0,o.indexOf(h)-10),o.indexOf(h)+h.length+20);console.log(`🔍 [otakonTags] Found "${h}" pattern: "${g}"`)}});const a=/\[OTAKON_SUGGESTIONS:\s*(\[[\s\S]*?\])\s*\]/g;let n;for(;(n=a.exec(o))!==null;)try{const h=n[1].replace(/'/g,'"'),g=JSON.parse(h);e.set("SUGGESTIONS",g),t=t.replace(n[0],""),console.log("🏷️ [otakonTags] Extracted SUGGESTIONS:",g)}catch{console.warn("[OtakonTags] Failed to parse SUGGESTIONS JSON:",n[1])}const i=/\[OTAKON_SUBTAB_UPDATE:\s*(\{[\s\S]*?\})\s*\]/g;let c;const l=[];for(;(c=i.exec(o))!==null;)try{const h=JSON.parse(c[1]);l.push(h),t=t.replace(c[0],"")}catch{console.warn("[OtakonTags] Failed to parse SUBTAB_UPDATE JSON:",c[1])}l.length>0&&e.set("SUBTAB_UPDATE",l);const u=/\[OTAKON_([A-Z_]+):\s*([^\[\]]+?)\]/g;let m,p=null;const y=o.match(/\[OTAKON_PROGRESS[:\s]+(\d+)/i);if(y&&(p=parseInt(y[1],10),console.log(`📊 [otakonTags] Found OTAKON_PROGRESS format: ${y[0]} → ${p}%`)),!p){const h=o.match(/\[?PROGRESS[:\s]+(\d+)/i);h&&(p=parseInt(h[1],10),console.log(`📊 [otakonTags] Found PROGRESS format: ${h[0]} → ${p}%`))}if(!p){const h=o.match(/(?:progress|completion|game progress)[:\s]+(?:approximately\s+)?(\d+)\s*%/i);h&&(p=parseInt(h[1],10),console.log(`📊 [otakonTags] Found inline progress format: ${h[0]} → ${p}%`))}if(!p){const h=o.match(/"stateUpdateTags"[^}]*"PROGRESS[:\s]+(\d+)/i);h&&(p=parseInt(h[1],10),console.log(`📊 [otakonTags] Found stateUpdateTags PROGRESS: ${h[0]} → ${p}%`))}for(p!==null&&p>=0&&p<=100?(e.set("PROGRESS",p),console.log(`📊 [otakonTags] ✅ Set PROGRESS tag to: ${p}%`)):console.log("📊 [otakonTags] ⚠️ No valid progress found in response");(m=u.exec(o))!==null;){const h=m[1];let g=m[2].trim();if(console.log(`🏷️ [otakonTags] Found tag: ${h} = ${m[2].substring(0,50)}`),!(h==="SUGGESTIONS"||h==="SUBTAB_UPDATE")){try{const _=g;_.startsWith("{")&&_.endsWith("}")&&(g=JSON.parse(_)),_.startsWith("[")&&_.endsWith("]")&&(g=JSON.parse(_.replace(/'/g,'"')))}catch{}if(h==="PROGRESS"){const _=String(g).trim(),I=_.match(/(\d+)/);if(I){const M=parseInt(I[1],10);g=Math.min(100,Math.max(0,M)),console.log(`📊 [otakonTags] Parsed PROGRESS: "${_}" → ${g}`)}else console.warn(`📊 [otakonTags] Could not parse PROGRESS value: "${_}"`)}e.set(h,g),t=t.replace(m[0],"")}}t=t.replace(/\[OTAKON_[A-Z_]+:[^\]]*\]/g,"").replace(/^["'][^"']*\?["']\s*,?\s*/gm,"").replace(/^["'](?:and\s+)?\[\d+\][^"']*\?["']\s*,?\s*/gim,"").replace(/^["'][^"']*\?["']\s*\]\s*/gm,"").replace(/^[^"']*\?["']\s*\]\s*/gm,"").replace(/^(?:["'][^"']*["']\s*,?\s*)+\]/g,"");const C=["Hint","Lore","Places of Interest","Strategy","What to focus on"];for(const h of C){const g=h.replace(/ /g,"\\s+"),_=new RegExp(`\\*+\\s*${g}(?:\\s*[:\\*]+\\s*:?|\\s*:)\\s*\\**|\\*\\*\\s*${g}\\s*\\*\\*|\\*\\*\\s+${g}(?![:\\w*])`,"gi");t=t.replace(_,`
...
`)}
`)}return e.join(`
`)}async function ls(o,e){try{const t=await U.getAIPreferences(o);if(t.responseHistoryScope==="off")return{previousTopics:[],corrections:[],scope:"off"};const[r,s]=await Promise.all([U.getResponseTopics(o,e,t.responseHistoryScope),U.getActiveCorrections(o,e,!0)]);return{previousTopics:r,corrections:s,scope:t.responseHistoryScope}}catch(t){return console.error("[PromptSystem] Error fetching behavior context:",t),null}}const Ye=500,zt=15e3,Ue=`
...
`,Vt=(o,e,t,r,s)=>{var m,p;let a=0;const n=((m=o.subtabs)==null?void 0:m.filter(y=>y.status==="loaded"&&y.content).map(y=>{const C=y.content||"",h=C.length>Ye?"..."+C.slice(-Ye):C,g=`### ${y.title} (ID: ${y.id})
${h}`;return a+=g.length,a>zt?null:g}).filter(Boolean).join(`

`))||"No subtabs available yet.",i=o.messages.slice(-10).map(y=>`${y.role==="user"?"User":"Otagon"}: ${y.content}`).join(`
`),c=o.contextSummary?`**Historical Context (Previous Sessions):**
${o.contextSummary}

`:"",l=s||te.getDefaultProfile(),u=te.buildProfileContext(l);return`
...
**🎮 GAME-SPECIFIC ACCURACY FOR "${o.gameTitle}":**
- ONLY use terminology, locations, and characters that exist in "${o.gameTitle}"
- NEVER mix in content from similar games (e.g., if this is Elden Ring, don't mention "bonfires" or "Firelink Shrine")
- If the user asks about something you're unsure exists in this game, say: "I'm not certain that exists in ${o.gameTitle}. Could you clarify?"
...
`},Jt=(o,e,t,r)=>{const s=r||te.getDefaultProfile(),a=te.buildProfileContext(s);return`
...
**Tag Definitions:**
${$e}
`},us=(o,e,t,r,s,a,n,i)=>{const c=n?Ht(n):"",l=i?`
**User Timezone:** ${i}
...
*Content loading...*`}}const gs=new Zt,Qt=`You are a correction validator for OTAKON, an AI gaming companion.
...
Is this correction valid? Respond with JSON only.`,ke="otakon_correction_submissions",se=3;function ot(){try{const o=localStorage.getItem(ke),e=Date.now();if(!o)return{allowed:!0,remaining:se};const t=JSON.parse(o);if(e>t.resetAt)return{allowed:!0,remaining:se};const r=se-t.count;return{allowed:r>0,remaining:Math.max(0,r)}}catch{return{allowed:!0,remaining:se}}}function tr(){try{const o=localStorage.getItem(ke),e=Date.now(),t=1440*60*1e3;let r={count:0,resetAt:e+t};o&&(r=JSON.parse(o),e>r.resetAt&&(r={count:0,resetAt:e+t})),r.count++,localStorage.setItem(ke,JSON.stringify(r))}catch{}}async function nt(o,e,t){if(!e.trim())return{isValid:!1,reason:"Correction text is empty"};if(e.length<5)return{isValid:!1,reason:"Correction is too short"};if(e.length>1e3)return{isValid:!1,reason:"Correction is too long (max 1000 characters)"};const r=[/\b(hate|kill|die|attack)\s+(all|every|those)\b/i,/\b(racial|ethnic)\s+slur/i,/\bviolence\s+against\b/i];for(const s of r)if(s.test(e))return{isValid:!1,reason:"Correction contains inappropriate content"};try{const{data:s,error:a}=await d.functions.invoke("gemini-chat",{body:{messages:[{role:"system",content:Qt},{role:"user",content:er(o,e,t)}],model:"gemini-2.0-flash",temperature:.1,maxTokens:200}});if(a)return console.error("[CorrectionService] Validation API error:",a),{isValid:!1,reason:"Validation service temporarily unavailable. Please try again later."};const n=(s==null?void 0:s.content)||(s==null?void 0:s.response)||"",i=n.match(/\{[\s\S]*\}/);if(i){const c=JSON.parse(i[0]);return{isValid:c.isValid===!0,reason:c.reason||"Validation complete",suggestedType:c.suggestedType||void 0}}return console.warn("[CorrectionService] Could not parse validation response:",n),{isValid:!1,reason:"Validation response was unclear. Please try again."}}catch(s){return console.error("[CorrectionService] Validation exception:",s),{isValid:!1,reason:"Validation failed. Please try again later."}}}async function rr(o,e){if(!ot().allowed)return{success:!1,error:`Daily correction limit reached (${se}/day). Try again tomorrow.`};const r=await nt(e.originalResponse,e.correctionText,e.gameTitle),{error:s}=await d.from("ai_feedback").insert({user_id:o,conversation_id:e.conversationId,message_id:e.messageId,feedback_type:"down",content_type:"message",category:"correction",comment:e.originalResponse.slice(0,500),correction_text:e.correctionText,correction_type:e.type,correction_scope:e.scope,is_validated:r.isValid,validation_reason:r.reason,game_title:e.gameTitle});if(s)return console.error("[CorrectionService] Failed to store feedback:",s),{success:!1,error:"Failed to save correction"};if(!r.isValid)return{success:!1,error:r.reason};const a=await U.addCorrection(o,{gameTitle:e.gameTitle,originalSnippet:e.originalResponse.slice(0,200),correctionText:e.correctionText,type:r.suggestedType||e.type,scope:e.scope});return a.success?(tr(),{success:!0,correction:(await U.getActiveCorrections(o,e.gameTitle)).find(c=>c.correctionText===e.correctionText)}):{success:!1,error:a.error}}const sr=[/\b(boss(?:es)?|mini-boss|final boss)\b/gi,/\b(enemy types?|elite enemies|common enemies)\b/gi,/\b(legendary weapon|rare item|unique gear)\b/gi,/\b(skill tree|ability points?|talent build)\b/gi,/\b(main quest|side quest|daily quest)\b/gi,/\b(damage build|tank build|support build|dps build)\b/gi,/\b(speedrun(?:ning)?|world record|personal best)\b/gi,/\b(secret area|hidden path|easter egg)\b/gi,/\b(tier list|meta build|optimal strategy)\b/gi,/\b(patch notes?|balance changes?|nerf(?:ed)?|buff(?:ed)?)\b/gi,/\b(dlc|expansion pack|season pass)\b/gi,/\b(game mechanics?|combat system|progression system)\b/gi,/\b(character build|loadout guide|equipment guide)\b/gi],ar=/\b([A-Z][a-z]{2,}(?:\s+(?:of|the|and)\s+)?[A-Z][a-z]{2,}(?:\s+[A-Z][a-z]+)?)\b/g;function or(o){if(!o||o.length<100)return[];const e=new Set;for(const n of sr){const i=o.match(n);if(i)for(const c of i){const l=c.toLowerCase().trim();l.length>=6&&l.length<=50&&l.includes(" ")&&e.add(l)}}let t=0;const r=o.match(ar);if(r)for(const n of r){if(t>=5)break;const i=n.toLowerCase().trim();i.length>=6&&i.length<=40&&(e.add(i),t++)}const s=/\b(chapter|level|stage|floor|wave|round|phase|part|episode|act)\s+(\d+)\b/gi;let a;for(;(a=s.exec(o))!==null;)e.add(`${a[1].toLowerCase()} ${a[2]}`);return Array.from(e).slice(0,10)}async function nr(o){return(await U.getBehaviorData(o)).aiCorrections}async function ir(o,e){return U.getActiveCorrections(o,e,!0)}function cr(){return ot()}const W={validateCorrection:nt,submitCorrection:rr,getAllCorrections:nr,getContextualCorrections:ir,getRateLimitStatus:cr,extractTopicsFromResponse:or,toggleCorrection:U.toggleCorrection,removeCorrection:U.removeCorrection};class lr{constructor(){f(this,"STORAGE_KEY","otakon_used_suggested_prompts");f(this,"LAST_RESET_KEY","otakon_suggested_prompts_last_reset");f(this,"RESET_INTERVAL_MS",1440*60*1e3);f(this,"usedPrompts",new Set);this.loadUsedPrompts(),this.checkAndResetIfNeeded()}loadUsedPrompts(){try{const e=localStorage.getItem(this.STORAGE_KEY);if(e){const t=JSON.parse(e);this.usedPrompts=new Set(t)}}catch{this.usedPrompts=new Set}}saveUsedPrompts(){try{const e=Array.from(this.usedPrompts);localStorage.setItem(this.STORAGE_KEY,JSON.stringify(e))}catch{}}checkAndResetIfNeeded(){try{const e=localStorage.getItem(this.LAST_RESET_KEY),t=Date.now();(!e||t-parseInt(e)>=this.RESET_INTERVAL_MS)&&(this.resetUsedPrompts(),localStorage.setItem(this.LAST_RESET_KEY,t.toString()))}catch{}}markPromptAsUsed(e){this.usedPrompts.add(e),this.saveUsedPrompts()}isPromptUsed(e){return this.usedPrompts.has(e)}getUnusedPrompts(e){return e.filter(t=>!this.isPromptUsed(t))}areAllPromptsUsed(e){return e.every(t=>this.isPromptUsed(t))}resetUsedPrompts(){this.usedPrompts.clear(),localStorage.removeItem(this.STORAGE_KEY)}getUsedCount(){return this.usedPrompts.size}getTimeUntilNextReset(){try{const e=localStorage.getItem(this.LAST_RESET_KEY);if(!e)return 0;const t=parseInt(e)+this.RESET_INTERVAL_MS;return Math.max(0,t-Date.now())}catch{return 0}}getStaticNewsPrompts(){return pt}processAISuggestions(e){if(!e)return[];let t=[];if(Array.isArray(e))t=e;else if(typeof e=="string"){let s=e.trim();s.startsWith('["')&&!s.endsWith('"]')&&(s.endsWith('"')||(s+='"'),s.endsWith("]")||(s+="]"));try{const a=JSON.parse(s);Array.isArray(a)?t=a:t=[e]}catch{s.includes('", "')||s.includes(`",
"`)?t=s.split(/",\s*"/).map(n=>n.replace(/^["\s]+|["\s]+$/g,"")).filter(n=>n.length>0):s.includes(`
`)?t=s.split(`
`).map(n=>n.replace(/^["\s]+|["\s]+$/g,"")).filter(n=>n.length>0):t=[e]}}return t.filter(s=>s&&typeof s=="string"&&s.trim().length>0).map(s=>s.trim()).slice(0,3)}getFallbackSuggestions(e,t){return t===!0||e==="game-hub"||e==="everything-else"?this.getStaticNewsPrompts():["What should I do next in this area?","Tell me about the story so far","Give me some tips for this game","What are the key mechanics I should know?"]}}const ms=new lr,ce=new Map,ur=300*1e3;function dr(o,e,t){return`${o}:${e}:${t||"global"}`}function gr(o){const e=ce.get(o);return e?Date.now()-e.timestamp>ur?(ce.delete(o),null):e.prompts:null}function mr(o,e){ce.set(o,{prompts:e,timestamp:Date.now()})}function it(o){for(const e of ce.keys())e.startsWith(o)&&ce.delete(e)}async function Le(o,e,t=null,r=20){const s=dr(o,e,t),a=gr(s);if(a)return a;try{const n=new Date;n.setDate(n.getDate()-7);let i=d.from("ai_shown_prompts").select("prompt_text").eq("auth_user_id",o).eq("prompt_type",e).gte("shown_at",n.toISOString()).order("shown_at",{ascending:!1}).limit(r);t&&(i=i.or(`game_title.eq.${t},game_title.is.null`));const{data:c,error:l}=await i;if(l)return console.error("[ShownPromptsService] Error fetching prompts:",l),[];const u=(c||[]).map(m=>m.prompt_text);return mr(s,u),u}catch(n){return console.error("[ShownPromptsService] Exception fetching prompts:",n),[]}}async function hr(o,e){try{const{error:t}=await d.from("ai_shown_prompts").insert({auth_user_id:o,prompt_text:e.promptText,prompt_type:e.promptType,game_title:e.gameTitle||null,conversation_id:e.conversationId||null});return t?(console.error("[ShownPromptsService] Error recording prompt:",t),!1):(it(o),!0)}catch(t){return console.error("[ShownPromptsService] Exception recording prompt:",t),!1}}async function pr(o,e){if(!e.length)return!0;try{const t=e.map(s=>({auth_user_id:o,prompt_text:s.promptText,prompt_type:s.promptType,game_title:s.gameTitle||null,conversation_id:s.conversationId||null})),{error:r}=await d.from("ai_shown_prompts").insert(t);return r?(console.error("[ShownPromptsService] Error batch recording prompts:",r),!1):(it(o),!0)}catch(t){return console.error("[ShownPromptsService] Exception batch recording prompts:",t),!1}}async function fr(o,e){try{const{error:t}=await d.from("ai_shown_prompts").update({clicked:!0,clicked_at:new Date().toISOString()}).eq("auth_user_id",o).eq("prompt_text",e).is("clicked",!1);return t?(console.error("[ShownPromptsService] Error marking prompt clicked:",t),!1):!0}catch(t){return console.error("[ShownPromptsService] Exception marking prompt clicked:",t),!1}}async function Sr(o,e,t,r=null){if(!e.length)return[];const s=await Le(o,t,r),a=new Set(s.map(n=>n.toLowerCase().trim()));return e.filter(n=>!a.has(n.toLowerCase().trim()))}async function yr(o,e,t,r=null){const s=await Le(o,t,r),a=e.toLowerCase().trim();return s.some(n=>n.toLowerCase().trim()===a)}const hs={getRecentShownPrompts:Le,recordShownPrompt:hr,recordShownPrompts:pr,markPromptClicked:fr,filterNewPrompts:Sr,hasPromptBeenShown:yr};class br{async generatePlayingSessionSummary(e){const t=e.messages.slice(-10),r=this.extractKeyPointsIntelligent(t,"playing"),s=this.extractObjectivesIntelligent(t,e),a=this.generateProgressContext(e),n=`**Playing Session Summary for ${e.gameTitle}**
...
---
`+g,console.error(`📝 [GameTabService] [${e}] Subtab ${c.id} content trimmed to ${y.length} chars`)}}return{...c,title:l.title||c.title,content:y,isNew:!0,status:"loaded"}}return c});if(a===0){console.error(`📝 [GameTabService] [${e}] ⚠️ No subtabs matched for update`);return}if(console.error(`📝 [GameTabService] [${e}] Writing ${a} updated subtabs to normalized table...`),!await q.setSubtabs(e,n))throw console.error(`📝 [GameTabService] [${e}] ❌ Failed to write subtabs to table`),new Error("Failed to update subtabs in database");E.clearCache(),await E.updateConversation(e,{updatedAt:Date.now()}),console.error(`📝 [GameTabService] [${e}] ✅ Updated ${a} subtabs successfully (table + cache cleared)`)}catch(r){throw console.error(`📝 [GameTabService] [${e}] ❌ Failed to update subtabs:`,r),r}}async generateSubtabsForExistingGameTabs(e,t,r){const s=Object.values(e).filter(a=>!a.isGameHub&&!a.isUnreleased&&a.gameTitle&&(!a.subtabs||a.subtabs.length===0));if(s.length===0){console.log("🔄 [GameTabService] No game tabs need subtabs generation");return}console.log(`🔄 [GameTabService] Found ${s.length} game tabs that need subtabs`);for(let a=0;a<s.length;a++){const n=s[a];try{console.log(`🔄 [GameTabService] Generating subtabs for "${n.gameTitle}" (${a+1}/${s.length})`),r==null||r(a,s.length,n.gameTitle||n.title);const i=this.generateInitialSubTabs(n.genre||"Default",t);await E.updateConversation(n.id,{subtabs:i,subtabsOrder:i.map(c=>c.id),updatedAt:Date.now()}),await q.setSubtabs(n.id,i),await this.generateInitialInsights({...n,subtabs:i},t),console.log(`✅ [GameTabService] Successfully generated subtabs for "${n.gameTitle}"`),a<s.length-1&&await new Promise(c=>setTimeout(c,1e3))}catch(i){console.error(`❌ [GameTabService] Failed to generate subtabs for "${n.gameTitle}":`,i)}}r==null||r(s.length,s.length,"Complete"),console.log(`🔄 [GameTabService] Finished generating subtabs for ${s.length} game tabs`)}}const fs=new Tr,K=o=>o;class Ss{static getCurrentUser(){return B.get(F.USER,null)}static setCurrentUser(e){B.set(F.USER,e)}static createUser(e,t=ft.FREE){const r=Date.now(),s=qe[t];return{id:`user_${r}`,authUserId:`user_${r}`,email:e,tier:t,hasProfileSetup:!1,hasSeenSplashScreens:!1,hasSeenHowToUse:!1,hasSeenFeaturesConnected:!1,hasSeenProFeatures:!1,pcConnected:!1,pcConnectionSkipped:!1,onboardingCompleted:!1,hasWelcomeMessage:!1,isNewUser:!0,hasUsedTrial:!1,lastActivity:r,preferences:{},textCount:0,imageCount:0,textLimit:s.text,imageLimit:s.image,totalRequests:0,lastReset:r,usage:{textCount:0,imageCount:0,textLimit:s.text,imageLimit:s.image,totalRequests:0,lastReset:r,tier:t},appState:{},profileData:{},onboardingData:{},behaviorData:{},feedbackData:{},usageData:{},createdAt:r,updatedAt:r}}static updateUser(e){const t=this.getCurrentUser();if(!t)return;const r={...t,...e,updatedAt:Date.now()};this.setCurrentUser(r)}static updateUsage(e){const t=this.getCurrentUser();t&&this.updateUser({usage:{...t.usage,...e}})}static resetUsage(){const e=this.getCurrentUser();if(!e)return;const t=qe[e.tier];this.updateUsage({textCount:0,imageCount:0,totalRequests:0,lastReset:Date.now(),textLimit:t.text,imageLimit:t.image})}static canMakeRequest(e){const t=this.getCurrentUser();if(!t)return!1;const{usage:r}=t;return e==="text"?r.textCount<r.textLimit:r.imageCount<r.imageLimit}static incrementUsage(e){const t=this.getCurrentUser();if(!t)return;const r={totalRequests:t.usage.totalRequests+1};e==="text"?r.textCount=t.usage.textCount+1:r.imageCount=t.usage.imageCount+1,this.updateUsage(r)}static logout(){B.remove(F.USER)}static async getCurrentUserAsync(){try{const e=B.get(F.USER,null),{data:{user:t},error:r}=await d.auth.getUser();if(r||!t)return e;const{data:s,error:a}=await d.from("users").select("*").eq("auth_user_id",t.id).single();if(a||!s)return console.error("Failed to fetch user from Supabase:",a),e;const n={id:s.id,authUserId:s.auth_user_id,email:s.email,tier:s.tier,textCount:s.text_count||0,imageCount:s.image_count||0,textLimit:me(s.text_limit),imageLimit:me(s.image_limit),totalRequests:s.total_requests||0,lastReset:Z(s.last_reset),hasProfileSetup:s.has_profile_setup||!1,hasSeenSplashScreens:s.has_seen_splash_screens||!1,hasSeenHowToUse:s.has_seen_how_to_use||!1,hasSeenFeaturesConnected:s.has_seen_features_connected||!1,hasSeenProFeatures:s.has_seen_pro_features||!1,pcConnected:s.pc_connected||!1,pcConnectionSkipped:s.pc_connection_skipped||!1,onboardingCompleted:s.onboarding_completed||!1,hasWelcomeMessage:s.has_welcome_message||!1,isNewUser:s.is_new_user||!1,hasUsedTrial:s.has_used_trial||!1,lastActivity:Z(s.updated_at),preferences:G(s.preferences),usage:{textCount:s.text_count||0,imageCount:s.image_count||0,textLimit:me(s.text_limit),imageLimit:me(s.image_limit),totalRequests:s.total_requests||0,lastReset:Z(s.last_reset),tier:s.tier},appState:G(s.app_state),profileData:G(s.profile_data),onboardingData:G(s.onboarding_data),behaviorData:G(s.behavior_data),feedbackData:G(s.feedback_data),usageData:G(s.usage_data),createdAt:Z(s.created_at),updatedAt:Z(s.updated_at)};return B.set(F.USER,n),n}catch(e){return console.error("Error in getCurrentUserAsync:",e),B.get(F.USER,null)}}static async setCurrentUserAsync(e){try{B.set(F.USER,e);const{error:t}=await d.from("users").update({tier:e.tier,text_count:e.textCount,image_count:e.imageCount,text_limit:e.textLimit,image_limit:e.imageLimit,total_requests:e.totalRequests,last_reset:new Date(e.lastReset).toISOString(),has_profile_setup:e.hasProfileSetup,has_seen_splash_screens:e.hasSeenSplashScreens,has_seen_how_to_use:e.hasSeenHowToUse,has_seen_features_connected:e.hasSeenFeaturesConnected,has_seen_pro_features:e.hasSeenProFeatures,pc_connected:e.pcConnected,pc_connection_skipped:e.pcConnectionSkipped,onboarding_completed:e.onboardingCompleted,has_welcome_message:e.hasWelcomeMessage,has_used_trial:e.hasUsedTrial,preferences:K(e.preferences),profile_data:K(e.profileData),app_state:K(e.appState),onboarding_data:K(e.onboardingData),behavior_data:K(e.behaviorData),feedback_data:K(e.feedbackData),usage_data:K(e.usageData),updated_at:new Date().toISOString()}).eq("auth_user_id",e.authUserId);t&&console.error("Failed to sync user to Supabase:",t)}catch(t){console.error("Error in setCurrentUserAsync:",t)}}static async updateUsageAsync(e){const t=await this.getCurrentUserAsync();if(!t)return;const r={...t,usage:{...t.usage,...e},textCount:e.textCount??t.textCount,imageCount:e.imageCount??t.imageCount,totalRequests:e.totalRequests??t.totalRequests,lastReset:e.lastReset??t.lastReset,updatedAt:Date.now()};await this.setCurrentUserAsync(r)}}class wr{hasTabCommand(e){return/^@\w+/.test(e.trim())}parseTabCommand(e,t){const r=e.trim();if(!this.hasTabCommand(r))return null;const s=r.match(/^@(\w+)\s*(\\modify|\\delete)?\s*(.*)$/);if(!s)return null;const[,a,n,i]=s,c=this.findMatchingTab(a,t.subtabs||[]);if(!c)return null;let l;return n==="\\delete"?l="delete":n==="\\modify"?l="modify":l="update",{type:l,tabId:c.id,tabName:c.title,instruction:i.trim()}}findMatchingTab(e,t){const r=this.normalizeTabName(e);let s=t.find(a=>this.normalizeTabName(a.id)===r||this.normalizeTabName(a.title)===r);return s||(s=t.find(a=>this.normalizeTabName(a.id).includes(r)||this.normalizeTabName(a.title).includes(r)),s)?s:(s=t.find(a=>r.includes(this.normalizeTabName(a.id))||r.includes(this.normalizeTabName(a.title))),s||null)}normalizeTabName(e){return e.toLowerCase().replace(/[_\s-]+/g,"").replace(/[^a-z0-9]/g,"")}getAvailableTabNames(e){return!e.subtabs||e.subtabs.length===0?[]:e.subtabs.map(t=>({id:t.id,title:t.title}))}formatTabSuggestion(e,t){return`@${e}`}getCommandHelp(){return`
**Tab Commands:**
