...
`),e.join(`
`)}function Yt(a,e){const t=[],r=(e==null?void 0:e.messageCount)||a.messages.length,s=a.gameProgress||0,o=(e==null?void 0:e.subtabsFilled)||0,n=(e==null?void 0:e.subtabsTotal)||0;if(a.isGameHub||r<3)return"";if(r>=20?t.push(`
...
`)}return t.join(`
`)}function qt(a){if(!a||a.scope==="off")return"";const e=[];if(a.previousTopics.length>0&&e.push(`
...
`)}
`)}return e.join(`
`)}async function ls(a,e){try{const t=await P.getAIPreferences(a);if(t.responseHistoryScope==="off")return{previousTopics:[],corrections:[],scope:"off"};const[r,s]=await Promise.all([P.getResponseTopics(a,e,t.responseHistoryScope),P.getActiveCorrections(a,e,!0)]);return{previousTopics:r,corrections:s,scope:t.responseHistoryScope}}catch(t){return console.error("[PromptSystem] Error fetching behavior context:",t),null}}const Ke=3e3,zt=2e4,Ee=["story_so_far","quest_log","characters","boss_strategy","tips","hidden_secrets","points_of_interest"],qe=`
...
`;async function Pe(){try{const a=await ht.getConversations(),e=Object.values(a).filter(r=>!r.isGameHub&&r.gameTitle).map(r=>({title:r.gameTitle,genre:r.genre||"Unknown",progress:r.gameProgress||0,isUnreleased:r.isUnreleased||!1})).sort((r,s)=>r.title.localeCompare(s.title));if(e.length===0)return`
**📚 USER'S GAME LIBRARY:**
...
`}catch(a){return console.warn("[PromptSystem] Failed to build game library context:",a),""}}const Jt=async(a,e,t,r,s)=>{var S;let o=0;const n=[],i=2500,u=[...a.subtabs||[]].filter(d=>d.status==="loaded"&&d.content).sort((d,m)=>{const A=Ee.indexOf(d.id),w=Ee.indexOf(m.id);return(A===-1?999:A)-(w===-1?999:w)}).map(d=>{const m=d.content||"";m.length>i&&m.includes("<details>")&&n.push(d.id);const A=m.length>Ke?m.slice(0,Ke)+"...[truncated]":m,w=`### ${d.title} (ID: ${d.id})
${A}`;if(o+w.length>zt){const C=Ee.indexOf(d.id);if(C!==-1&&C<3){const F=`### ${d.title} (ID: ${d.id}) [SUMMARY]
${m.slice(0,500)}...`;return o+=F.length,F}return null}return o+=w.length,w}).filter(Boolean).join(`

`)||"No subtabs available yet.",f=n.length>0?`
...
This keeps subtabs useful without losing important context.`:"",T=a.messages.slice(-10).map(d=>`${d.role==="user"?"User":"Otagon"}: ${d.content}`).join(`
`),p=a.contextSummary?`**Historical Context (Previous Sessions):**
${a.contextSummary}

`:"",O=s||Q.getDefaultProfile(),V=Q.buildProfileContext(O);let G="";if(a.gameTitle){const d=gt.getByGameTitle(a.gameTitle);if(d!=null&&d.igdbGameId)try{const m=await mt.getForContext(d.igdbGameId);if(m){const A=Math.ceil(m.length/4);G=`
...
`,console.log(`🎮 [PromptSystem] Injecting ${m.length} chars of FULL game knowledge (no truncation)`)}}catch(m){console.warn("🎮 [PromptSystem] Failed to fetch game knowledge:",m)}}const h=await Pe();return`
...
`},Xt=async(a,e,t,r)=>{const s=r||Q.getDefaultProfile(),o=Q.buildProfileContext(s),n=await Pe(),i=a.messages.slice(-10).map(c=>`${c.role==="user"?"User":"Otagon"}: ${c.content}`).join(`
`);return`
...
**Tag Definitions:**
${Se}
`},Qt=async(a,e,t,r)=>{const s=r||Q.getDefaultProfile(),o=Q.buildProfileContext(s),n=await Pe(),i=a.gameTitle?`
...
**Tag Definitions:**
${Se}
`},us=async(a,e,t,r,s,o,n,i,c)=>{const u=n?qt(n):"",f=i?`
**User Timezone:** ${i}
...
*Content loading...*`}}const gs=new er,tr=`You are a correction validator for OTAKON, an AI gaming companion.
...
Provide ONLY the summary, no additional commentary.`;try{const c={id:"temp-summary",title:"Summary Request",messages:[{id:"summary-msg-"+Date.now(),role:"user",content:i,timestamp:Date.now()}],createdAt:Date.now(),updatedAt:Date.now(),isActive:!1,isGameHub:!1};console.log(`📡 [GEMINI CALL #2] 📋 Context Summarization | Messages: ${e.length} | Game: ${t||"N/A"} | Target: ${this.MAX_WORDS} words`);const u={id:"system",email:"system@otakon.ai",profileData:null},T=(await St.getChatResponse(c,u,i,!1,!1,void 0,void 0,"summarization")).content.trim(),p=this.countWords(T);return console.log(`✅ [GEMINI CALL] Context Summarization SUCCESS | ${p} words (reduced from ${s})`),R.success(`Conversation summarized (${s} → ${p} words)`),de.delete("summarization"),{summary:T,wordCount:p,messagesIncluded:e.length,originalWordCount:s}}catch(c){console.error("❌ [ContextSummarization] Failed to generate summary:",c);const u=de.get("summarization")||0,f=3;if(u<f)return de.set("summarization",u+1),console.log(`🔄 [ContextSummarization] Retry ${u+1}/${f} in 1 second...`),await new Promise(p=>setTimeout(p,1e3)),this.summarizeMessages(e,t,r);console.error("❌ [ContextSummarization] Max retries reached"),de.delete("summarization");const T=e.slice(0,5).map(p=>p.content.substring(0,100)).join(" ... ").substring(0,this.MAX_WORDS*6);return{summary:`[Previous conversation context] ${T}`,wordCount:this.countWords(T),messagesIncluded:e.length,originalWordCount:s}}}async applyContextSummarization(e){if(!this.shouldSummarize(e))return e;const{toSummarize:t,toKeep:r}=this.splitMessages(e.messages);if(t.length===0)return e;const s=await this.summarizeMessages(t,e.gameTitle,e.genre),n=[{id:"summary-"+Date.now(),role:"system",content:s.summary,timestamp:t[t.length-1].timestamp,metadata:{isSummary:!0,messagesIncluded:s.messagesIncluded,originalWordCount:s.originalWordCount,summaryWordCount:s.wordCount}},...r],i=s.summary.replace(/!\[.*?\]\(data:image\/.*?\)/g,""),c=i.split(/\s+/).filter(f=>f.length>0),u=c.length>500?c.slice(0,500).join(" ")+"...":i;return console.log(`✅ [ContextSummarization] Context optimized: ${e.messages.length} messages → ${n.length} (${s.originalWordCount} words → ${s.wordCount} + recent)`),{...e,messages:n,contextSummary:u,lastSummarizedAt:Date.now(),updatedAt:Date.now()}}async getOptimizedContext(e){return this.shouldSummarize(e)?(await this.applyContextSummarization(e)).messages:e.messages}willTriggerSummarization(e){return this.getTotalWordCount(e.messages)>this.MAX_WORDS*3*.8}}const Wr=new Kr,Os=Object.freeze(Object.defineProperty({__proto__:null,contextSummarizationService:Wr},Symbol.toStringTag,{value:"Module"}));export{ws as A,fs as B,Qr as C,jr as D,le as E,Jr as F,_s as G,As as H,Os as I,bs as U,es as W,as as a,W as b,gs as c,P as d,ds as e,ls as f,us as g,ms as h,Q as i,Ts as j,ps as k,Es as l,ns as m,is as n,os as o,ss as p,cs as q,ys as r,Xr as s,R as t,Ss as u,ts as v,rs as w,Zr as x,Ct as y,hs as z};
