...
`)}
`)}return e.join(`
`)}async function Yr(s,e){try{const t=await N.getAIPreferences(s);if(t.responseHistoryScope==="off")return{previousTopics:[],corrections:[],scope:"off"};const[r,a]=await Promise.all([N.getResponseTopics(s,e,t.responseHistoryScope),N.getActiveCorrections(s,e,!0)]);return{previousTopics:r,corrections:a,scope:t.responseHistoryScope}}catch(t){return console.error("[PromptSystem] Error fetching behavior context:",t),null}}const Ue=3e3,Gt=2e4,fe=["story_so_far","quest_log","characters","boss_strategy","tips","hidden_secrets","points_of_interest"],$e=`
...
`:"",_=a||z.getDefaultProfile(),H=z.buildProfileContext(_);let k="";if(s.gameTitle){const d=at.getByGameTitle(s.gameTitle);if(d!=null&&d.igdbGameId)try{const m=await ot.getForContext(d.igdbGameId);m&&(k=`
...
`},Lt=(s,e,t,r)=>{const a=r||z.getDefaultProfile(),o=z.buildProfileContext(a),n=s.messages.slice(-10).map(i=>`${i.role==="user"?"User":"Otagon"}: ${i.content}`).join(`
`);return`
...
**Tag Definitions:**
${me}
`},xt=(s,e,t,r)=>{const a=r||z.getDefaultProfile(),o=z.buildProfileContext(a),n=s.gameTitle?`
...
*Content loading...*`}}const zr=new $t,Wt=`You are a correction validator for OTAKON, an AI gaming companion.
...
Example: @story_so_far The player defeated the first boss
    `.trim()}validateCommand(e){switch(e.type){case"update":if(!e.instruction)return{valid:!1,error:"Update command requires content. Example: @story_so_far The player..."};break;case"modify":if(!e.instruction)return{valid:!1,error:"Modify command requires instructions. Example: @tips \\modify Change to combat strategies"};break}return{valid:!0}}describeCommand(e){switch(e.type){case"update":return`Updating "${e.tabName}" with: ${e.instruction}`;case"modify":return`Modifying "${e.tabName}": ${e.instruction}`;case"delete":return`Deleting "${e.tabName}"`}}}const ts=new dr;let g,Y=[],Le=!1,V="",G=null,P=null,A=null,D=null;const gr="otakonSpeechRate",Re=async()=>{try{const s=navigator;if(s.wakeLock){if(G!==null)try{await G.release()}catch{}G=await s.wakeLock.request("screen"),console.log("🔒 [TTS] Wake lock acquired - screen will stay on"),G.addEventListener("release",async()=>{console.log("🔓 [TTS] Wake lock released"),G=null,g&&g.speaking&&(console.log("🔒 [TTS] Reacquiring wake lock (TTS still active)"),await Re())})}else console.warn("⚠️ [TTS] Wake lock API not available")}catch(s){console.warn("⚠️ [TTS] Wake lock request failed:",s)}},je=async()=>{try{G!==null&&(await G.release(),G=null)}catch{}},mr=()=>{try{if(!P){const s=window,e=s.AudioContext||s.webkitAudioContext;e&&(P=new e,console.log("🔊 [TTS] Audio context initialized"))}A||(A=new Audio,A.src="data:audio/wav;base64,UklGRnoGAABXQVZFZm10IBAAAAABAAEAQB8AAEAfAAABAAgAZGF0YQoGAACBhYqFbF1fdJivrJBhNjVgodDbq2EcBj+a2teleShtr9teleShtr9teleShtr9teleShtr9t",A.loop=!0,A.volume=.01,A.load(),console.log("🔇 [TTS] Silent audio initialized for background playback"))}catch(s){console.warn("⚠️ [TTS] Audio context init failed:",s)}},Je=async()=>{try{P&&P.state==="suspended"&&(await P.resume(),console.log("🔊 [TTS] Audio context resumed")),A&&(A.currentTime=0,await A.play(),console.log("🔇 [TTS] Silent audio playing for background session")),D||(D=setInterval(()=>{g&&g.speaking?P&&P.state==="suspended"&&P.resume().catch(()=>{}):D&&(clearInterval(D),D=null)},5e3))}catch(s){console.warn("⚠️ [TTS] Silent audio start failed:",s)}},Xe=()=>{try{A&&(A.pause(),A.currentTime=0,console.log("🔇 [TTS] Silent audio stopped")),D&&(clearInterval(D),D=null)}catch(s){console.warn("⚠️ [TTS] Silent audio stop failed:",s)}},hr=()=>new Promise((s,e)=>{if(!g)return e(new Error("Speech synthesis not initialized."));if(Y=g.getVoices(),Y.length>0){s();return}g.onvoiceschanged=()=>{Y=g.getVoices(),s()},setTimeout(()=>{Y.length===0&&(Y=g.getVoices()),s()},1e3)}),re=()=>{g&&g.speaking&&g.cancel(),V="","mediaSession"in navigator&&navigator.mediaSession.playbackState!=="none"&&(navigator.mediaSession.playbackState="paused"),je(),Xe(),window.dispatchEvent(new CustomEvent("otakon:ttsStopped"))},Qe=()=>{g&&g.speaking&&!g.paused&&(g.pause(),"mediaSession"in navigator&&(navigator.mediaSession.playbackState="paused"),window.dispatchEvent(new CustomEvent("otakon:ttsPaused")))},Ze=()=>{g&&g.paused&&(g.resume(),"mediaSession"in navigator&&(navigator.mediaSession.playbackState="playing"),window.dispatchEvent(new CustomEvent("otakon:ttsResumed")))},pr=async()=>{V&&(re(),await tt(V))},fr=()=>g?g.speaking:!1,yr=()=>{re(),window.dispatchEvent(new CustomEvent("otakon:disableHandsFree"))},Sr=()=>{if("mediaSession"in navigator){console.log("🎵 [TTS] Setting up media session");try{navigator.mediaSession.setActionHandler("play",()=>{console.log("🎵 [TTS] Media session play action"),g&&g.paused&&Ze()}),navigator.mediaSession.setActionHandler("pause",()=>{console.log("🎵 [TTS] Media session pause action"),g&&g.speaking&&!g.paused&&Qe()}),navigator.mediaSession.setActionHandler("stop",()=>{console.log("🎵 [TTS] Media session stop action"),yr()}),console.log("✅ [TTS] Media session handlers registered")}catch(s){console.warn("⚠️ [TTS] Failed to set media session handlers:",s)}}else console.warn("⚠️ [TTS] Media session API not available")},br=async()=>{document.hidden?(console.log("📱 [TTS] App went to background, isSpeaking:",g==null?void 0:g.speaking),g&&g.speaking&&(await Je(),g.paused||setTimeout(()=>{g&&g.speaking&&!g.paused&&console.log("📱 [TTS] Nudging speech synthesis to stay alive")},100))):(console.log("📱 [TTS] App came to foreground, isSpeaking:",g==null?void 0:g.speaking),g&&g.speaking&&await Re())},Er=async()=>{if(typeof window<"u"&&"speechSynthesis"in window){if(Le)return;Le=!0,g=window.speechSynthesis,await hr(),Sr(),mr(),document.addEventListener("visibilitychange",br),g.getVoices().length===0&&g.speak(new SpeechSynthesisUtterance(""))}},et=()=>Y.filter(s=>s.lang.startsWith("en-")),tt=async s=>new Promise((e,t)=>{try{if(!g)return console.error("Text-to-Speech is not available on this browser."),t(new Error("Text-to-Speech is not available on this browser."));if(!s.trim())return e();re(),V=s;const r=new SpeechSynthesisUtterance(s),a=localStorage.getItem(gr);r.rate=a?parseFloat(a):.94;const o=localStorage.getItem("otakonPreferredVoiceURI"),n=et();let i;if(o&&(i=n.find(c=>c.voiceURI===o)),!i&&n.length>0){const c=n.find(u=>u.name.toLowerCase().includes("female"));c?i=c:i=n[0]}i&&(r.voice=i),r.onstart=async()=>{if(console.log("🎤 [TTS] Speech started"),V=s,await Re(),await Je(),"serviceWorker"in navigator&&navigator.serviceWorker.controller&&navigator.serviceWorker.controller.postMessage({type:"TTS_STARTED"}),"mediaSession"in navigator)try{navigator.mediaSession.playbackState="playing";const c=s.replace(/[*_~`#]/g,"").replace(/\n+/g," ").trim(),u=c.length>100?c.substring(0,97)+"...":c;navigator.mediaSession.metadata=new MediaMetadata({title:u,artist:"Otagon AI",album:"Gaming Companion",artwork:[{src:"/icon-192.png",sizes:"192x192",type:"image/png"},{src:"/icon-512.png",sizes:"512x512",type:"image/png"}]}),console.log("🎵 [TTS] Media session metadata set")}catch(c){console.warn("⚠️ [TTS] Failed to set media session:",c)}window.dispatchEvent(new CustomEvent("otakon:ttsStarted"))},r.onend=()=>{V="","mediaSession"in navigator&&(navigator.mediaSession.playbackState="paused"),"serviceWorker"in navigator&&navigator.serviceWorker.controller&&navigator.serviceWorker.controller.postMessage({type:"TTS_STOPPED"}),je(),Xe(),window.dispatchEvent(new CustomEvent("otakon:ttsStopped")),e()},r.onerror=c=>{console.error("SpeechSynthesis Utterance Error",c),re(),t(c)},g.speak(r)}catch(r){console.error("TTS Error:",r),t(r)}}),rs={init:Er,getAvailableVoices:et,speak:tt,cancel:re,pause:Qe,resume:Ze,restart:pr,isSpeaking:fr};class Tr{constructor(){E(this,"submittedFeedback",new Set)}hasSubmittedFeedback(e){return this.submittedFeedback.has(e)}async submitFeedback(e){try{const t=oe.getCurrentUser();if(!(t!=null&&t.authUserId))return console.warn("[FeedbackService] User not authenticated"),{success:!1,error:"User not authenticated"};if(this.submittedFeedback.has(e.messageId))return console.log("[FeedbackService] Feedback already submitted for message:",e.messageId),{success:!0};const{error:r}=await l.from("ai_feedback").insert({user_id:t.authUserId,conversation_id:e.conversationId,message_id:e.messageId,feedback_type:e.feedbackType,content_type:e.contentType,category:e.category||null,comment:e.comment||null});return r?r.code==="23505"?(console.log("[FeedbackService] Feedback already exists in database"),this.submittedFeedback.add(e.messageId),{success:!0}):(console.error("[FeedbackService] Failed to submit feedback:",r),{success:!1,error:r.message}):(this.submittedFeedback.add(e.messageId),console.log("[FeedbackService] Feedback submitted:",{messageId:e.messageId,type:e.feedbackType,contentType:e.contentType}),{success:!0})}catch(t){return console.error("[FeedbackService] Error submitting feedback:",t),{success:!1,error:"Failed to submit feedback"}}}async submitPositiveFeedback(e,t,r="message"){return this.submitFeedback({messageId:e,conversationId:t,feedbackType:"up",contentType:r})}async submitNegativeFeedback(e,t,r,a,o="message"){return this.submitFeedback({messageId:e,conversationId:t,feedbackType:"down",contentType:o,category:r,comment:a})}async submitCorrection(e){try{const t=oe.getCurrentUser();if(!(t!=null&&t.authUserId))return console.warn("[FeedbackService] User not authenticated for correction"),{success:!1,error:"User not authenticated"};if(!x.getRateLimitStatus().allowed)return{success:!1,error:"Daily correction limit reached. Try again tomorrow.",rateLimitRemaining:0};const a={originalResponse:e.originalResponse,correctionText:e.correctionText,type:e.correctionType,scope:e.correctionScope,gameTitle:e.gameTitle,messageId:e.messageId,conversationId:e.conversationId},o=await x.submitCorrection(t.authUserId,a);return o.success?(console.log("[FeedbackService] Correction submitted successfully:",{messageId:e.messageId,type:e.correctionType,scope:e.correctionScope}),{success:!0,correction:o.correction,rateLimitRemaining:x.getRateLimitStatus().remaining}):{success:!1,error:o.error,rateLimitRemaining:x.getRateLimitStatus().remaining}}catch(t){return console.error("[FeedbackService] Error submitting correction:",t),{success:!1,error:"Failed to submit correction"}}}getCorrectionRateLimit(){return x.getRateLimitStatus()}async getUserCorrections(){const e=oe.getCurrentUser();return e!=null&&e.authUserId?x.getAllCorrections(e.authUserId):[]}async toggleCorrection(e,t){const r=oe.getCurrentUser();return r!=null&&r.authUserId?x.toggleCorrection(r.authUserId,e,t):!1}async getFeedbackStats(){try{const{data:e,error:t}=await l.from("ai_feedback").select("feedback_type, category");if(t)return console.error("[FeedbackService] Failed to get feedback stats:",t),null;const r={totalFeedback:e.length,positiveCount:e.filter(a=>a.feedback_type==="up").length,negativeCount:e.filter(a=>a.feedback_type==="down").length,categoryBreakdown:{}};return e.forEach(a=>{a.category&&(r.categoryBreakdown[a.category]=(r.categoryBreakdown[a.category]||0)+1)}),r}catch(e){return console.error("[FeedbackService] Error getting feedback stats:",e),null}}clearLocalTracking(){this.submittedFeedback.clear()}}const wr=new Tr,ss=Object.freeze(Object.defineProperty({__proto__:null,feedbackService:wr},Symbol.toStringTag,{value:"Module"}));function _r(s){var n;const e=s.split(","),t=((n=e[0].match(/:(.*?);/))==null?void 0:n[1])||"image/png",r=atob(e[1]),a=r.length,o=new Uint8Array(a);for(let i=0;i<a;i++)o[i]=r.charCodeAt(i);return new Blob([o],{type:t})}async function Ar(s,e){try{const t=_r(s),r=t.size,a=50*1024*1024;if(r>a)return{success:!1,error:"Screenshot exceeds 50MB size limit"};const o=Date.now(),n=Math.random().toString(36).substring(2,15),i=`${o}_${n}.png`,c=`${e}/${i}`,{error:u}=await l.storage.from("screenshots").upload(c,t,{contentType:"image/png",cacheControl:"3600",upsert:!1});if(u)return console.error("Screenshot upload error:",u),{success:!1,error:u.message};const{data:S}=l.storage.from("screenshots").getPublicUrl(c);return S!=null&&S.publicUrl?{success:!0,publicUrl:S.publicUrl,fileSize:r}:{success:!1,error:"Failed to generate public URL"}}catch(t){return console.error("Screenshot upload exception:",t),{success:!1,error:t instanceof Error?t.message:"Unknown error"}}}const as=Object.freeze(Object.defineProperty({__proto__:null,uploadScreenshot:Ar},Symbol.toStringTag,{value:"Module"})),ce=new Map;class Or{constructor(){E(this,"MAX_WORDS",300);E(this,"RECENT_MESSAGE_COUNT",8)}countWords(e){return e.trim().split(/\s+/).filter(t=>t.length>0).length}getTotalWordCount(e){return e.reduce((t,r)=>{const a=this.countWords(r.content);return t+a},0)}shouldSummarize(e){return!e.messages||e.messages.length<=this.RECENT_MESSAGE_COUNT?!1:this.getTotalWordCount(e.messages)>this.MAX_WORDS*3}splitMessages(e){if(e.length<=this.RECENT_MESSAGE_COUNT)return{toSummarize:[],toKeep:e};const t=e.length-this.RECENT_MESSAGE_COUNT;return{toSummarize:e.slice(0,t),toKeep:e.slice(t)}}async summarizeMessages(e,t,r){const a=this.getTotalWordCount(e),o=e.map(c=>`${c.role==="user"?"User":"Assistant"}: ${c.content}`).join(`
