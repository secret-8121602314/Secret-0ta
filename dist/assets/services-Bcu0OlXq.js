...
`),e.join(`
`)}function Ht(s,e){const t=[],r=(e==null?void 0:e.messageCount)||s.messages.length,a=s.gameProgress||0,o=(e==null?void 0:e.subtabsFilled)||0,n=(e==null?void 0:e.subtabsTotal)||0;if(s.isGameHub||r<3)return"";if(r>=20?t.push(`
...
`)}return t.join(`
`)}function Yt(s){if(!s||s.scope==="off")return"";const e=[];if(s.previousTopics.length>0&&e.push(`
...
`)}
`)}return e.join(`
`)}async function cs(s,e){try{const t=await k.getAIPreferences(s);if(t.responseHistoryScope==="off")return{previousTopics:[],corrections:[],scope:"off"};const[r,a]=await Promise.all([k.getResponseTopics(s,e,t.responseHistoryScope),k.getActiveCorrections(s,e,!0)]);return{previousTopics:r,corrections:a,scope:t.responseHistoryScope}}catch(t){return console.error("[PromptSystem] Error fetching behavior context:",t),null}}const $e=3e3,zt=2e4,be=["story_so_far","quest_log","characters","boss_strategy","tips","hidden_secrets","points_of_interest"],Ye=`
...
`;async function ke(){try{const s=await mt.getConversations(),e=Object.values(s).filter(r=>!r.isGameHub&&r.gameTitle).map(r=>({title:r.gameTitle,genre:r.genre||"Unknown",progress:r.gameProgress||0,isUnreleased:r.isUnreleased||!1})).sort((r,a)=>r.title.localeCompare(a.title));if(e.length===0)return`
**📚 USER'S GAME LIBRARY:**
No game tabs created yet. User is new or exploring games.
`;const t=e.map(r=>{const a=r.progress>0?` (${r.progress}% progress)`:"",o=r.isUnreleased?" [UNRELEASED]":"";return`  • ${r.title} [${r.genre}]${a}${o}`}).join(`
`);return`
...
`}catch(s){return console.warn("[PromptSystem] Failed to build game library context:",s),""}}const jt=async(s,e,t,r,a)=>{var S;let o=0;const n=[],i=2500,u=[...s.subtabs||[]].filter(d=>d.status==="loaded"&&d.content).sort((d,m)=>{const A=be.indexOf(d.id),w=be.indexOf(m.id);return(A===-1?999:A)-(w===-1?999:w)}).map(d=>{const m=d.content||"";m.length>i&&m.includes("<details>")&&n.push(d.id);const A=m.length>$e?m.slice(0,$e)+"...[truncated]":m,w=`### ${d.title} (ID: ${d.id})
${A}`;if(o+w.length>zt){const I=be.indexOf(d.id);if(I!==-1&&I<3){const L=`### ${d.title} (ID: ${d.id}) [SUMMARY]
${m.slice(0,500)}...`;return o+=L.length,L}return null}return o+=w.length,w}).filter(Boolean).join(`
...
`:"",O=a||X.getDefaultProfile(),q=X.buildProfileContext(O);let U="";if(s.gameTitle){const d=dt.getByGameTitle(s.gameTitle);if(d!=null&&d.igdbGameId)try{const m=await gt.getForContext(d.igdbGameId);if(m){const A=Math.ceil(m.length/4);U=`

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🎮 GAME KNOWLEDGE DATABASE: "${s.gameTitle.toUpperCase()}"
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

⚠️ VERIFY FIRST: Is query about "${s.gameTitle}"?
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
END OF ${s.gameTitle.toUpperCase()} KNOWLEDGE (${A} tokens)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

`,console.log(`🎮 [PromptSystem] Injecting ${m.length} chars of FULL game knowledge (no truncation)`)}}catch(m){console.warn("🎮 [PromptSystem] Failed to fetch game knowledge:",m)}}const h=await ke();return`
...
`},Jt=async(s,e,t,r)=>{const a=r||X.getDefaultProfile(),o=X.buildProfileContext(a),n=await ke(),i=s.messages.slice(-10).map(c=>`${c.role==="user"?"User":"Otagon"}: ${c.content}`).join(`
`);return`
...
**Tag Definitions:**
${ye}
`},Xt=async(s,e,t,r)=>{const a=r||X.getDefaultProfile(),o=X.buildProfileContext(a),n=await ke(),i=s.gameTitle?`
...
**Tag Definitions:**
${ye}
`},ls=async(s,e,t,r,a,o,n,i,c)=>{const u=n?Yt(n):"",f=i?`
**User Timezone:** ${i}
...
*Content loading...*`}}const ds=new Zt,er=`You are a correction validator for OTAKON, an AI gaming companion.
...
Is this correction valid? Respond with JSON only.`,Ae="otakon_correction_submissions",te=3;function ze(){try{const s=localStorage.getItem(Ae),e=Date.now();if(!s)return{allowed:!0,remaining:te};const t=JSON.parse(s);if(e>t.resetAt)return{allowed:!0,remaining:te};const r=te-t.count;return{allowed:r>0,remaining:Math.max(0,r)}}catch{return{allowed:!0,remaining:te}}}function rr(){try{const s=localStorage.getItem(Ae),e=Date.now(),t=1440*60*1e3;let r={count:0,resetAt:e+t};s&&(r=JSON.parse(s),e>r.resetAt&&(r={count:0,resetAt:e+t})),r.count++,localStorage.setItem(Ae,JSON.stringify(r))}catch{}}async function qe(s,e,t){if(!e.trim())return{isValid:!1,reason:"Correction text is empty"};if(e.length<5)return{isValid:!1,reason:"Correction is too short"};if(e.length>1e3)return{isValid:!1,reason:"Correction is too long (max 1000 characters)"};const r=[/\b(hate|kill|die|attack)\s+(all|every|those)\b/i,/\b(racial|ethnic)\s+slur/i,/\bviolence\s+against\b/i];for(const a of r)if(a.test(e))return{isValid:!1,reason:"Correction contains inappropriate content"};try{const{data:a,error:o}=await l.functions.invoke("gemini-chat",{body:{messages:[{role:"system",content:er},{role:"user",content:tr(s,e,t)}],model:"gemini-3-flash-preview",temperature:.1,maxTokens:200}});if(o)return console.error("[CorrectionService] Validation API error:",o),{isValid:!1,reason:"Validation service temporarily unavailable. Please try again later."};const n=(a==null?void 0:a.content)||(a==null?void 0:a.response)||"",i=n.match(/\{[\s\S]*\}/);if(i){const c=JSON.parse(i[0]);return{isValid:c.isValid===!0,reason:c.reason||"Validation complete",suggestedType:c.suggestedType||void 0}}return console.warn("[CorrectionService] Could not parse validation response:",n),{isValid:!1,reason:"Validation response was unclear. Please try again."}}catch(a){return console.error("[CorrectionService] Validation exception:",a),{isValid:!1,reason:"Validation failed. Please try again later."}}}async function sr(s,e){if(!ze().allowed)return{success:!1,error:`Daily correction limit reached (${te}/day). Try again tomorrow.`};const r=await qe(e.originalResponse,e.correctionText,e.gameTitle),{error:a}=await l.from("ai_feedback").insert({user_id:s,conversation_id:e.conversationId,message_id:e.messageId,feedback_type:"down",content_type:"message",category:"correction",comment:e.originalResponse.slice(0,500),correction_text:e.correctionText,correction_type:e.type,correction_scope:e.scope,is_validated:r.isValid,validation_reason:r.reason,game_title:e.gameTitle});if(a)return console.error("[CorrectionService] Failed to store feedback:",a),{success:!1,error:"Failed to save correction"};if(!r.isValid)return{success:!1,error:r.reason};const o=await k.addCorrection(s,{gameTitle:e.gameTitle,originalSnippet:e.originalResponse.slice(0,200),correctionText:e.correctionText,type:r.suggestedType||e.type,scope:e.scope});return o.success?(rr(),{success:!0,correction:(await k.getActiveCorrections(s,e.gameTitle)).find(c=>c.correctionText===e.correctionText)}):{success:!1,error:o.error}}const ar=[/\b(boss(?:es)?|mini-boss|final boss)\b/gi,/\b(enemy types?|elite enemies|common enemies)\b/gi,/\b(legendary weapon|rare item|unique gear)\b/gi,/\b(skill tree|ability points?|talent build)\b/gi,/\b(main quest|side quest|daily quest)\b/gi,/\b(damage build|tank build|support build|dps build)\b/gi,/\b(speedrun(?:ning)?|world record|personal best)\b/gi,/\b(secret area|hidden path|easter egg)\b/gi,/\b(tier list|meta build|optimal strategy)\b/gi,/\b(patch notes?|balance changes?|nerf(?:ed)?|buff(?:ed)?)\b/gi,/\b(dlc|expansion pack|season pass)\b/gi,/\b(game mechanics?|combat system|progression system)\b/gi,/\b(character build|loadout guide|equipment guide)\b/gi],or=/\b([A-Z][a-z]{2,}(?:\s+(?:of|the|and)\s+)?[A-Z][a-z]{2,}(?:\s+[A-Z][a-z]+)?)\b/g;function nr(s){if(!s||s.length<100)return[];const e=new Set;for(const n of ar){const i=s.match(n);if(i)for(const c of i){const u=c.toLowerCase().trim();u.length>=6&&u.length<=50&&u.includes(" ")&&e.add(u)}}let t=0;const r=s.match(or);if(r)for(const n of r){if(t>=5)break;const i=n.toLowerCase().trim();i.length>=6&&i.length<=40&&(e.add(i),t++)}const a=/\b(chapter|level|stage|floor|wave|round|phase|part|episode|act)\s+(\d+)\b/gi;let o;for(;(o=a.exec(s))!==null;)e.add(`${o[1].toLowerCase()} ${o[2]}`);return Array.from(e).slice(0,10)}async function ir(s){return(await k.getBehaviorData(s)).aiCorrections}async function cr(s,e){return k.getActiveCorrections(s,e,!0)}function lr(){return ze()}const W={validateCorrection:qe,submitCorrection:sr,getAllCorrections:ir,getContextualCorrections:cr,getRateLimitStatus:lr,extractTopicsFromResponse:nr,toggleCorrection:k.toggleCorrection,removeCorrection:k.removeCorrection},ae={free:{gameKnowledge:0,aiMessages:4},pro:{gameKnowledge:20,aiMessages:30},vanguard_pro:{gameKnowledge:20,aiMessages:30}},Oe=new Set(["fortnite","apex legends","warzone","call of duty warzone","pubg","playerunknown's battlegrounds","league of legends","lol","dota 2","dota","smite","heroes of the storm","overwatch","overwatch 2","valorant","rainbow six siege","r6","paladins","world of warcraft","wow","final fantasy xiv","ffxiv","ff14","guild wars 2","gw2","elder scrolls online","eso","destiny 2","destiny","warframe","lost ark","new world","diablo 4","diablo iv","path of exile","poe","genshin impact","honkai star rail","zenless zone zero","zzz","wuthering waves","tower of fantasy","fifa","ea fc","fc 24","fc 25","madden","nba 2k","2k24","2k25","hearthstone","marvel snap","legends of runeterra","lor","magic arena","mtg arena","street fighter 6","sf6","tekken 8","mortal kombat 1","mk1","guilty gear strive","the finals","xdefiant","helldivers 2","sea of thieves","no man's sky","fall guys","rocket league","dead by daylight","dbd"]),ve=new Set(["gta 6","grand theft auto 6","grand theft auto vi","monster hunter wilds","death stranding 2","death stranding 2: on the beach","ghost of yotei","like a dragon: pirate yakuza in hawaii","kingdom come deliverance 2","kingdom come 2","avowed","civilization 7","civ 7","civilization vii","fable","assassin's creed shadows","split fiction","doom: the dark ages","borderlands 4","mafia: the old country","marvel 1943","judge 0"]),Ve=new Date("2025-01-31T23:59:59Z").getTime();function je(s){return s?s*1e3>Ve:!1}function Ce(s){if(!s)return!1;const e=s.toLowerCase().trim();return Oe.has(e)||Array.from(Oe).some(t=>e.includes(t))}function Ie(s){if(!s)return!1;const e=s.toLowerCase().trim();return ve.has(e)||Array.from(ve).some(t=>e.includes(t))}function Je(s,e,t){const r=s.toLowerCase();if(je(t))return console.log("🔍 [GroundingControl] Game detected as post-cutoff via IGDB release date"),"post_cutoff_game";if(Ie(e)||Ie(s))return"post_cutoff_game";const a=Ce(e)||Ce(s);return a&&(r.includes("meta")||r.includes("tier list")||r.includes("best")||r.includes("current")||r.includes("viable")||r.includes("nerf")||r.includes("buff")||r.includes("season")||r.includes("ranked")||r.includes("competitive")||r.includes("patch"))?"live_service_meta":r.includes("latest news")||r.includes("recent news")||r.includes("gaming news")||r.includes("announced today")||r.includes("announced this week")||r.includes("just announced")||r.includes("breaking news")||r.includes("new announcement")?"current_news":r.includes("patch notes")||r.includes("latest patch")||r.includes("recent update")||r.includes("new update")||r.includes("hotfix")||r.includes("balance change")||r.includes("what changed")||r.includes("patch")&&(r.includes("today")||r.includes("latest")||r.includes("new"))?"patch_notes":r.includes("release")&&r.includes("date")||r.includes("when does")||r.includes("when is")||r.includes("coming out")||r.includes("launch date")||r.includes("coming soon")||r.includes("2025")||r.includes("2026")?"release_dates":!a&&(r.includes("how do i")||r.includes("how to")||r.includes("help me")||r.includes("stuck on")||r.includes("boss")||r.includes("strategy")||r.includes("build")||r.includes("tips")||r.includes("guide")||r.includes("walkthrough")||r.includes("where is")||r.includes("where can i")||r.includes("best way to")||r.includes("how do you")||r.includes("explain"))?"game_help":"general_knowledge"}function Xe(s,e,t,r){const a=ae[e];return s==="post_cutoff_game"?t>=a.gameKnowledge?{useGrounding:!1,reason:`Game knowledge limit reached (${t}/${a.gameKnowledge}). AI will use training knowledge.`,usageType:"game_knowledge"}:{useGrounding:!0,reason:"Game released after AI knowledge cutoff (Jan 2025) - web search required",usageType:"game_knowledge"}:r>=a.aiMessages?{useGrounding:!1,reason:`AI message limit reached (${r}/${a.aiMessages}). AI will use training knowledge.`,usageType:"ai_message"}:s==="live_service_meta"?{useGrounding:!0,reason:"Live service game - current meta/patch info requires web search",usageType:"ai_message"}:s==="current_news"?{useGrounding:!0,reason:"Current news query requires web search",usageType:"ai_message"}:s==="patch_notes"?{useGrounding:!0,reason:"Recent patch notes require web search",usageType:"ai_message"}:s==="release_dates"?{useGrounding:!0,reason:"Release date verification via web search",usageType:"ai_message"}:s==="game_help"?{useGrounding:!1,reason:"Known game - AI has comprehensive training knowledge",usageType:"ai_message"}:s==="general_knowledge"?{useGrounding:!1,reason:"General gaming knowledge - AI can answer from training",usageType:"ai_message"}:{useGrounding:!1,reason:"Default: use AI knowledge",usageType:"ai_message"}}const he=new Map,ur=300*1e3;let j=null;function Qe(){const s=new Date;return`${s.getFullYear()}-${String(s.getMonth()+1).padStart(2,"0")}`}async function Pe(s){var r,a;console.log("[GroundingControl] getGroundingUsage called for:",s);const e=Qe(),t=he.get(s);if(t&&t.month===e&&Date.now()-t.lastSync<ur){const o={gameKnowledge:t.gameKnowledgeCount,aiMessages:t.aiMessageCount,total:t.count};return console.log("[GroundingControl] Returning cached usage:",o),o}if(j===!1)return{gameKnowledge:(t==null?void 0:t.gameKnowledgeCount)||0,aiMessages:(t==null?void 0:t.aiMessageCount)||0,total:(t==null?void 0:t.count)||0};try{const{data:o,error:n}=await l.from("user_grounding_usage").select("usage_count, game_knowledge_count, ai_message_count").eq("auth_user_id",s).eq("month_year",e).maybeSingle();if(n)return n.code==="42P01"||n.code==="PGRST205"||(r=n.message)!=null&&r.includes("does not exist")||(a=n.message)!=null&&a.includes("Could not find")?(console.warn("[GroundingControl] DB table not yet created, using in-memory tracking"),j=!1,{gameKnowledge:(t==null?void 0:t.gameKnowledgeCount)||0,aiMessages:(t==null?void 0:t.aiMessageCount)||0,total:(t==null?void 0:t.count)||0}):(console.error("[GroundingControl] Failed to fetch usage:",n),{gameKnowledge:(t==null?void 0:t.gameKnowledgeCount)||0,aiMessages:(t==null?void 0:t.aiMessageCount)||0,total:(t==null?void 0:t.count)||0});if(j=!0,console.log("[GroundingControl] Raw data from DB:",o),!o)return console.log("[GroundingControl] No usage record found for this month - user has full quota"),{gameKnowledge:0,aiMessages:0,total:0};const i=(o==null?void 0:o.game_knowledge_count)||0,c=(o==null?void 0:o.ai_message_count)||0,u=(o==null?void 0:o.usage_count)||0;return he.set(s,{count:u,gameKnowledgeCount:i,aiMessageCount:c,month:e,lastSync:Date.now()}),{gameKnowledge:i,aiMessages:c,total:u}}catch(o){return console.error("[GroundingControl] Error fetching usage:",o),{gameKnowledge:(t==null?void 0:t.gameKnowledgeCount)||0,aiMessages:(t==null?void 0:t.aiMessageCount)||0,total:(t==null?void 0:t.count)||0}}}async function dr(s,e="ai_message"){var a;const t=Qe(),r=he.get(s);if(r&&r.month===t?(r.count++,e==="game_knowledge"?r.gameKnowledgeCount++:r.aiMessageCount++,r.lastSync=Date.now()):he.set(s,{count:1,gameKnowledgeCount:e==="game_knowledge"?1:0,aiMessageCount:e==="ai_message"?1:0,month:t,lastSync:Date.now()}),j===!1){console.log(`🔍 [GroundingControl] Incremented ${e} usage (in-memory) for ${s} (month: ${t})`);return}try{const{error:o}=await l.rpc("increment_grounding_usage",{p_auth_user_id:s,p_month_year:t,p_usage_type:e});if(o){if(o.code==="42P01"||(a=o.message)!=null&&a.includes("does not exist")){console.warn("[GroundingControl] DB table not yet created, using in-memory tracking"),j=!1;return}await l.rpc("increment_grounding_usage",{p_auth_user_id:s,p_month_year:t})}j=!0,console.log(`🔍 [GroundingControl] Incremented usage for ${s} (month: ${t})`)}catch(o){console.error("[GroundingControl] Failed to increment usage:",o)}}async function gr(s,e){try{const t=e||"free";console.log("[GroundingControl] getRemainingQuota called with:",{authUserId:s,tier:t});const r=await Pe(s);console.log("[GroundingControl] getGroundingUsage returned:",r);const a=ae[t];return console.log("[GroundingControl] Limits for tier:",a),r?{gameKnowledge:{used:r.gameKnowledge,limit:a.gameKnowledge,remaining:Math.max(0,a.gameKnowledge-r.gameKnowledge)},aiMessages:{used:r.aiMessages,limit:a.aiMessages,remaining:Math.max(0,a.aiMessages-r.aiMessages)}}:(console.error("[GroundingControl] getGroundingUsage returned undefined"),{gameKnowledge:{used:0,limit:a.gameKnowledge,remaining:a.gameKnowledge},aiMessages:{used:0,limit:a.aiMessages,remaining:a.aiMessages}})}catch(t){console.error("[GroundingControl] Error in getRemainingQuota:",t);const a=ae[e||"free"];return{gameKnowledge:{used:0,limit:a.gameKnowledge,remaining:a.gameKnowledge},aiMessages:{used:0,limit:a.aiMessages,remaining:a.aiMessages}}}}async function mr(s,e,t,r,a){const o=Je(t,r,a),n=await Pe(s),i=ae[e],c={gameKnowledge:{used:n.gameKnowledge,limit:i.gameKnowledge,remaining:Math.max(0,i.gameKnowledge-n.gameKnowledge)},aiMessages:{used:n.aiMessages,limit:i.aiMessages,remaining:Math.max(0,i.aiMessages-n.aiMessages)}},{useGrounding:u,reason:f,usageType:T}=Xe(o,e,n.gameKnowledge,n.aiMessages);return console.log("🔍 [GroundingControl] Check result:",{tier:e,queryType:o,useGrounding:u,usageType:T,reason:f,usage:{gameKnowledge:`${n.gameKnowledge}/${i.gameKnowledge}`,aiMessages:`${n.aiMessages}/${i.aiMessages}`},remainingQuota:c,igdbReleaseDate:a?new Date(a*1e3).toISOString():"N/A"}),{useGrounding:u,queryType:o,reason:f,usageType:T,remainingQuota:c}}const gs={classifyQuery:Je,shouldUseGrounding:Xe,getGroundingUsage:Pe,incrementGroundingUsage:dr,getRemainingQuota:gr,checkGroundingEligibility:mr,isLiveServiceGame:Ce,isPostCutoffGame:Ie,isRecentRelease:je,GROUNDING_LIMITS:ae,LIVE_SERVICE_GAMES:Oe,POST_CUTOFF_GAMES:ve,KNOWLEDGE_CUTOFF_TIMESTAMP:Ve},hr={light:10,medium:20,heavy:40,success:[10,50,10],error:[50,100,50],warning:[20,50,20,50,20],buttonPress:15,modalOpen:[10,30,10],modalClose:10,tabSwitch:12,messageSend:[10,40,15],swipe:8,longPress:[0,50,30],notification:[20,100,20,100,30]};function Ze(){return typeof navigator<"u"&&"vibrate"in navigator&&typeof navigator.vibrate=="function"}function et(){return St()&&Ze()}function _(s){var e;if(!et())return!1;try{const t=hr[s];return(e=navigator.vibrate)==null||e.call(navigator,t),!0}catch(t){return console.warn("[Haptic] Failed to trigger haptic feedback:",t),!1}}function pr(){var s;Ze()&&((s=navigator.vibrate)==null||s.call(navigator,0))}const ms={light:()=>_("light"),medium:()=>_("medium"),heavy:()=>_("heavy"),success:()=>_("success"),error:()=>_("error"),warning:()=>_("warning"),button:()=>_("buttonPress"),modalOpen:()=>_("modalOpen"),modalClose:()=>_("modalClose"),tabSwitch:()=>_("tabSwitch"),messageSend:()=>_("messageSend"),swipe:()=>_("swipe"),longPress:()=>_("longPress"),notification:()=>_("notification"),stop:pr,isAvailable:et};class fr{constructor(){E(this,"STORAGE_KEY","otakon_used_suggested_prompts");E(this,"LAST_RESET_KEY","otakon_suggested_prompts_last_reset");E(this,"RESET_INTERVAL_MS",1440*60*1e3);E(this,"usedPrompts",new Set);this.loadUsedPrompts(),this.checkAndResetIfNeeded()}loadUsedPrompts(){try{const e=localStorage.getItem(this.STORAGE_KEY);if(e){const t=JSON.parse(e);this.usedPrompts=new Set(t)}}catch{this.usedPrompts=new Set}}saveUsedPrompts(){try{const e=Array.from(this.usedPrompts);localStorage.setItem(this.STORAGE_KEY,JSON.stringify(e))}catch{}}checkAndResetIfNeeded(){try{const e=localStorage.getItem(this.LAST_RESET_KEY),t=Date.now();(!e||t-parseInt(e)>=this.RESET_INTERVAL_MS)&&(this.resetUsedPrompts(),localStorage.setItem(this.LAST_RESET_KEY,t.toString()))}catch{}}markPromptAsUsed(e){this.usedPrompts.add(e),this.saveUsedPrompts()}isPromptUsed(e){return this.usedPrompts.has(e)}getUnusedPrompts(e){return e.filter(t=>!this.isPromptUsed(t))}areAllPromptsUsed(e){return e.every(t=>this.isPromptUsed(t))}resetUsedPrompts(){this.usedPrompts.clear(),localStorage.removeItem(this.STORAGE_KEY)}getUsedCount(){return this.usedPrompts.size}getTimeUntilNextReset(){try{const e=localStorage.getItem(this.LAST_RESET_KEY);if(!e)return 0;const t=parseInt(e)+this.RESET_INTERVAL_MS;return Math.max(0,t-Date.now())}catch{return 0}}getStaticNewsPrompts(){return ht}processAISuggestions(e){if(console.log("🔧 [SuggestedPromptsService] processAISuggestions INPUT:",e),console.log("🔧 [SuggestedPromptsService] INPUT type:",typeof e),console.log("🔧 [SuggestedPromptsService] INPUT is Array:",Array.isArray(e)),console.log("🔧 [SuggestedPromptsService] INPUT JSON:",JSON.stringify(e)),!e)return console.log("🔧 [SuggestedPromptsService] No suggestions - returning []"),[];if(Array.isArray(e)&&e.length===0)return console.log("🔧 [SuggestedPromptsService] Empty array - returning []"),[];let t=[];if(Array.isArray(e))console.log("🔧 [SuggestedPromptsService] Input is array, checking elements..."),t=e.flat().filter(a=>{const o=typeof a=="string"&&a.trim().length>0;return!o&&a!==null&&a!==void 0&&console.log("🔧 [SuggestedPromptsService] Filtering out non-string item:",a,typeof a),o}),console.log("🔧 [SuggestedPromptsService] Array after filtering:",t);else if(typeof e=="string"){let a=e.trim();a.startsWith('["')&&!a.endsWith('"]')&&(a.endsWith('"')||(a+='"'),a.endsWith("]")||(a+="]"));try{const o=JSON.parse(a);Array.isArray(o)?t=o:t=[e]}catch{a.includes('", "')||a.includes(`",
"`)?t=a.split(/",\s*"/).map(n=>n.replace(/^["\s]+|["\s]+$/g,"")).filter(n=>n.length>0):a.includes(`
`)?t=a.split(`
`).map(n=>n.replace(/^["\s]+|["\s]+$/g,"")).filter(n=>n.length>0):t=[e]}}const r=t.filter(a=>a&&typeof a=="string"&&a.trim().length>0).map(a=>a.trim()).slice(0,3);return console.log("🔧 [SuggestedPromptsService] suggestionsArray before filter:",t),console.log("🔧 [SuggestedPromptsService] RESULT after filter/map/slice:",r),console.log("🔧 [SuggestedPromptsService] RESULT length:",r.length),r}getFallbackSuggestions(e,t){return t===!0||e==="game-hub"||e==="everything-else"?this.getStaticNewsPrompts():["What should I do next in this area?","Tell me about the story so far","Give me some tips for this game","What are the key mechanics I should know?"]}}const hs=new fr,oe=new Map,yr=300*1e3;function Sr(s,e,t){return`${s}:${e}:${t||"global"}`}function br(s){const e=oe.get(s);return e?Date.now()-e.timestamp>yr?(oe.delete(s),null):e.prompts:null}function Er(s,e){oe.set(s,{prompts:e,timestamp:Date.now()})}function tt(s){for(const e of oe.keys())e.startsWith(s)&&oe.delete(e)}async function Ue(s,e,t=null,r=20){const a=Sr(s,e,t),o=br(a);if(o)return o;try{const n=new Date;n.setDate(n.getDate()-7);let i=l.from("ai_shown_prompts").select("prompt_text").eq("auth_user_id",s).eq("prompt_type",e).gte("shown_at",n.toISOString()).order("shown_at",{ascending:!1}).limit(r);t&&(i=i.or(`game_title.eq.${t},game_title.is.null`));const{data:c,error:u}=await i;if(u)return console.error("[ShownPromptsService] Error fetching prompts:",u),[];const f=(c||[]).map(T=>T.prompt_text);return Er(a,f),f}catch(n){return console.error("[ShownPromptsService] Exception fetching prompts:",n),[]}}async function Tr(s,e){try{const{error:t}=await l.from("ai_shown_prompts").insert({auth_user_id:s,prompt_text:e.promptText,prompt_type:e.promptType,game_title:e.gameTitle||null,conversation_id:e.conversationId||null});return t?(console.error("[ShownPromptsService] Error recording prompt:",t),!1):(tt(s),!0)}catch(t){return console.error("[ShownPromptsService] Exception recording prompt:",t),!1}}async function wr(s,e){if(!e.length)return!0;try{const t=e.map(a=>({auth_user_id:s,prompt_text:a.promptText,prompt_type:a.promptType,game_title:a.gameTitle||null,conversation_id:a.conversationId||null})),{error:r}=await l.from("ai_shown_prompts").insert(t);return r?(console.error("[ShownPromptsService] Error batch recording prompts:",r),!1):(tt(s),!0)}catch(t){return console.error("[ShownPromptsService] Exception batch recording prompts:",t),!1}}async function _r(s,e){try{const{error:t}=await l.from("ai_shown_prompts").update({clicked:!0,clicked_at:new Date().toISOString()}).eq("auth_user_id",s).eq("prompt_text",e).is("clicked",!1);return t?(console.error("[ShownPromptsService] Error marking prompt clicked:",t),!1):!0}catch(t){return console.error("[ShownPromptsService] Exception marking prompt clicked:",t),!1}}async function Ar(s,e,t,r=null){if(!e.length)return[];const a=await Ue(s,t,r),o=new Set(a.map(n=>n.toLowerCase().trim()));return e.filter(n=>!o.has(n.toLowerCase().trim()))}async function Or(s,e,t,r=null){const a=await Ue(s,t,r),o=e.toLowerCase().trim();return a.some(n=>n.toLowerCase().trim()===o)}const ps={getRecentShownPrompts:Ue,recordShownPrompt:Tr,recordShownPrompts:wr,markPromptClicked:_r,filterNewPrompts:Ar,hasPromptBeenShown:Or},z=class z{static getInstance(){return z.instance||(z.instance=new z),z.instance}async getSubtabs(e){return this.getSubtabsFromTable(e)}async setSubtabs(e,t,r=!1){console.error(`🔄 [SubtabsService] ${r?"Updating":"Creating"} ${t.length} subtabs in normalized table for conversation:`,e);const o=await this.setSubtabsInTable(e,t);return console.error("  ✅ Table write:",o?"SUCCESS":"FAILED"),o}async addSubtab(e,t){const{data:r,error:a}=await l.from("conversations").select("is_unreleased, title").eq("id",e).single();if(a)return console.error("Error checking conversation for unreleased status:",a),null;if(r!=null&&r.is_unreleased)throw new Error("Subtabs cannot be created for unreleased games. This feature will be available once the game is released.");return await this.addSubtabToTable(e,t)}async updateSubtab(e,t,r){return await this.updateSubtabInTable(t,r)}async deleteSubtab(e,t){return this.deleteSubtabFromTable(t)}async getSubtabsFromTable(e){try{const{data:t,error:r}=await l.from("subtabs").select("*").eq("conversation_id",e).order("order_index",{ascending:!0});return r?(console.error("Error getting subtabs from table:",r),[]):(t||[]).map(a=>{const o=typeof a.metadata=="object"&&a.metadata!==null?a.metadata:{};return{id:a.id,conversationId:a.conversation_id??void 0,title:a.title,content:a.content||"",type:a.tab_type,isNew:o.isNew||!1,status:o.status||"loaded",instruction:o.instruction}})}catch(t){return console.error("Error getting subtabs from table:",t),[]}}async setSubtabsInTable(e,t){try{const{data:r,error:a}=await l.from("conversations").select("*").eq("id",e).single(),o=r==null?void 0:r.auth_user_id;if(a||!o)return console.error("❌ [SubtabsService] Error getting conversation auth_user_id:",a),console.error("❌ [SubtabsService] Conversation may not exist yet. ConversationId:",e),!1;console.error(`🔄 [SubtabsService] Using auth_user_id: ${o} for ${t.length} subtabs`);const{error:n}=await l.from("subtabs").delete().eq("conversation_id",e);if(n)return console.error("Error deleting existing subtabs:",n),!1;if(t.length>0){const i=t.map((u,f)=>(u.type||console.error(`⚠️ [SubtabsService] Subtab "${u.title}" has NULL type! Using fallback.`),{id:u.id,conversation_id:e,game_id:null,title:u.title||"Untitled",content:u.content||"",tab_type:u.type||"chat",order_index:f,auth_user_id:o,metadata:{isNew:u.isNew,status:u.status,instruction:u.instruction}}));console.error("🔄 [SubtabsService] Inserting subtabs:",i.map(u=>({title:u.title,tab_type:u.tab_type,has_auth_user_id:!!u.auth_user_id})));const{error:c}=await l.from("subtabs").insert(i);if(c)return console.error("❌ [SubtabsService] Error inserting subtabs:",c),console.error("❌ [SubtabsService] Failed subtabs data:",JSON.stringify(i,null,2)),!1;console.error("✅ [SubtabsService] Successfully inserted",t.length,"subtabs")}return!0}catch(r){return console.error("❌ [SubtabsService] Error setting subtabs in table:",r),!1}}async addSubtabToTable(e,t){var r;try{const{data:a}=await l.from("conversations").select("game_id").eq("id",e).single(),o=(a==null?void 0:a.game_id)||"",{data:n}=await l.from("subtabs").select("order_index").eq("conversation_id",e).order("order_index",{ascending:!1}).limit(1),i=((r=n==null?void 0:n[0])==null?void 0:r.order_index)??-1,{data:c,error:u}=await l.from("subtabs").insert({id:t.id,conversation_id:e,game_id:o,title:t.title,content:t.content,tab_type:t.type,order_index:i+1,metadata:{isNew:t.isNew,status:t.status,instruction:t.instruction}}).select().single();return u?(console.error("Error adding subtab to table:",u),null):{id:c.id,conversationId:c.conversation_id??void 0,title:c.title,content:pt(c.content),type:c.tab_type,isNew:typeof c.metadata=="object"&&c.metadata!==null&&!Array.isArray(c.metadata)&&c.metadata.isNew||!1,status:(typeof c.metadata=="object"&&c.metadata!==null&&!Array.isArray(c.metadata)?c.metadata.status:void 0)||"loaded",instruction:typeof c.metadata=="object"&&c.metadata!==null&&!Array.isArray(c.metadata)?c.metadata.instruction:void 0}}catch(a){return console.error("Error adding subtab to table:",a),null}}async updateSubtabInTable(e,t){try{const r={};if(t.title!==void 0&&(r.title=t.title),t.content!==void 0&&(r.content=t.content),t.type!==void 0&&(r.tab_type=t.type),t.isNew!==void 0||t.status!==void 0||t.instruction!==void 0){const{data:o}=await l.from("subtabs").select("metadata").eq("id",e).single(),n=typeof(o==null?void 0:o.metadata)=="object"&&(o==null?void 0:o.metadata)!==null?o.metadata:{};r.metadata={...n,...t.isNew!==void 0&&{isNew:t.isNew},...t.status!==void 0&&{status:t.status},...t.instruction!==void 0&&{instruction:t.instruction}}}const{error:a}=await l.from("subtabs").update(r).eq("id",e);return a?(console.error("Error updating subtab in table:",a),!1):!0}catch(r){return console.error("Error updating subtab in table:",r),!1}}async deleteSubtabFromTable(e){try{const{error:t}=await l.from("subtabs").delete().eq("id",e);return t?(console.error("Error deleting subtab from table:",t),!1):!0}catch(t){return console.error("Error deleting subtab from table:",t),!1}}async getSubtabsFromJsonb(e){try{const{data:t,error:r}=await l.from("conversations").select("subtabs").eq("id",e).single();return r?(console.error("Error getting subtabs from JSONB:",r),[]):(t==null?void 0:t.subtabs)||[]}catch(t){return console.error("Error getting subtabs from JSONB:",t),[]}}async setSubtabsInJsonb(e,t){try{const{error:r}=await l.from("conversations").update({subtabs:t,subtabs_order:t.map(a=>a.id)}).eq("id",e);return r?(console.error("Error setting subtabs in JSONB:",r),!1):!0}catch(r){return console.error("Error setting subtabs in JSONB:",r),!1}}async migrateConversationSubtabs(e){try{const t=await this.getSubtabsFromJsonb(e);return t.length===0?!0:await this.setSubtabsInTable(e,t)}catch(t){return console.error("Error migrating subtabs:",t),!1}}async rollbackConversationSubtabs(e){try{const t=await this.getSubtabsFromTable(e);return t.length===0?!0:await this.setSubtabsInJsonb(e,t)}catch(t){return console.error("Error rolling back subtabs:",t),!1}}async migrateAllSubtabs(){try{const{data:e,error:t}=await l.from("conversations").select("id, subtabs").not("subtabs","is",null);if(t)return console.error("Error fetching conversations:",t),{success:0,failed:0};let r=0,a=0;const o=(e||[]).filter(i=>i.subtabs&&Array.isArray(i.subtabs)&&i.subtabs.length>0).map(i=>this.migrateConversationSubtabs(i.id));return(await Promise.allSettled(o)).forEach(i=>{i.status==="fulfilled"&&i.value?r++:a++}),{success:r,failed:a}}catch(e){return console.error("Error in batch migration:",e),{success:0,failed:0}}}};E(z,"instance");let Ne=z;const fs=Ne.getInstance(),We={free:3,pro:10,vanguard_pro:10},ys={async canCreateUnreleasedTab(s,e){const t=We[e]||We.free,{count:r,error:a}=await l.from("unreleased_game_tabs").select("*",{count:"exact",head:!0}).eq("user_id",s);if(a)return console.error("[UnreleasedTabLimit] Error counting tabs:",a),{canCreate:!0,currentCount:0,limit:t};const o=r||0;return{canCreate:o<t,currentCount:o,limit:t}},async trackUnreleasedTab(s,e,t,r){const{error:a}=await l.from("unreleased_game_tabs").insert({user_id:s,conversation_id:e,game_id:t,game_title:r});return a?(console.error("[UnreleasedTabLimit] Error tracking tab:",a),!1):!0},async untrackUnreleasedTab(s){const{error:e}=await l.from("unreleased_game_tabs").delete().eq("conversation_id",s);return e?(console.error("[UnreleasedTabLimit] Error untracking tab:",e),!1):!0},async getUserUnreleasedTabs(s){const{data:e,error:t}=await l.from("unreleased_game_tabs").select("conversation_id, game_title, created_at").eq("user_id",s).order("created_at",{ascending:!1});return t?(console.error("[UnreleasedTabLimit] Error getting tabs:",t),[]):(e||[]).map(r=>({conversationId:r.conversation_id,gameTitle:r.game_title,createdAt:r.created_at}))}},K=s=>s;class Ss{static getCurrentUser(){return $.get(x.USER,null)}static setCurrentUser(e){$.set(x.USER,e)}static createUser(e,t=ft.FREE){const r=Date.now(),a=De[t];return{id:`user_${r}`,authUserId:`user_${r}`,email:e,tier:t,hasProfileSetup:!1,hasSeenSplashScreens:!1,hasSeenHowToUse:!1,hasSeenFeaturesConnected:!1,hasSeenProFeatures:!1,hasSeenWelcomeGuide:!1,pcConnected:!1,pcConnectionSkipped:!1,onboardingCompleted:!1,hasWelcomeMessage:!1,isNewUser:!0,hasUsedTrial:!1,lastActivity:r,preferences:{},textCount:0,imageCount:0,textLimit:a.text,imageLimit:a.image,totalRequests:0,lastReset:r,usage:{textCount:0,imageCount:0,textLimit:a.text,imageLimit:a.image,totalRequests:0,lastReset:r,tier:t},appState:{},profileData:{},onboardingData:{},behaviorData:{},feedbackData:{},usageData:{},createdAt:r,updatedAt:r}}static updateUser(e){const t=this.getCurrentUser();if(!t)return;const r={...t,...e,updatedAt:Date.now()};this.setCurrentUser(r)}static updateUsage(e){const t=this.getCurrentUser();t&&this.updateUser({usage:{...t.usage,...e}})}static resetUsage(){const e=this.getCurrentUser();if(!e)return;const t=De[e.tier];this.updateUsage({textCount:0,imageCount:0,totalRequests:0,lastReset:Date.now(),textLimit:t.text,imageLimit:t.image})}static canMakeRequest(e){const t=this.getCurrentUser();if(!t)return!1;const{usage:r}=t;return e==="text"?r.textCount<r.textLimit:r.imageCount<r.imageLimit}static incrementUsage(e){const t=this.getCurrentUser();if(!t)return;const r={totalRequests:t.usage.totalRequests+1};e==="text"?r.textCount=t.usage.textCount+1:r.imageCount=t.usage.imageCount+1,this.updateUsage(r)}static logout(){$.remove(x.USER)}static async getCurrentUserAsync(){try{const e=$.get(x.USER,null),{data:{user:t},error:r}=await l.auth.getUser();if(r||!t)return e;const{data:a,error:o}=await l.from("users").select("*").eq("auth_user_id",t.id).single();if(o||!a)return console.error("Failed to fetch user from Supabase:",o),e;const n={id:a.id,authUserId:a.auth_user_id,email:a.email,tier:a.tier,textCount:a.text_count||0,imageCount:a.image_count||0,textLimit:ie(a.text_limit),imageLimit:ie(a.image_limit),totalRequests:a.total_requests||0,lastReset:Q(a.last_reset),hasProfileSetup:a.has_profile_setup||!1,hasSeenSplashScreens:a.has_seen_splash_screens||!1,hasSeenHowToUse:a.has_seen_how_to_use||!1,hasSeenFeaturesConnected:a.has_seen_features_connected||!1,hasSeenProFeatures:a.has_seen_pro_features||!1,hasSeenWelcomeGuide:a.has_seen_welcome_guide||!1,pcConnected:a.pc_connected||!1,pcConnectionSkipped:a.pc_connection_skipped||!1,onboardingCompleted:a.onboarding_completed||!1,hasWelcomeMessage:a.has_welcome_message||!1,isNewUser:a.is_new_user||!1,hasUsedTrial:a.has_used_trial||!1,lastActivity:Q(a.updated_at),preferences:F(a.preferences),usage:{textCount:a.text_count||0,imageCount:a.image_count||0,textLimit:ie(a.text_limit),imageLimit:ie(a.image_limit),totalRequests:a.total_requests||0,lastReset:Q(a.last_reset),tier:a.tier},appState:F(a.app_state),profileData:F(a.profile_data),onboardingData:F(a.onboarding_data),behaviorData:F(a.behavior_data),feedbackData:F(a.feedback_data),usageData:F(a.usage_data),createdAt:Q(a.created_at),updatedAt:Q(a.updated_at)};return $.set(x.USER,n),console.log("🔍 [UserService] Loaded user from database:",{email:n.email,hasSeenWelcomeGuide:n.hasSeenWelcomeGuide,authUserId:n.authUserId}),n}catch(e){return console.error("Error in getCurrentUserAsync:",e),$.get(x.USER,null)}}static async setCurrentUserAsync(e){var t;try{console.log("🔍 [UserService] setCurrentUserAsync called with hasSeenWelcomeGuide:",e.hasSeenWelcomeGuide),$.set(x.USER,e),e.authUserId&&(await bt.setUser(e.authUserId,e),console.log("✅ [UserService] Updated cache for user:",e.authUserId));const{data:r,error:a,count:o}=await l.from("users").update({tier:e.tier,text_count:e.textCount,image_count:e.imageCount,text_limit:e.textLimit,image_limit:e.imageLimit,total_requests:e.totalRequests,last_reset:new Date(e.lastReset).toISOString(),has_profile_setup:e.hasProfileSetup,has_seen_splash_screens:e.hasSeenSplashScreens,has_seen_how_to_use:e.hasSeenHowToUse,has_seen_features_connected:e.hasSeenFeaturesConnected,has_seen_pro_features:e.hasSeenProFeatures,has_seen_welcome_guide:e.hasSeenWelcomeGuide,pc_connected:e.pcConnected,pc_connection_skipped:e.pcConnectionSkipped,onboarding_completed:e.onboardingCompleted,has_welcome_message:e.hasWelcomeMessage,has_used_trial:e.hasUsedTrial,preferences:K(e.preferences),profile_data:K(e.profileData),app_state:K(e.appState),onboarding_data:K(e.onboardingData),behavior_data:K(e.behaviorData),feedback_data:K(e.feedbackData),usage_data:K(e.usageData),updated_at:new Date().toISOString()}).eq("auth_user_id",e.authUserId).select();a?(console.error("❌ [UserService] Failed to sync user to Supabase:",a),console.error("❌ [UserService] Error details:",{message:a.message,code:a.code,details:a.details,hint:a.hint})):!r||r.length===0?(console.error("❌ [UserService] Database update succeeded but 0 rows affected!",{authUserId:e.authUserId,hasSeenWelcomeGuide:e.hasSeenWelcomeGuide,returnedData:r,count:o}),console.error("❌ [UserService] This indicates RLS policy is blocking the update silently!")):(console.log("✅ [UserService] Successfully updated database for user:",e.authUserId),console.log("✅ [UserService] Verified has_seen_welcome_guide in DB:",(t=r[0])==null?void 0:t.has_seen_welcome_guide))}catch(r){console.error("Error in setCurrentUserAsync:",r)}}static async updateUsageAsync(e){const t=await this.getCurrentUserAsync();if(!t)return;const r={...t,usage:{...t.usage,...e},textCount:e.textCount??t.textCount,imageCount:e.imageCount??t.imageCount,totalRequests:e.totalRequests??t.totalRequests,lastReset:e.lastReset??t.lastReset,updatedAt:Date.now()};await this.setCurrentUserAsync(r)}}class vr{hasTabCommand(e){return/^@\w+/.test(e.trim())}parseTabCommand(e,t){const r=e.trim();if(!this.hasTabCommand(r))return null;const a=r.match(/^@(\w+)\s*(\\modify|\\delete)?\s*(.*)$/);if(!a)return null;const[,o,n,i]=a,c=this.findMatchingTab(o,t.subtabs||[]);if(!c)return null;let u;return n==="\\delete"?u="delete":n==="\\modify"?u="modify":u="update",{type:u,tabId:c.id,tabName:c.title,instruction:i.trim()}}findMatchingTab(e,t){const r=this.normalizeTabName(e);let a=t.find(o=>this.normalizeTabName(o.id)===r||this.normalizeTabName(o.title)===r);return a||(a=t.find(o=>this.normalizeTabName(o.id).includes(r)||this.normalizeTabName(o.title).includes(r)),a)?a:(a=t.find(o=>r.includes(this.normalizeTabName(o.id))||r.includes(this.normalizeTabName(o.title))),a||null)}normalizeTabName(e){return e.toLowerCase().replace(/[_\s-]+/g,"").replace(/[^a-z0-9]/g,"")}getAvailableTabNames(e){return!e.subtabs||e.subtabs.length===0?[]:e.subtabs.map(t=>({id:t.id,title:t.title}))}formatTabSuggestion(e,t){return`@${e}`}getCommandHelp(){return`
**Tab Commands:**
...
Example: @story_so_far The player defeated the first boss
    `.trim()}validateCommand(e){switch(e.type){case"update":if(!e.instruction)return{valid:!1,error:"Update command requires content. Example: @story_so_far The player..."};break;case"modify":if(!e.instruction)return{valid:!1,error:"Modify command requires instructions. Example: @tips \\modify Change to combat strategies"};break}return{valid:!0}}describeCommand(e){switch(e.type){case"update":return`Updating "${e.tabName}" with: ${e.instruction}`;case"modify":return`Modifying "${e.tabName}": ${e.instruction}`;case"delete":return`Deleting "${e.tabName}"`}}}const bs=new vr;let g,V=[],Ke=!1,J="",D=null,G=null,v=null,M=null;const Cr="otakonSpeechRate",Ge=async()=>{try{const s=navigator;if(s.wakeLock){if(D!==null)try{await D.release()}catch{}D=await s.wakeLock.request("screen"),console.log("🔒 [TTS] Wake lock acquired - screen will stay on"),D.addEventListener("release",async()=>{console.log("🔓 [TTS] Wake lock released"),D=null,g&&g.speaking&&(console.log("🔒 [TTS] Reacquiring wake lock (TTS still active)"),await Ge())})}else console.warn("⚠️ [TTS] Wake lock API not available")}catch(s){console.warn("⚠️ [TTS] Wake lock request failed:",s)}},rt=async()=>{try{D!==null&&(await D.release(),D=null)}catch{}},Ir=()=>{try{if(!G){const s=window,e=s.AudioContext||s.webkitAudioContext;e&&(G=new e,console.log("🔊 [TTS] Audio context initialized"))}v||(v=new Audio,v.src="data:audio/wav;base64,UklGRnoGAABXQVZFZm10IBAAAAABAAEAQB8AAEAfAAABAAgAZGF0YQoGAACBhYqFbF1fdJivrJBhNjVgodDbq2EcBj+a2teleShtr9teleShtr9teleShtr9teleShtr9t",v.loop=!0,v.volume=.01,v.load(),console.log("🔇 [TTS] Silent audio initialized for background playback"))}catch(s){console.warn("⚠️ [TTS] Audio context init failed:",s)}},st=async()=>{try{G&&G.state==="suspended"&&(await G.resume(),console.log("🔊 [TTS] Audio context resumed")),v&&(v.currentTime=0,await v.play(),console.log("🔇 [TTS] Silent audio playing for background session")),M||(M=setInterval(()=>{g&&g.speaking?G&&G.state==="suspended"&&G.resume().catch(()=>{}):M&&(clearInterval(M),M=null)},5e3))}catch(s){console.warn("⚠️ [TTS] Silent audio start failed:",s)}},at=()=>{try{v&&(v.pause(),v.currentTime=0,console.log("🔇 [TTS] Silent audio stopped")),M&&(clearInterval(M),M=null)}catch(s){console.warn("⚠️ [TTS] Silent audio stop failed:",s)}},Nr=()=>new Promise((s,e)=>{if(!g)return e(new Error("Speech synthesis not initialized."));if(V=g.getVoices(),V.length>0){s();return}g.onvoiceschanged=()=>{V=g.getVoices(),s()},setTimeout(()=>{V.length===0&&(V=g.getVoices()),s()},1e3)}),ne=()=>{g&&g.speaking&&g.cancel(),J="","mediaSession"in navigator&&navigator.mediaSession.playbackState!=="none"&&(navigator.mediaSession.playbackState="paused"),rt(),at(),window.dispatchEvent(new CustomEvent("otakon:ttsStopped"))},ot=()=>{g&&g.speaking&&!g.paused&&(g.pause(),"mediaSession"in navigator&&(navigator.mediaSession.playbackState="paused"),window.dispatchEvent(new CustomEvent("otakon:ttsPaused")))},nt=()=>{g&&g.paused&&(g.resume(),"mediaSession"in navigator&&(navigator.mediaSession.playbackState="playing"),window.dispatchEvent(new CustomEvent("otakon:ttsResumed")))},Rr=async()=>{J&&(ne(),await ct(J))},kr=()=>g?g.speaking:!1,Pr=()=>{ne(),window.dispatchEvent(new CustomEvent("otakon:disableHandsFree"))},Ur=()=>{if("mediaSession"in navigator){console.log("🎵 [TTS] Setting up media session");try{navigator.mediaSession.setActionHandler("play",()=>{console.log("🎵 [TTS] Media session play action"),g&&g.paused&&nt()}),navigator.mediaSession.setActionHandler("pause",()=>{console.log("🎵 [TTS] Media session pause action"),g&&g.speaking&&!g.paused&&ot()}),navigator.mediaSession.setActionHandler("stop",()=>{console.log("🎵 [TTS] Media session stop action"),Pr()}),console.log("✅ [TTS] Media session handlers registered")}catch(s){console.warn("⚠️ [TTS] Failed to set media session handlers:",s)}}else console.warn("⚠️ [TTS] Media session API not available")},Gr=async()=>{document.hidden?(console.log("📱 [TTS] App went to background, isSpeaking:",g==null?void 0:g.speaking),g&&g.speaking&&(await st(),g.paused||setTimeout(()=>{g&&g.speaking&&!g.paused&&console.log("📱 [TTS] Nudging speech synthesis to stay alive")},100))):(console.log("📱 [TTS] App came to foreground, isSpeaking:",g==null?void 0:g.speaking),g&&g.speaking&&await Ge())},Dr=async()=>{if(typeof window<"u"&&"speechSynthesis"in window){if(Ke)return;Ke=!0,g=window.speechSynthesis,await Nr(),Ur(),Ir(),document.addEventListener("visibilitychange",Gr),g.getVoices().length===0&&g.speak(new SpeechSynthesisUtterance(""))}},it=()=>V.filter(s=>s.lang.startsWith("en-")),ct=async s=>new Promise((e,t)=>{try{if(!g)return console.error("Text-to-Speech is not available on this browser."),t(new Error("Text-to-Speech is not available on this browser."));if(!s.trim())return e();ne(),J=s;const r=new SpeechSynthesisUtterance(s),a=localStorage.getItem(Cr);r.rate=a?parseFloat(a):.94;const o=localStorage.getItem("otakonPreferredVoiceURI"),n=it();let i;if(o&&(i=n.find(c=>c.voiceURI===o)),!i&&n.length>0){const c=n.find(u=>u.name.toLowerCase().includes("female"));c?i=c:i=n[0]}i&&(r.voice=i),r.onstart=async()=>{if(console.log("🎤 [TTS] Speech started"),J=s,await Ge(),await st(),"serviceWorker"in navigator&&navigator.serviceWorker.controller&&navigator.serviceWorker.controller.postMessage({type:"TTS_STARTED"}),"mediaSession"in navigator)try{navigator.mediaSession.playbackState="playing";const c=s.replace(/[*_~`#]/g,"").replace(/\n+/g," ").trim(),u=c.length>100?c.substring(0,97)+"...":c;navigator.mediaSession.metadata=new MediaMetadata({title:u,artist:"Otagon AI",album:"Gaming Companion",artwork:[{src:"/icon-192.png",sizes:"192x192",type:"image/png"},{src:"/icon-512.png",sizes:"512x512",type:"image/png"}]}),console.log("🎵 [TTS] Media session metadata set")}catch(c){console.warn("⚠️ [TTS] Failed to set media session:",c)}window.dispatchEvent(new CustomEvent("otakon:ttsStarted"))},r.onend=()=>{J="","mediaSession"in navigator&&(navigator.mediaSession.playbackState="paused"),"serviceWorker"in navigator&&navigator.serviceWorker.controller&&navigator.serviceWorker.controller.postMessage({type:"TTS_STOPPED"}),rt(),at(),window.dispatchEvent(new CustomEvent("otakon:ttsStopped")),e()},r.onerror=c=>{console.error("SpeechSynthesis Utterance Error",c),ne(),t(c)},g.speak(r)}catch(r){console.error("TTS Error:",r),t(r)}}),Es={init:Dr,getAvailableVoices:it,speak:ct,cancel:ne,pause:ot,resume:nt,restart:Rr,isSpeaking:kr};class Mr{constructor(){E(this,"submittedFeedback",new Set)}hasSubmittedFeedback(e){return this.submittedFeedback.has(e)}async submitFeedback(e){try{const t=ce.getCurrentUser();if(!(t!=null&&t.authUserId))return console.warn("[FeedbackService] User not authenticated"),{success:!1,error:"User not authenticated"};if(this.submittedFeedback.has(e.messageId))return console.log("[FeedbackService] Feedback already submitted for message:",e.messageId),{success:!0};const{error:r}=await l.from("ai_feedback").insert({user_id:t.authUserId,conversation_id:e.conversationId,message_id:e.messageId,feedback_type:e.feedbackType,content_type:e.contentType,category:e.category||null,comment:e.comment||null});return r?r.code==="23505"?(console.log("[FeedbackService] Feedback already exists in database"),this.submittedFeedback.add(e.messageId),{success:!0}):(console.error("[FeedbackService] Failed to submit feedback:",r),{success:!1,error:r.message}):(this.submittedFeedback.add(e.messageId),console.log("[FeedbackService] Feedback submitted:",{messageId:e.messageId,type:e.feedbackType,contentType:e.contentType}),{success:!0})}catch(t){return console.error("[FeedbackService] Error submitting feedback:",t),{success:!1,error:"Failed to submit feedback"}}}async submitPositiveFeedback(e,t,r="message"){return this.submitFeedback({messageId:e,conversationId:t,feedbackType:"up",contentType:r})}async submitNegativeFeedback(e,t,r,a,o="message"){return this.submitFeedback({messageId:e,conversationId:t,feedbackType:"down",contentType:o,category:r,comment:a})}async submitCorrection(e){try{const t=ce.getCurrentUser();if(!(t!=null&&t.authUserId))return console.warn("[FeedbackService] User not authenticated for correction"),{success:!1,error:"User not authenticated"};if(!W.getRateLimitStatus().allowed)return{success:!1,error:"Daily correction limit reached. Try again tomorrow.",rateLimitRemaining:0};const a={originalResponse:e.originalResponse,correctionText:e.correctionText,type:e.correctionType,scope:e.correctionScope,gameTitle:e.gameTitle,messageId:e.messageId,conversationId:e.conversationId},o=await W.submitCorrection(t.authUserId,a);return o.success?(console.log("[FeedbackService] Correction submitted successfully:",{messageId:e.messageId,type:e.correctionType,scope:e.correctionScope}),{success:!0,correction:o.correction,rateLimitRemaining:W.getRateLimitStatus().remaining}):{success:!1,error:o.error,rateLimitRemaining:W.getRateLimitStatus().remaining}}catch(t){return console.error("[FeedbackService] Error submitting correction:",t),{success:!1,error:"Failed to submit correction"}}}getCorrectionRateLimit(){return W.getRateLimitStatus()}async getUserCorrections(){const e=ce.getCurrentUser();return e!=null&&e.authUserId?W.getAllCorrections(e.authUserId):[]}async toggleCorrection(e,t){const r=ce.getCurrentUser();return r!=null&&r.authUserId?W.toggleCorrection(r.authUserId,e,t):!1}async getFeedbackStats(){try{const{data:e,error:t}=await l.from("ai_feedback").select("feedback_type, category");if(t)return console.error("[FeedbackService] Failed to get feedback stats:",t),null;const r={totalFeedback:e.length,positiveCount:e.filter(a=>a.feedback_type==="up").length,negativeCount:e.filter(a=>a.feedback_type==="down").length,categoryBreakdown:{}};return e.forEach(a=>{a.category&&(r.categoryBreakdown[a.category]=(r.categoryBreakdown[a.category]||0)+1)}),r}catch(e){return console.error("[FeedbackService] Error getting feedback stats:",e),null}}clearLocalTracking(){this.submittedFeedback.clear()}}const Lr=new Mr,Ts=Object.freeze(Object.defineProperty({__proto__:null,feedbackService:Lr},Symbol.toStringTag,{value:"Module"}));function xr(s){var n;const e=s.split(","),t=((n=e[0].match(/:(.*?);/))==null?void 0:n[1])||"image/png",r=atob(e[1]),a=r.length,o=new Uint8Array(a);for(let i=0;i<a;i++)o[i]=r.charCodeAt(i);return new Blob([o],{type:t})}async function Fr(s,e){try{const t=xr(s),r=t.size,a=50*1024*1024;if(r>a)return{success:!1,error:"Screenshot exceeds 50MB size limit"};const o=Date.now(),n=Math.random().toString(36).substring(2,15),i=`${o}_${n}.png`,c=`${e}/${i}`,{error:u}=await l.storage.from("screenshots").upload(c,t,{contentType:"image/png",cacheControl:"3600",upsert:!1});if(u)return console.error("Screenshot upload error:",u),{success:!1,error:u.message};const{data:f}=l.storage.from("screenshots").getPublicUrl(c);return f!=null&&f.publicUrl?{success:!0,publicUrl:f.publicUrl,fileSize:r}:{success:!1,error:"Failed to generate public URL"}}catch(t){return console.error("Screenshot upload exception:",t),{success:!1,error:t instanceof Error?t.message:"Unknown error"}}}const ws=Object.freeze(Object.defineProperty({__proto__:null,uploadScreenshot:Fr},Symbol.toStringTag,{value:"Module"})),ue=new Map;class $r{constructor(){E(this,"MAX_WORDS",300);E(this,"RECENT_MESSAGE_COUNT",8)}countWords(e){return e.trim().split(/\s+/).filter(t=>t.length>0).length}getTotalWordCount(e){return e.reduce((t,r)=>{const a=this.countWords(r.content);return t+a},0)}shouldSummarize(e){return!e.messages||e.messages.length<=this.RECENT_MESSAGE_COUNT?!1:this.getTotalWordCount(e.messages)>this.MAX_WORDS*3}splitMessages(e){if(e.length<=this.RECENT_MESSAGE_COUNT)return{toSummarize:[],toKeep:e};const t=e.length-this.RECENT_MESSAGE_COUNT;return{toSummarize:e.slice(0,t),toKeep:e.slice(t)}}async summarizeMessages(e,t,r){const a=this.getTotalWordCount(e),o=e.map(c=>`${c.role==="user"?"User":"Assistant"}: ${c.content}`).join(`
...
Provide ONLY the summary, no additional commentary.`;try{const c={id:"temp-summary",title:"Summary Request",messages:[{id:"summary-msg-"+Date.now(),role:"user",content:i,timestamp:Date.now()}],createdAt:Date.now(),updatedAt:Date.now(),isActive:!1,isGameHub:!1};console.log(`📡 [GEMINI CALL #2] 📋 Context Summarization | Messages: ${e.length} | Game: ${t||"N/A"} | Target: ${this.MAX_WORDS} words`);const u={id:"system",email:"system@otakon.ai",profileData:null},T=(await yt.getChatResponse(c,u,i,!1,!1,void 0,void 0,"summarization")).content.trim(),p=this.countWords(T);return console.log(`✅ [GEMINI CALL] Context Summarization SUCCESS | ${p} words (reduced from ${a})`),re.success(`Conversation summarized (${a} → ${p} words)`),ue.delete("summarization"),{summary:T,wordCount:p,messagesIncluded:e.length,originalWordCount:a}}catch(c){console.error("❌ [ContextSummarization] Failed to generate summary:",c);const u=ue.get("summarization")||0,f=3;if(u<f)return ue.set("summarization",u+1),console.log(`🔄 [ContextSummarization] Retry ${u+1}/${f} in 1 second...`),await new Promise(p=>setTimeout(p,1e3)),this.summarizeMessages(e,t,r);console.error("❌ [ContextSummarization] Max retries reached"),ue.delete("summarization");const T=e.slice(0,5).map(p=>p.content.substring(0,100)).join(" ... ").substring(0,this.MAX_WORDS*6);return{summary:`[Previous conversation context] ${T}`,wordCount:this.countWords(T),messagesIncluded:e.length,originalWordCount:a}}}async applyContextSummarization(e){if(!this.shouldSummarize(e))return e;const{toSummarize:t,toKeep:r}=this.splitMessages(e.messages);if(t.length===0)return e;const a=await this.summarizeMessages(t,e.gameTitle,e.genre),n=[{id:"summary-"+Date.now(),role:"system",content:a.summary,timestamp:t[t.length-1].timestamp,metadata:{isSummary:!0,messagesIncluded:a.messagesIncluded,originalWordCount:a.originalWordCount,summaryWordCount:a.wordCount}},...r],i=a.summary.replace(/!\[.*?\]\(data:image\/.*?\)/g,""),c=i.split(/\s+/).filter(f=>f.length>0),u=c.length>500?c.slice(0,500).join(" ")+"...":i;return console.log(`✅ [ContextSummarization] Context optimized: ${e.messages.length} messages → ${n.length} (${a.originalWordCount} words → ${a.wordCount} + recent)`),{...e,messages:n,contextSummary:u,lastSummarizedAt:Date.now(),updatedAt:Date.now()}}async getOptimizedContext(e){return this.shouldSummarize(e)?(await this.applyContextSummarization(e)).messages:e.messages}willTriggerSummarization(e){return this.getTotalWordCount(e.messages)>this.MAX_WORDS*3*.8}}const Wr=new $r,_s=Object.freeze(Object.defineProperty({__proto__:null,contextSummarizationService:Wr},Symbol.toStringTag,{value:"Module"}));export{ps as A,Xr as B,Vr as C,jr as D,le as E,Ts as F,ws as G,_s as H,Ss as U,Zr as W,ss as a,W as b,ds as c,k as d,us as e,cs as f,ls as g,gs as h,X as i,Es as j,hs as k,bs as l,os as m,ns as n,as as o,rs as p,is as q,fs as r,Jr as s,re as t,ys as u,es as v,ts as w,Qr as x,Ct as y,ms as z};
