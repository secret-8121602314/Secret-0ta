...
`),e.join(`
`)}function Wt(s,e){const t=[],r=(e==null?void 0:e.messageCount)||s.messages.length,a=s.gameProgress||0,o=(e==null?void 0:e.subtabsFilled)||0,n=(e==null?void 0:e.subtabsTotal)||0;if(s.isGameHub||r<3)return"";if(r>=20?t.push(`
...
`)}return t.join(`
`)}function Kt(s){if(!s||s.scope==="off")return"";const e=[];if(s.previousTopics.length>0&&e.push(`
...
`)}
`)}return e.join(`
`)}async function os(s,e){try{const t=await R.getAIPreferences(s);if(t.responseHistoryScope==="off")return{previousTopics:[],corrections:[],scope:"off"};const[r,a]=await Promise.all([R.getResponseTopics(s,e,t.responseHistoryScope),R.getActiveCorrections(s,e,!0)]);return{previousTopics:r,corrections:a,scope:t.responseHistoryScope}}catch(t){return console.error("[PromptSystem] Error fetching behavior context:",t),null}}const Fe=3e3,Bt=2e4,be=["story_so_far","quest_log","characters","boss_strategy","tips","hidden_secrets","points_of_interest"],He=`
...
`,qt=async(s,e,t,r,a)=>{var h;let o=0;const n=[],i=2500,u=[...s.subtabs||[]].filter(d=>d.status==="loaded"&&d.content).sort((d,m)=>{const y=be.indexOf(d.id),_=be.indexOf(m.id);return(y===-1?999:y)-(_===-1?999:_)}).map(d=>{const m=d.content||"";m.length>i&&m.includes("<details>")&&n.push(d.id);const y=m.length>Fe?m.slice(0,Fe)+"...[truncated]":m,_=`### ${d.title} (ID: ${d.id})
${y}`;if(o+_.length>Bt){const N=be.indexOf(d.id);if(N!==-1&&N<3){const C=`### ${d.title} (ID: ${d.id}) [SUMMARY]
...
This keeps subtabs useful without losing important context.`:"",E=s.messages.slice(-10).map(d=>`${d.role==="user"?"User":"Otagon"}: ${d.content}`).join(`
`),p=s.contextSummary?`**Historical Context (Previous Sessions):**
${s.contextSummary}

`:"",A=a||J.getDefaultProfile(),q=J.buildProfileContext(A);let G="";if(s.gameTitle){const d=ut.getByGameTitle(s.gameTitle);if(d!=null&&d.igdbGameId)try{const m=await dt.getForContext(d.igdbGameId);m&&(G=`
...
`},Yt=(s,e,t,r)=>{const a=r||J.getDefaultProfile(),o=J.buildProfileContext(a),n=s.messages.slice(-10).map(i=>`${i.role==="user"?"User":"Otagon"}: ${i.content}`).join(`
`);return`
...
**Tag Definitions:**
${ye}
`},Vt=(s,e,t,r)=>{const a=r||J.getDefaultProfile(),o=J.buildProfileContext(a),n=s.gameTitle?`
...
**Tag Definitions:**
${ye}
`},ns=async(s,e,t,r,a,o,n,i,c)=>{const u=n?Kt(n):"",f=i?`
**User Timezone:** ${i}
...
*Content loading...*`}}const cs=new Jt,Xt=`You are a correction validator for OTAKON, an AI gaming companion.
...
Example: @story_so_far The player defeated the first boss
    `.trim()}validateCommand(e){switch(e.type){case"update":if(!e.instruction)return{valid:!1,error:"Update command requires content. Example: @story_so_far The player..."};break;case"modify":if(!e.instruction)return{valid:!1,error:"Modify command requires instructions. Example: @tips \\modify Change to combat strategies"};break}return{valid:!0}}describeCommand(e){switch(e.type){case"update":return`Updating "${e.tabName}" with: ${e.instruction}`;case"modify":return`Modifying "${e.tabName}": ${e.instruction}`;case"delete":return`Deleting "${e.tabName}"`}}}const fs=new _r;let g,Y=[],We=!1,j="",D=null,U=null,O=null,M=null;const Ar="otakonSpeechRate",Ge=async()=>{try{const s=navigator;if(s.wakeLock){if(D!==null)try{await D.release()}catch{}D=await s.wakeLock.request("screen"),console.log("🔒 [TTS] Wake lock acquired - screen will stay on"),D.addEventListener("release",async()=>{console.log("🔓 [TTS] Wake lock released"),D=null,g&&g.speaking&&(console.log("🔒 [TTS] Reacquiring wake lock (TTS still active)"),await Ge())})}else console.warn("⚠️ [TTS] Wake lock API not available")}catch(s){console.warn("⚠️ [TTS] Wake lock request failed:",s)}},tt=async()=>{try{D!==null&&(await D.release(),D=null)}catch{}},Or=()=>{try{if(!U){const s=window,e=s.AudioContext||s.webkitAudioContext;e&&(U=new e,console.log("🔊 [TTS] Audio context initialized"))}O||(O=new Audio,O.src="data:audio/wav;base64,UklGRnoGAABXQVZFZm10IBAAAAABAAEAQB8AAEAfAAABAAgAZGF0YQoGAACBhYqFbF1fdJivrJBhNjVgodDbq2EcBj+a2teleShtr9teleShtr9teleShtr9teleShtr9t",O.loop=!0,O.volume=.01,O.load(),console.log("🔇 [TTS] Silent audio initialized for background playback"))}catch(s){console.warn("⚠️ [TTS] Audio context init failed:",s)}},rt=async()=>{try{U&&U.state==="suspended"&&(await U.resume(),console.log("🔊 [TTS] Audio context resumed")),O&&(O.currentTime=0,await O.play(),console.log("🔇 [TTS] Silent audio playing for background session")),M||(M=setInterval(()=>{g&&g.speaking?U&&U.state==="suspended"&&U.resume().catch(()=>{}):M&&(clearInterval(M),M=null)},5e3))}catch(s){console.warn("⚠️ [TTS] Silent audio start failed:",s)}},st=()=>{try{O&&(O.pause(),O.currentTime=0,console.log("🔇 [TTS] Silent audio stopped")),M&&(clearInterval(M),M=null)}catch(s){console.warn("⚠️ [TTS] Silent audio stop failed:",s)}},vr=()=>new Promise((s,e)=>{if(!g)return e(new Error("Speech synthesis not initialized."));if(Y=g.getVoices(),Y.length>0){s();return}g.onvoiceschanged=()=>{Y=g.getVoices(),s()},setTimeout(()=>{Y.length===0&&(Y=g.getVoices()),s()},1e3)}),ae=()=>{g&&g.speaking&&g.cancel(),j="","mediaSession"in navigator&&navigator.mediaSession.playbackState!=="none"&&(navigator.mediaSession.playbackState="paused"),tt(),st(),window.dispatchEvent(new CustomEvent("otakon:ttsStopped"))},at=()=>{g&&g.speaking&&!g.paused&&(g.pause(),"mediaSession"in navigator&&(navigator.mediaSession.playbackState="paused"),window.dispatchEvent(new CustomEvent("otakon:ttsPaused")))},ot=()=>{g&&g.paused&&(g.resume(),"mediaSession"in navigator&&(navigator.mediaSession.playbackState="playing"),window.dispatchEvent(new CustomEvent("otakon:ttsResumed")))},Cr=async()=>{j&&(ae(),await it(j))},Ir=()=>g?g.speaking:!1,Nr=()=>{ae(),window.dispatchEvent(new CustomEvent("otakon:disableHandsFree"))},kr=()=>{if("mediaSession"in navigator){console.log("🎵 [TTS] Setting up media session");try{navigator.mediaSession.setActionHandler("play",()=>{console.log("🎵 [TTS] Media session play action"),g&&g.paused&&ot()}),navigator.mediaSession.setActionHandler("pause",()=>{console.log("🎵 [TTS] Media session pause action"),g&&g.speaking&&!g.paused&&at()}),navigator.mediaSession.setActionHandler("stop",()=>{console.log("🎵 [TTS] Media session stop action"),Nr()}),console.log("✅ [TTS] Media session handlers registered")}catch(s){console.warn("⚠️ [TTS] Failed to set media session handlers:",s)}}else console.warn("⚠️ [TTS] Media session API not available")},Rr=async()=>{document.hidden?(console.log("📱 [TTS] App went to background, isSpeaking:",g==null?void 0:g.speaking),g&&g.speaking&&(await rt(),g.paused||setTimeout(()=>{g&&g.speaking&&!g.paused&&console.log("📱 [TTS] Nudging speech synthesis to stay alive")},100))):(console.log("📱 [TTS] App came to foreground, isSpeaking:",g==null?void 0:g.speaking),g&&g.speaking&&await Ge())},Pr=async()=>{if(typeof window<"u"&&"speechSynthesis"in window){if(We)return;We=!0,g=window.speechSynthesis,await vr(),kr(),Or(),document.addEventListener("visibilitychange",Rr),g.getVoices().length===0&&g.speak(new SpeechSynthesisUtterance(""))}},nt=()=>Y.filter(s=>s.lang.startsWith("en-")),it=async s=>new Promise((e,t)=>{try{if(!g)return console.error("Text-to-Speech is not available on this browser."),t(new Error("Text-to-Speech is not available on this browser."));if(!s.trim())return e();ae(),j=s;const r=new SpeechSynthesisUtterance(s),a=localStorage.getItem(Ar);r.rate=a?parseFloat(a):.94;const o=localStorage.getItem("otakonPreferredVoiceURI"),n=nt();let i;if(o&&(i=n.find(c=>c.voiceURI===o)),!i&&n.length>0){const c=n.find(u=>u.name.toLowerCase().includes("female"));c?i=c:i=n[0]}i&&(r.voice=i),r.onstart=async()=>{if(console.log("🎤 [TTS] Speech started"),j=s,await Ge(),await rt(),"serviceWorker"in navigator&&navigator.serviceWorker.controller&&navigator.serviceWorker.controller.postMessage({type:"TTS_STARTED"}),"mediaSession"in navigator)try{navigator.mediaSession.playbackState="playing";const c=s.replace(/[*_~`#]/g,"").replace(/\n+/g," ").trim(),u=c.length>100?c.substring(0,97)+"...":c;navigator.mediaSession.metadata=new MediaMetadata({title:u,artist:"Otagon AI",album:"Gaming Companion",artwork:[{src:"/icon-192.png",sizes:"192x192",type:"image/png"},{src:"/icon-512.png",sizes:"512x512",type:"image/png"}]}),console.log("🎵 [TTS] Media session metadata set")}catch(c){console.warn("⚠️ [TTS] Failed to set media session:",c)}window.dispatchEvent(new CustomEvent("otakon:ttsStarted"))},r.onend=()=>{j="","mediaSession"in navigator&&(navigator.mediaSession.playbackState="paused"),"serviceWorker"in navigator&&navigator.serviceWorker.controller&&navigator.serviceWorker.controller.postMessage({type:"TTS_STOPPED"}),tt(),st(),window.dispatchEvent(new CustomEvent("otakon:ttsStopped")),e()},r.onerror=c=>{console.error("SpeechSynthesis Utterance Error",c),ae(),t(c)},g.speak(r)}catch(r){console.error("TTS Error:",r),t(r)}}),ys={init:Pr,getAvailableVoices:nt,speak:it,cancel:ae,pause:at,resume:ot,restart:Cr,isSpeaking:Ir};class Gr{constructor(){T(this,"submittedFeedback",new Set)}hasSubmittedFeedback(e){return this.submittedFeedback.has(e)}async submitFeedback(e){try{const t=ne.getCurrentUser();if(!(t!=null&&t.authUserId))return console.warn("[FeedbackService] User not authenticated"),{success:!1,error:"User not authenticated"};if(this.submittedFeedback.has(e.messageId))return console.log("[FeedbackService] Feedback already submitted for message:",e.messageId),{success:!0};const{error:r}=await l.from("ai_feedback").insert({user_id:t.authUserId,conversation_id:e.conversationId,message_id:e.messageId,feedback_type:e.feedbackType,content_type:e.contentType,category:e.category||null,comment:e.comment||null});return r?r.code==="23505"?(console.log("[FeedbackService] Feedback already exists in database"),this.submittedFeedback.add(e.messageId),{success:!0}):(console.error("[FeedbackService] Failed to submit feedback:",r),{success:!1,error:r.message}):(this.submittedFeedback.add(e.messageId),console.log("[FeedbackService] Feedback submitted:",{messageId:e.messageId,type:e.feedbackType,contentType:e.contentType}),{success:!0})}catch(t){return console.error("[FeedbackService] Error submitting feedback:",t),{success:!1,error:"Failed to submit feedback"}}}async submitPositiveFeedback(e,t,r="message"){return this.submitFeedback({messageId:e,conversationId:t,feedbackType:"up",contentType:r})}async submitNegativeFeedback(e,t,r,a,o="message"){return this.submitFeedback({messageId:e,conversationId:t,feedbackType:"down",contentType:o,category:r,comment:a})}async submitCorrection(e){try{const t=ne.getCurrentUser();if(!(t!=null&&t.authUserId))return console.warn("[FeedbackService] User not authenticated for correction"),{success:!1,error:"User not authenticated"};if(!$.getRateLimitStatus().allowed)return{success:!1,error:"Daily correction limit reached. Try again tomorrow.",rateLimitRemaining:0};const a={originalResponse:e.originalResponse,correctionText:e.correctionText,type:e.correctionType,scope:e.correctionScope,gameTitle:e.gameTitle,messageId:e.messageId,conversationId:e.conversationId},o=await $.submitCorrection(t.authUserId,a);return o.success?(console.log("[FeedbackService] Correction submitted successfully:",{messageId:e.messageId,type:e.correctionType,scope:e.correctionScope}),{success:!0,correction:o.correction,rateLimitRemaining:$.getRateLimitStatus().remaining}):{success:!1,error:o.error,rateLimitRemaining:$.getRateLimitStatus().remaining}}catch(t){return console.error("[FeedbackService] Error submitting correction:",t),{success:!1,error:"Failed to submit correction"}}}getCorrectionRateLimit(){return $.getRateLimitStatus()}async getUserCorrections(){const e=ne.getCurrentUser();return e!=null&&e.authUserId?$.getAllCorrections(e.authUserId):[]}async toggleCorrection(e,t){const r=ne.getCurrentUser();return r!=null&&r.authUserId?$.toggleCorrection(r.authUserId,e,t):!1}async getFeedbackStats(){try{const{data:e,error:t}=await l.from("ai_feedback").select("feedback_type, category");if(t)return console.error("[FeedbackService] Failed to get feedback stats:",t),null;const r={totalFeedback:e.length,positiveCount:e.filter(a=>a.feedback_type==="up").length,negativeCount:e.filter(a=>a.feedback_type==="down").length,categoryBreakdown:{}};return e.forEach(a=>{a.category&&(r.categoryBreakdown[a.category]=(r.categoryBreakdown[a.category]||0)+1)}),r}catch(e){return console.error("[FeedbackService] Error getting feedback stats:",e),null}}clearLocalTracking(){this.submittedFeedback.clear()}}const Ur=new Gr,Ss=Object.freeze(Object.defineProperty({__proto__:null,feedbackService:Ur},Symbol.toStringTag,{value:"Module"}));function Dr(s){var n;const e=s.split(","),t=((n=e[0].match(/:(.*?);/))==null?void 0:n[1])||"image/png",r=atob(e[1]),a=r.length,o=new Uint8Array(a);for(let i=0;i<a;i++)o[i]=r.charCodeAt(i);return new Blob([o],{type:t})}async function Mr(s,e){try{const t=Dr(s),r=t.size,a=50*1024*1024;if(r>a)return{success:!1,error:"Screenshot exceeds 50MB size limit"};const o=Date.now(),n=Math.random().toString(36).substring(2,15),i=`${o}_${n}.png`,c=`${e}/${i}`,{error:u}=await l.storage.from("screenshots").upload(c,t,{contentType:"image/png",cacheControl:"3600",upsert:!1});if(u)return console.error("Screenshot upload error:",u),{success:!1,error:u.message};const{data:f}=l.storage.from("screenshots").getPublicUrl(c);return f!=null&&f.publicUrl?{success:!0,publicUrl:f.publicUrl,fileSize:r}:{success:!1,error:"Failed to generate public URL"}}catch(t){return console.error("Screenshot upload exception:",t),{success:!1,error:t instanceof Error?t.message:"Unknown error"}}}const bs=Object.freeze(Object.defineProperty({__proto__:null,uploadScreenshot:Mr},Symbol.toStringTag,{value:"Module"})),ue=new Map;class Lr{constructor(){T(this,"MAX_WORDS",300);T(this,"RECENT_MESSAGE_COUNT",8)}countWords(e){return e.trim().split(/\s+/).filter(t=>t.length>0).length}getTotalWordCount(e){return e.reduce((t,r)=>{const a=this.countWords(r.content);return t+a},0)}shouldSummarize(e){return!e.messages||e.messages.length<=this.RECENT_MESSAGE_COUNT?!1:this.getTotalWordCount(e.messages)>this.MAX_WORDS*3}splitMessages(e){if(e.length<=this.RECENT_MESSAGE_COUNT)return{toSummarize:[],toKeep:e};const t=e.length-this.RECENT_MESSAGE_COUNT;return{toSummarize:e.slice(0,t),toKeep:e.slice(t)}}async summarizeMessages(e,t,r){const a=this.getTotalWordCount(e),o=e.map(c=>`${c.role==="user"?"User":"Assistant"}: ${c.content}`).join(`
...
Provide ONLY the summary, no additional commentary.`;try{const c={id:"temp-summary",title:"Summary Request",messages:[{id:"summary-msg-"+Date.now(),role:"user",content:i,timestamp:Date.now()}],createdAt:Date.now(),updatedAt:Date.now(),isActive:!1,isGameHub:!1};console.log(`📡 [GEMINI CALL #2] 📋 Context Summarization | Messages: ${e.length} | Game: ${t||"N/A"} | Target: ${this.MAX_WORDS} words`);const u={id:"system",email:"system@otakon.ai",profileData:null},E=(await ft.getChatResponse(c,u,i,!1,!1,void 0,void 0,"summarization")).content.trim(),p=this.countWords(E);return console.log(`✅ [GEMINI CALL] Context Summarization SUCCESS | ${p} words (reduced from ${a})`),ee.success(`Conversation summarized (${a} → ${p} words)`),ue.delete("summarization"),{summary:E,wordCount:p,messagesIncluded:e.length,originalWordCount:a}}catch(c){console.error("❌ [ContextSummarization] Failed to generate summary:",c);const u=ue.get("summarization")||0,f=3;if(u<f)return ue.set("summarization",u+1),console.log(`🔄 [ContextSummarization] Retry ${u+1}/${f} in 1 second...`),await new Promise(p=>setTimeout(p,1e3)),this.summarizeMessages(e,t,r);console.error("❌ [ContextSummarization] Max retries reached"),ue.delete("summarization");const E=e.slice(0,5).map(p=>p.content.substring(0,100)).join(" ... ").substring(0,this.MAX_WORDS*6);return{summary:`[Previous conversation context] ${E}`,wordCount:this.countWords(E),messagesIncluded:e.length,originalWordCount:a}}}async applyContextSummarization(e){if(!this.shouldSummarize(e))return e;const{toSummarize:t,toKeep:r}=this.splitMessages(e.messages);if(t.length===0)return e;const a=await this.summarizeMessages(t,e.gameTitle,e.genre),n=[{id:"summary-"+Date.now(),role:"system",content:a.summary,timestamp:t[t.length-1].timestamp,metadata:{isSummary:!0,messagesIncluded:a.messagesIncluded,originalWordCount:a.originalWordCount,summaryWordCount:a.wordCount}},...r],i=a.summary.replace(/!\[.*?\]\(data:image\/.*?\)/g,""),c=i.split(/\s+/).filter(f=>f.length>0),u=c.length>500?c.slice(0,500).join(" ")+"...":i;return console.log(`✅ [ContextSummarization] Context optimized: ${e.messages.length} messages → ${n.length} (${a.originalWordCount} words → ${a.wordCount} + recent)`),{...e,messages:n,contextSummary:u,lastSummarizedAt:Date.now(),updatedAt:Date.now()}}async getOptimizedContext(e){return this.shouldSummarize(e)?(await this.applyContextSummarization(e)).messages:e.messages}willTriggerSummarization(e){return this.getTotalWordCount(e.messages)>this.MAX_WORDS*3*.8}}const xr=new Lr,Ts=Object.freeze(Object.defineProperty({__proto__:null,contextSummarizationService:xr},Symbol.toStringTag,{value:"Module"}));export{gs as A,Vr as B,zr as C,qr as D,ce as E,Ss as F,bs as G,Ts as H,ps as U,Jr as W,es as a,$ as b,cs as c,R as d,is as e,os as f,ns as g,ls as h,J as i,ys as j,ds as k,fs as l,rs as m,ss as n,ts as o,Qr as p,as as q,ms as r,Yr as s,ee as t,hs as u,Xr as v,Zr as w,jr as x,At as y,us as z};
