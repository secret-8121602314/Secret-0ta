...
`,De=(i,e,t,s,r)=>{var h,p;const a=((h=i.subtabs)==null?void 0:h.filter(y=>y.status==="loaded"&&y.content).map(y=>`### ${y.title} (ID: ${y.id})
${y.content}`).join(`

`))||"No subtabs available yet.",n=i.messages.slice(-10).map(y=>`${y.role==="user"?"User":"Otagon"}: ${y.content}`).join(`
`),c=i.contextSummary?`**Historical Context (Previous Sessions):**
${i.contextSummary}

`:"",o=r||$.getDefaultProfile(),d=$.buildProfileContext(o);return`
...
`},Le=(i,e,t,s)=>{const r=s||$.getDefaultProfile();return`
...
Example: @story_so_far The player defeated the first boss
    `.trim()}validateCommand(e){switch(e.type){case"update":if(!e.instruction)return{valid:!1,error:"Update command requires content. Example: @story_so_far The player..."};break;case"modify":if(!e.instruction)return{valid:!1,error:"Modify command requires instructions. Example: @tips \\modify Change to combat strategies"};break}return{valid:!0}}describeCommand(e){switch(e.type){case"update":return`Updating "${e.tabName}" with: ${e.instruction}`;case"modify":return`Modifying "${e.tabName}": ${e.instruction}`;case"delete":return`Deleting "${e.tabName}"`}}}const Ct=new ze;let m,G=[],ue=!1,H="",W=null,A=null,v=null,re=!1;const je="otakonSpeechRate",ne=async()=>{try{const i=navigator;i.wakeLock&&(W=await i.wakeLock.request("screen"),W.addEventListener("release",()=>{m&&m.speaking&&!re&&ne()}))}catch{}},ge=async()=>{try{W!==null&&(await W.release(),W=null)}catch{}},Be=()=>{try{if(!A){const i=window;A=new(i.AudioContext||i.webkitAudioContext)}v||(v=new Audio,v.src="data:audio/wav;base64,UklGRigAAABXQVZFZm10IBIAAAABAAEARKwAAIhYAQACABAAAABkYXRhAgAAAAEA",v.loop=!0,v.volume=.01,A&&A.createMediaElementSource(v).connect(A.destination))}catch{}},pe=async()=>{try{v&&A&&(A.state==="suspended"&&await A.resume(),await v.play())}catch{}},fe=()=>{try{v&&(v.pause(),v.currentTime=0)}catch{}},Ye=()=>new Promise((i,e)=>{if(!m)return e(new Error("Speech synthesis not initialized."));if(G=m.getVoices(),G.length>0){i();return}m.onvoiceschanged=()=>{G=m.getVoices(),i()},setTimeout(()=>{G.length===0&&(G=m.getVoices()),i()},1e3)}),q=()=>{m&&m.speaking&&m.cancel(),H="","mediaSession"in navigator&&navigator.mediaSession.playbackState!=="none"&&(navigator.mediaSession.playbackState="paused"),ge(),fe(),window.dispatchEvent(new CustomEvent("otakon:ttsStopped"))},Je=()=>{m&&m.speaking&&!m.paused&&(m.pause(),"mediaSession"in navigator&&(navigator.mediaSession.playbackState="paused"),window.dispatchEvent(new CustomEvent("otakon:ttsPaused")))},Ve=()=>{m&&m.paused&&(m.resume(),"mediaSession"in navigator&&(navigator.mediaSession.playbackState="playing"),window.dispatchEvent(new CustomEvent("otakon:ttsResumed")))},Xe=async()=>{H&&(q(),await be(H))},Ze=()=>m?m.speaking:!1,de=()=>{q(),window.dispatchEvent(new CustomEvent("otakon:disableHandsFree"))},Qe=()=>{"mediaSession"in navigator&&(navigator.mediaSession.setActionHandler("play",()=>{}),navigator.mediaSession.setActionHandler("pause",de),navigator.mediaSession.setActionHandler("stop",de))},et=async()=>{document.hidden?(re=!0,m&&m.speaking&&await pe()):(re=!1,m&&m.speaking&&await ne())},tt=async()=>{if(typeof window<"u"&&"speechSynthesis"in window){if(ue)return;ue=!0,m=window.speechSynthesis,await Ye(),Qe(),Be(),document.addEventListener("visibilitychange",et),m.getVoices().length===0&&m.speak(new SpeechSynthesisUtterance(""))}},ye=()=>G.filter(i=>i.lang.startsWith("en-")),be=async i=>new Promise((e,t)=>{try{if(!m)return console.error("Text-to-Speech is not available on this browser."),t(new Error("Text-to-Speech is not available on this browser."));if(!i.trim())return e();q(),H=i;const s=new SpeechSynthesisUtterance(i),r=localStorage.getItem(je);s.rate=r?parseFloat(r):.94;const a=localStorage.getItem("otakonPreferredVoiceURI"),n=ye();let c;if(a&&(c=n.find(o=>o.voiceURI===a)),!c&&n.length>0){const o=n.find(d=>d.name.toLowerCase().includes("female"));o?c=o:c=n[0]}c&&(s.voice=c),s.onstart=async()=>{await ne(),await pe(),"serviceWorker"in navigator&&navigator.serviceWorker.controller&&navigator.serviceWorker.controller.postMessage({type:"TTS_STARTED"}),"mediaSession"in navigator&&(navigator.mediaSession.playbackState="playing",navigator.mediaSession.metadata=new MediaMetadata({title:i.length>50?i.substring(0,50)+"...":i,artist:"Your AI Gaming Companion",album:"Otakon",artwork:[{src:"/icon-192.png",sizes:"192x192",type:"image/png"},{src:"/icon-512.png",sizes:"512x512",type:"image/png"}]})),window.dispatchEvent(new CustomEvent("otakon:ttsStarted"))},s.onend=()=>{H="","mediaSession"in navigator&&(navigator.mediaSession.playbackState="paused"),"serviceWorker"in navigator&&navigator.serviceWorker.controller&&navigator.serviceWorker.controller.postMessage({type:"TTS_STOPPED"}),ge(),fe(),window.dispatchEvent(new CustomEvent("otakon:ttsStopped")),e()},s.onerror=o=>{console.error("SpeechSynthesis Utterance Error",o),q(),t(o)},m.speak(s)}catch(s){console.error("TTS Error:",s),t(s)}}),_t={init:tt,getAvailableVoices:ye,speak:be,cancel:q,pause:Je,resume:Ve,restart:Xe,isSpeaking:Ze};class he{static async acquireMigrationLock(e,t){const s=[e,t].sort().join("|");return this.activeMigrations.has(s)?(console.warn("🔒 [MessageRouting] Migration already in progress:",s),!1):(this.activeMigrations.add(s),setTimeout(()=>{this.activeMigrations.delete(s)},this.LOCK_TIMEOUT),!0)}static releaseMigrationLock(e,t){const s=[e,t].sort().join("|");this.activeMigrations.delete(s)}static async migrateMessagesAtomic(e,t,s){if(!await this.acquireMigrationLock(t,s)){console.warn("⚠️ [MessageRouting] Skipping migration - another migration in progress");return}try{const a=await b.getConversations(!1),n=a[t],c=a[s];if(!n)throw console.error("📦 [MessageRouting] Source conversation not found:",t),console.error("📦 [MessageRouting] Available conversations:",Object.keys(a)),new Error(`Source conversation ${t} not found`);if(!c)throw console.error("📦 [MessageRouting] Destination conversation not found:",s),console.error("📦 [MessageRouting] Available conversations:",Object.keys(a)),new Error(`Destination conversation ${s} not found`);const o=n.messages.filter(p=>e.includes(p.id));if(o.length===0)return;const d=o.filter(p=>!c.messages.some(y=>y.id===p.id)),h={...a,[s]:{...c,messages:[...c.messages,...d],updatedAt:Date.now()},[t]:{...n,messages:n.messages.filter(p=>!e.includes(p.id)),updatedAt:Date.now()}};await b.setConversations(h)}finally{this.releaseMigrationLock(t,s)}}static shouldRouteMessage(e,t,s){return!t||e===t?!1:!!(s&&t)}static messageExists(e,t){return e.some(s=>s.id===t)}}u(he,"activeMigrations",new Set),u(he,"LOCK_TIMEOUT",1e4);class st{constructor(){u(this,"MAX_WORDS",300);u(this,"RECENT_MESSAGE_COUNT",8)}countWords(e){return e.trim().split(/\s+/).filter(t=>t.length>0).length}getTotalWordCount(e){return e.reduce((t,s)=>{const r=this.countWords(s.content);return t+r},0)}shouldSummarize(e){return!e.messages||e.messages.length<=this.RECENT_MESSAGE_COUNT?!1:this.getTotalWordCount(e.messages)>this.MAX_WORDS*3}splitMessages(e){if(e.length<=this.RECENT_MESSAGE_COUNT)return{toSummarize:[],toKeep:e};const t=e.length-this.RECENT_MESSAGE_COUNT;return{toSummarize:e.slice(0,t),toKeep:e.slice(t)}}async summarizeMessages(e,t,s){const r=this.getTotalWordCount(e),a=e.map(o=>`${o.role==="user"?"User":"Assistant"}: ${o.content}`).join(`
...
Provide ONLY the summary, no additional commentary.`;try{const o={id:"temp-summary",title:"Summary Request",messages:[{id:"summary-msg-"+Date.now(),role:"user",content:c,timestamp:Date.now()}],createdAt:Date.now(),updatedAt:Date.now(),isActive:!1,isGameHub:!1},d={id:"system",email:"system@otakon.ai",profileData:null},p=(await me.getChatResponse(o,d,c,!1,!1)).content.trim(),y=this.countWords(p);return console.log(`✅ [ContextSummarization] Summary generated: ${y} words (reduced from ${r})`),{summary:p,wordCount:y,messagesIncluded:e.length,originalWordCount:r}}catch(o){console.error("❌ [ContextSummarization] Failed to generate summary:",o);const d=e.slice(0,5).map(h=>h.content.substring(0,100)).join(" ... ").substring(0,this.MAX_WORDS*6);return{summary:`[Previous conversation context] ${d}`,wordCount:this.countWords(d),messagesIncluded:e.length,originalWordCount:r}}}async applyContextSummarization(e){if(!this.shouldSummarize(e))return e;const{toSummarize:t,toKeep:s}=this.splitMessages(e.messages);if(t.length===0)return e;const r=await this.summarizeMessages(t,e.gameTitle,e.genre),n=[{id:"summary-"+Date.now(),role:"system",content:r.summary,timestamp:t[t.length-1].timestamp,metadata:{isSummary:!0,messagesIncluded:r.messagesIncluded,originalWordCount:r.originalWordCount,summaryWordCount:r.wordCount}},...s],c=r.summary.replace(/!\[.*?\]\(data:image\/.*?\)/g,""),o=c.split(/\s+/).filter(h=>h.length>0),d=o.length>500?o.slice(0,500).join(" ")+"...":c;return console.log(`✅ [ContextSummarization] Context optimized: ${e.messages.length} messages → ${n.length} (${r.originalWordCount} words → ${r.wordCount} + recent)`),{...e,messages:n,contextSummary:d,lastSummarizedAt:Date.now(),updatedAt:Date.now()}}async getOptimizedContext(e){return this.shouldSummarize(e)?(await this.applyContextSummarization(e)).messages:e.messages}willTriggerSummarization(e){return this.getTotalWordCount(e.messages)>this.MAX_WORDS*3*.8}}const rt=new st,At=Object.freeze(Object.defineProperty({__proto__:null,contextSummarizationService:rt},Symbol.toStringTag,{value:"Module"}));export{j as E,he as M,O as S,Et as U,mt as W,w as a,ft as b,gt as c,St as d,bt as e,$ as f,yt as g,dt as h,_t as i,wt as j,Tt as k,Ct as l,Ue as m,vt as n,ht as o,pt as p,ut as q,At as r,Re as s,M as t};
//# sourceMappingURL=services-DlbEXRyH.js.map
