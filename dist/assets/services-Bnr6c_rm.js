...
`;async function Pe(){try{const a=await pt.getConversations(),e=Object.values(a).filter(r=>!r.isGameHub&&r.gameTitle).map(r=>({title:r.gameTitle||"Unknown",genre:r.genre||"Unknown",progress:r.gameProgress||0,isUnreleased:r.isUnreleased||!1})).sort((r,s)=>r.title.localeCompare(s.title));if(e.length===0)return`
**📚 USER'S GAME LIBRARY:**
...
`}catch(a){return console.warn("[PromptSystem] Failed to build game library context:",a),""}}const Xt=async(a,e,t,r,s)=>{var m;const o=Math.floor(a.messages.length/2);console.log(`💬 [PromptSystem] Conversation turn count: ${o}`);let n=0;const i=[],c=2500,p=[...a.subtabs||[]].filter(g=>g.status==="loaded"&&g.content).sort((g,h)=>{const C=Ee.indexOf(g.id),_=Ee.indexOf(h.id);return(C===-1?999:C)-(_===-1?999:_)}).map(g=>{const h=g.content||"";h.length>c&&h.includes("<details>")&&i.push(g.id);const C=h.length>We?h.slice(0,We)+"...[truncated]":h,_=`### ${g.title} (ID: ${g.id})
${C}`;if(n+_.length>Vt){const A=Ee.indexOf(g.id);if(A!==-1&&A<3){const Me=`### ${g.title} (ID: ${g.id}) [SUMMARY]
${h.slice(0,500)}...`;return n+=Me.length,Me}return null}return n+=_.length,_}).filter(Boolean).join(`

`)||"No subtabs available yet.",f=i.length>0?`
...
This keeps subtabs useful without losing important context.`:"",T=a.messages.slice(-10).map(g=>`${g.role==="user"?"User":"Otagon"}: ${g.content}`).join(`
`),k=a.contextSummary?`**Historical Context (Previous Sessions):**
${a.contextSummary}

`:"",P=s||X.getDefaultProfile(),Se=X.buildProfileContext(P);let F="";if(a.gameTitle){const g=mt.getByGameTitle(a.gameTitle);if(g!=null&&g.igdbGameId)try{const h=await ht.getForContext(g.igdbGameId);if(h){const C=Math.ceil(h.length/4);F=`
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
END OF ${a.gameTitle.toUpperCase()} KNOWLEDGE (${C} tokens)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

`,console.log(`🎮 [PromptSystem] Injecting ${h.length} chars of FULL game knowledge (no truncation)`)}}catch(h){console.warn("🎮 [PromptSystem] Failed to fetch game knowledge:",h)}}const y=await Pe();return`
...
`},Qt=async(a,e,t,r)=>{const s=r||X.getDefaultProfile(),o=X.buildProfileContext(s),n=await Pe(),i=a.messages.slice(-10).map(c=>`${c.role==="user"?"User":"Otagon"}: ${c.content}`).join(`
`);return`
...
**Tag Definitions:**
${ye}
`},Zt=async(a,e,t,r)=>{const s=r||X.getDefaultProfile(),o=X.buildProfileContext(s),n=await Pe(),i=a.gameTitle?`
...
**Tag Definitions:**
${ye}
`},gs=async(a,e,t,r,s,o,n,i,c)=>{const u=n?zt(n):"",p=i?`
**User Timezone:** ${i}
...
Example: @story_so_far The player defeated the first boss
    `.trim()}validateCommand(e){switch(e.type){case"update":if(!e.instruction)return{valid:!1,error:"Update command requires content. Example: @story_so_far The player..."};break;case"modify":if(!e.instruction)return{valid:!1,error:"Modify command requires instructions. Example: @tips \\modify Change to combat strategies"};break}return{valid:!0}}describeCommand(e){switch(e.type){case"update":return`Updating "${e.tabName}" with: ${e.instruction}`;case"modify":return`Modifying "${e.tabName}": ${e.instruction}`;case"delete":return`Deleting "${e.tabName}"`}}}const ws=new Ir;let d,V=[],He=!1,J="",M=null,D=null,v=null,L=null;const Nr="otakonSpeechRate",De=async()=>{try{const a=navigator;if(a.wakeLock){if(M!==null)try{await M.release()}catch{}M=await a.wakeLock.request("screen"),console.log("🔒 [TTS] Wake lock acquired - screen will stay on"),M.addEventListener("release",async()=>{console.log("🔓 [TTS] Wake lock released"),M=null,d&&d.speaking&&(console.log("🔒 [TTS] Reacquiring wake lock (TTS still active)"),await De())})}else console.warn("⚠️ [TTS] Wake lock API not available")}catch(a){console.warn("⚠️ [TTS] Wake lock request failed:",a)}},at=async()=>{try{M!==null&&(await M.release(),M=null)}catch{}},kr=()=>{try{if(!D){const a=window,e=a.AudioContext||a.webkitAudioContext;e&&(D=new e,console.log("🔊 [TTS] Audio context initialized"))}v||(v=new Audio,v.src="data:audio/wav;base64,UklGRnoGAABXQVZFZm10IBAAAAABAAEAQB8AAEAfAAABAAgAZGF0YQoGAACBhYqFbF1fdJivrJBhNjVgodDbq2EcBj+a2teleShtr9teleShtr9teleShtr9teleShtr9t",v.loop=!0,v.volume=.01,v.load(),console.log("🔇 [TTS] Silent audio initialized for background playback"))}catch(a){console.warn("⚠️ [TTS] Audio context init failed:",a)}},ot=async()=>{try{D&&D.state==="suspended"&&(await D.resume(),console.log("🔊 [TTS] Audio context resumed")),v&&(v.currentTime=0,await v.play(),console.log("🔇 [TTS] Silent audio playing for background session")),L||(L=setInterval(()=>{d&&d.speaking?D&&D.state==="suspended"&&D.resume().catch(()=>{}):L&&(clearInterval(L),L=null)},5e3))}catch(a){console.warn("⚠️ [TTS] Silent audio start failed:",a)}},nt=()=>{try{v&&(v.pause(),v.currentTime=0,console.log("🔇 [TTS] Silent audio stopped")),L&&(clearInterval(L),L=null)}catch(a){console.warn("⚠️ [TTS] Silent audio stop failed:",a)}},Rr=()=>new Promise((a,e)=>{if(!d)return e(new Error("Speech synthesis not initialized."));if(V=d.getVoices(),V.length>0){a();return}d.onvoiceschanged=()=>{V=d.getVoices(),a()},setTimeout(()=>{V.length===0&&(V=d.getVoices()),a()},1e3)}),ie=()=>{d&&d.speaking&&d.cancel(),J="","mediaSession"in navigator&&navigator.mediaSession.playbackState!=="none"&&(navigator.mediaSession.playbackState="paused"),at(),nt(),window.dispatchEvent(new CustomEvent("otakon:ttsStopped"))},it=()=>{d&&d.speaking&&!d.paused&&(d.pause(),"mediaSession"in navigator&&(navigator.mediaSession.playbackState="paused"),window.dispatchEvent(new CustomEvent("otakon:ttsPaused")))},ct=()=>{d&&d.paused&&(d.resume(),"mediaSession"in navigator&&(navigator.mediaSession.playbackState="playing"),window.dispatchEvent(new CustomEvent("otakon:ttsResumed")))},Pr=async()=>{J&&(ie(),await ut(J))},Ur=()=>d?d.speaking:!1,Gr=()=>{ie(),window.dispatchEvent(new CustomEvent("otakon:disableHandsFree"))},Dr=()=>{if("mediaSession"in navigator){console.log("🎵 [TTS] Setting up media session");try{navigator.mediaSession.setActionHandler("play",()=>{console.log("🎵 [TTS] Media session play action"),d&&d.paused&&ct()}),navigator.mediaSession.setActionHandler("pause",()=>{console.log("🎵 [TTS] Media session pause action"),d&&d.speaking&&!d.paused&&it()}),navigator.mediaSession.setActionHandler("stop",()=>{console.log("🎵 [TTS] Media session stop action"),Gr()}),console.log("✅ [TTS] Media session handlers registered")}catch(a){console.warn("⚠️ [TTS] Failed to set media session handlers:",a)}}else console.warn("⚠️ [TTS] Media session API not available")},Mr=async()=>{document.hidden?(console.log("📱 [TTS] App went to background, isSpeaking:",d==null?void 0:d.speaking),d&&d.speaking&&(await ot(),d.paused||setTimeout(()=>{d&&d.speaking&&!d.paused&&console.log("📱 [TTS] Nudging speech synthesis to stay alive")},100))):(console.log("📱 [TTS] App came to foreground, isSpeaking:",d==null?void 0:d.speaking),d&&d.speaking&&await De())},Lr=async()=>{if(typeof window<"u"&&"speechSynthesis"in window){if(He)return;He=!0,d=window.speechSynthesis,await Rr(),Dr(),kr(),document.addEventListener("visibilitychange",Mr),d.getVoices().length===0&&d.speak(new SpeechSynthesisUtterance(""))}},lt=()=>V.filter(a=>a.lang.startsWith("en-")),ut=async a=>new Promise((e,t)=>{try{if(!d)return console.error("Text-to-Speech is not available on this browser."),t(new Error("Text-to-Speech is not available on this browser."));if(!a.trim())return e();ie(),J=a;const r=new SpeechSynthesisUtterance(a),s=localStorage.getItem(Nr);r.rate=s?parseFloat(s):.94;const o=localStorage.getItem("otakonPreferredVoiceURI"),n=lt();let i;if(o&&(i=n.find(c=>c.voiceURI===o)),!i&&n.length>0){const c=n.find(u=>u.name.toLowerCase().includes("female"));c?i=c:i=n[0]}i&&(r.voice=i),r.onstart=async()=>{if(console.log("🎤 [TTS] Speech started"),J=a,await De(),await ot(),"serviceWorker"in navigator&&navigator.serviceWorker.controller&&navigator.serviceWorker.controller.postMessage({type:"TTS_STARTED"}),"mediaSession"in navigator)try{navigator.mediaSession.playbackState="playing";const c=a.replace(/[*_~`#]/g,"").replace(/\n+/g," ").trim(),u=c.length>100?c.substring(0,97)+"...":c;navigator.mediaSession.metadata=new MediaMetadata({title:u,artist:"Otagon AI",album:"Gaming Companion",artwork:[{src:"/icon-192.png",sizes:"192x192",type:"image/png"},{src:"/icon-512.png",sizes:"512x512",type:"image/png"}]}),console.log("🎵 [TTS] Media session metadata set")}catch(c){console.warn("⚠️ [TTS] Failed to set media session:",c)}window.dispatchEvent(new CustomEvent("otakon:ttsStarted"))},r.onend=()=>{J="","mediaSession"in navigator&&(navigator.mediaSession.playbackState="paused"),"serviceWorker"in navigator&&navigator.serviceWorker.controller&&navigator.serviceWorker.controller.postMessage({type:"TTS_STOPPED"}),at(),nt(),window.dispatchEvent(new CustomEvent("otakon:ttsStopped")),e()},r.onerror=c=>{console.error("SpeechSynthesis Utterance Error",c),ie(),t(c)},d.speak(r)}catch(r){console.error("TTS Error:",r),t(r)}}),_s={init:Lr,getAvailableVoices:lt,speak:ut,cancel:ie,pause:it,resume:ct,restart:Pr,isSpeaking:Ur};class Fr{constructor(){b(this,"cache",new Map);b(this,"lastFetch",new Map);b(this,"CACHE_TTL_MS",3e4)}get(e){const t=this.cache.get(e),r=this.lastFetch.get(e);return!t||!r?null:Date.now()-r>this.CACHE_TTL_MS?(this.cache.delete(e),this.lastFetch.delete(e),null):t}set(e,t){this.cache.set(e,t),this.lastFetch.set(e,Date.now())}invalidate(e){this.cache.delete(e),this.lastFetch.delete(e)}clear(){this.cache.clear(),this.lastFetch.clear()}}const As=new Fr;class ee{static async trackKeyAdded(e,t,r){try{await l.from("user_analytics").insert({user_id:e.id,auth_user_id:e.authUserId,event_type:"byok_key_added",event_data:{provider:t,tier:e.tier,...r,timestamp:new Date().toISOString()}})}catch(s){console.error("[BYOKAnalytics] Error tracking key_added:",s)}}static async trackKeyRemoved(e,t,r){try{await l.from("user_analytics").insert({user_id:e.id,auth_user_id:e.authUserId,event_type:"byok_key_removed",event_data:{provider:t,reason:r,timestamp:new Date().toISOString()}})}catch(s){console.error("[BYOKAnalytics] Error tracking key_removed:",s)}}static async trackKeyVerification(e,t,r,s){try{await l.from("user_analytics").insert({user_id:e.id,auth_user_id:e.authUserId,event_type:"byok_key_verified",event_data:{provider:t,success:r,error:s,timestamp:new Date().toISOString()}})}catch(o){console.error("[BYOKAnalytics] Error tracking key_verified:",o)}}static async trackBYOKRequest(e,t,r,s,o,n){try{await l.from("user_analytics").insert({user_id:e,auth_user_id:t,event_type:"byok_request_made",event_data:{provider:r,model:s,tokensUsed:o,requestType:n,timestamp:new Date().toISOString()}})}catch(i){console.error("[BYOKAnalytics] Error tracking byok_request:",i)}}static async getBYOKStats(e){try{const{data:t}=await l.from("user_analytics").select("event_data, created_at").eq("user_id",e).eq("event_type","byok_request_made").order("created_at",{ascending:!1}),r=(t==null?void 0:t.length)||0,s=(t==null?void 0:t.reduce((i,c)=>{const u=c.event_data;return i+(u.tokensUsed||0)},0))||0,o=[...new Set((t==null?void 0:t.map(i=>i.event_data.provider))||[])],n=t&&t.length>0?t[0].created_at:null;return{totalRequests:r,totalTokens:s,providers:o,lastUsed:n}}catch(t){return console.error("[BYOKAnalytics] Error getting BYOK stats:",t),{totalRequests:0,totalTokens:0,providers:[],lastUsed:null}}}}class vs{static async testGeminiKey(e){var t;try{const r=await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash-exp:generateContent?key=${e}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({contents:[{parts:[{text:"test"}]}]})});if(r.ok)return{valid:!0};{const s=await r.text();console.error("[ApiKeyService] API key test failed:",s);let o="Invalid API key";try{const n=JSON.parse(s);(t=n.error)!=null&&t.message&&(o=n.error.message)}catch{r.status===400?o="Invalid API key format or request":r.status===403?o="API key denied - check permissions":r.status===404&&(o="API endpoint not found - key may be invalid")}return{valid:!1,error:o}}}catch(r){return console.error("[ApiKeyService] Error testing API key:",r),{valid:!1,error:"Failed to test API key"}}}static async saveGeminiKey(e,t,r=!1){var s,o,n;try{const i=t.trim();if(!i.startsWith("AIzaSy"))throw console.error("[ApiKeyService] Invalid key format. Key starts with:",i.substring(0,10)),new Error(`Invalid API key format. Gemini API keys start with "AIzaSy" but yours starts with "${i.substring(0,6)}..."`);if(i.length<39)throw new Error("API key is too short. Gemini API keys are typically 39 characters long.");if(!r){N.info("Testing your API key...");const f=await this.testGeminiKey(i);if(!f.valid)if(((s=f.error)==null?void 0:s.includes("quota"))||((o=f.error)==null?void 0:o.includes("429"))||((n=f.error)==null?void 0:n.includes("RESOURCE_EXHAUSTED")))console.warn("[ApiKeyService] Key validation failed due to quota, but allowing save"),N.warning("⚠️ Could not test key (quota limit), but saving anyway..."),await ee.trackKeyVerification(e,"google",!1,"quota_exceeded_during_test");else throw await ee.trackKeyVerification(e,"google",!1,f.error),new Error(f.error||"Invalid API key")}const c={gemini_api_key_encrypted:i,uses_custom_gemini_key:!0,updated_at:new Date().toISOString()};try{c.custom_key_verified_at=new Date().toISOString(),c.had_custom_key_before=!0}catch{console.warn("[ApiKeyService] BYOK columns may not exist yet - using basic fields only")}const{error:u,data:p}=await l.from("users").update(c).eq("auth_user_id",e.authUserId).select();if(u){console.error("[ApiKeyService] Error saving API key:",u);const f=u.message||"Failed to save API key to database";throw new Error(`Database error: ${f}`)}if(!p||p.length===0)throw console.error("[ApiKeyService] No data returned after update"),new Error("User record not found or update failed");console.log("[ApiKeyService] ✅ API key saved successfully:",{userId:e.id,authUserId:e.authUserId,usesCustomKey:!0}),localStorage.removeItem("otakon_user"),localStorage.removeItem(`otakon_user_cache_${e.authUserId}`),await ee.trackKeyAdded(e,"google"),await ee.trackKeyVerification(e,"google",!0),N.success("✅ Custom API key saved! You now have unlimited queries.")}catch(i){const c=i instanceof Error?i.message:"Failed to save API key";throw N.error(c),i}}static async removeGeminiKey(e,t="user_action"){try{const{error:r}=await l.from("users").update({gemini_api_key_encrypted:null,uses_custom_gemini_key:!1,updated_at:new Date().toISOString()}).eq("auth_user_id",e.authUserId);if(r)throw console.error("[ApiKeyService] Error removing API key:",r),new Error("Failed to remove API key");await ee.trackKeyRemoved(e,"google",t),t==="user_action"&&N.success("Custom API key removed. Back to platform limits.")}catch(r){const s=r instanceof Error?r.message:"Failed to remove API key";throw N.error(s),r}}static async clearUserApiKeys(e){try{const{error:t}=await l.from("users").update({gemini_api_key_encrypted:null,uses_custom_gemini_key:!1,had_custom_key_before:!0,updated_at:new Date().toISOString()}).eq("auth_user_id",e);if(t)throw console.error("[ApiKeyService] Error clearing API keys:",t),t;console.log("[ApiKeyService] ✅ Custom API keys cleared for user:",e)}catch(t){throw console.error("[ApiKeyService] Failed to clear API keys:",t),t}}static needsReVerification(e){if(!e)return!0;const t=new Date(e);return(Date.now()-t.getTime())/(1e3*60*60*24)>30}}class xr{constructor(){b(this,"submittedFeedback",new Set)}hasSubmittedFeedback(e){return this.submittedFeedback.has(e)}async submitFeedback(e){try{const t=le.getCurrentUser();if(!(t!=null&&t.authUserId))return console.warn("[FeedbackService] User not authenticated"),{success:!1,error:"User not authenticated"};if(this.submittedFeedback.has(e.messageId))return console.log("[FeedbackService] Feedback already submitted for message:",e.messageId),{success:!0};const{error:r}=await l.from("ai_feedback").insert({user_id:t.authUserId,conversation_id:e.conversationId,message_id:e.messageId,feedback_type:e.feedbackType,content_type:e.contentType,category:e.category||null,comment:e.comment||null});return r?r.code==="23505"?(console.log("[FeedbackService] Feedback already exists in database"),this.submittedFeedback.add(e.messageId),{success:!0}):(console.error("[FeedbackService] Failed to submit feedback:",r),{success:!1,error:r.message}):(this.submittedFeedback.add(e.messageId),console.log("[FeedbackService] Feedback submitted:",{messageId:e.messageId,type:e.feedbackType,contentType:e.contentType}),{success:!0})}catch(t){return console.error("[FeedbackService] Error submitting feedback:",t),{success:!1,error:"Failed to submit feedback"}}}async submitPositiveFeedback(e,t,r="message"){return this.submitFeedback({messageId:e,conversationId:t,feedbackType:"up",contentType:r})}async submitNegativeFeedback(e,t,r,s,o="message"){return this.submitFeedback({messageId:e,conversationId:t,feedbackType:"down",contentType:o,category:r,comment:s})}async submitCorrection(e){try{const t=le.getCurrentUser();if(!(t!=null&&t.authUserId))return console.warn("[FeedbackService] User not authenticated for correction"),{success:!1,error:"User not authenticated"};if(!W.getRateLimitStatus().allowed)return{success:!1,error:"Daily correction limit reached. Try again tomorrow.",rateLimitRemaining:0};const s={originalResponse:e.originalResponse,correctionText:e.correctionText,type:e.correctionType,scope:e.correctionScope,gameTitle:e.gameTitle,messageId:e.messageId,conversationId:e.conversationId},o=await W.submitCorrection(t.authUserId,s);return o.success?(console.log("[FeedbackService] Correction submitted successfully:",{messageId:e.messageId,type:e.correctionType,scope:e.correctionScope}),{success:!0,correction:o.correction,rateLimitRemaining:W.getRateLimitStatus().remaining}):{success:!1,error:o.error,rateLimitRemaining:W.getRateLimitStatus().remaining}}catch(t){return console.error("[FeedbackService] Error submitting correction:",t),{success:!1,error:"Failed to submit correction"}}}getCorrectionRateLimit(){return W.getRateLimitStatus()}async getUserCorrections(){const e=le.getCurrentUser();return e!=null&&e.authUserId?W.getAllCorrections(e.authUserId):[]}async toggleCorrection(e,t){const r=le.getCurrentUser();return r!=null&&r.authUserId?W.toggleCorrection(r.authUserId,e,t):!1}async getFeedbackStats(){try{const{data:e,error:t}=await l.from("ai_feedback").select("feedback_type, category");if(t)return console.error("[FeedbackService] Failed to get feedback stats:",t),null;const r={totalFeedback:e.length,positiveCount:e.filter(s=>s.feedback_type==="up").length,negativeCount:e.filter(s=>s.feedback_type==="down").length,categoryBreakdown:{}};return e.forEach(s=>{s.category&&(r.categoryBreakdown[s.category]=(r.categoryBreakdown[s.category]||0)+1)}),r}catch(e){return console.error("[FeedbackService] Error getting feedback stats:",e),null}}clearLocalTracking(){this.submittedFeedback.clear()}}const Kr=new xr,Os=Object.freeze(Object.defineProperty({__proto__:null,feedbackService:Kr},Symbol.toStringTag,{value:"Module"}));function $r(a){var n;const e=a.split(","),t=((n=e[0].match(/:(.*?);/))==null?void 0:n[1])||"image/png",r=atob(e[1]),s=r.length,o=new Uint8Array(s);for(let i=0;i<s;i++)o[i]=r.charCodeAt(i);return new Blob([o],{type:t})}async function Wr(a,e){try{const t=$r(a),r=t.size,s=50*1024*1024;if(r>s)return{success:!1,error:"Screenshot exceeds 50MB size limit"};const o=Date.now(),n=Math.random().toString(36).substring(2,15),i=`${o}_${n}.png`,c=`${e}/${i}`,{error:u}=await l.storage.from("screenshots").upload(c,t,{contentType:"image/png",cacheControl:"3600",upsert:!1});if(u)return console.error("Screenshot upload error:",u),{success:!1,error:u.message};const{data:p}=l.storage.from("screenshots").getPublicUrl(c);return p!=null&&p.publicUrl?{success:!0,publicUrl:p.publicUrl,fileSize:r}:{success:!1,error:"Failed to generate public URL"}}catch(t){return console.error("Screenshot upload exception:",t),{success:!1,error:t instanceof Error?t.message:"Unknown error"}}}const Cs=Object.freeze(Object.defineProperty({__proto__:null,uploadScreenshot:Wr},Symbol.toStringTag,{value:"Module"})),de=new Map;class Br{constructor(){b(this,"MAX_WORDS",300);b(this,"RECENT_MESSAGE_COUNT",8)}countWords(e){return e.trim().split(/\s+/).filter(t=>t.length>0).length}getTotalWordCount(e){return e.reduce((t,r)=>{const s=this.countWords(r.content);return t+s},0)}shouldSummarize(e){return!e.messages||e.messages.length<=this.RECENT_MESSAGE_COUNT?!1:this.getTotalWordCount(e.messages)>this.MAX_WORDS*3}splitMessages(e){if(e.length<=this.RECENT_MESSAGE_COUNT)return{toSummarize:[],toKeep:e};const t=e.length-this.RECENT_MESSAGE_COUNT;return{toSummarize:e.slice(0,t),toKeep:e.slice(t)}}async summarizeMessages(e,t,r){const s=this.getTotalWordCount(e),o=e.map(c=>`${c.role==="user"?"User":"Assistant"}: ${c.content}`).join(`
...
Provide ONLY the summary, no additional commentary.`;try{const c={id:"temp-summary",title:"Summary Request",messages:[{id:"summary-msg-"+Date.now(),role:"user",content:i,timestamp:Date.now()}],createdAt:Date.now(),updatedAt:Date.now(),isActive:!1,isGameHub:!1};console.log(`📡 [GEMINI CALL #2] 📋 Context Summarization | Messages: ${e.length} | Game: ${t||"N/A"} | Target: ${this.MAX_WORDS} words`);const u={id:"system",email:"system@otakon.ai",profileData:null},f=(await bt.getChatResponse(c,u,i,!1,!1,void 0,void 0,"summarization")).content.trim(),T=this.countWords(f);return console.log(`✅ [GEMINI CALL] Context Summarization SUCCESS | ${T} words (reduced from ${s})`),N.success(`Conversation summarized (${s} → ${T} words)`),de.delete("summarization"),{summary:f,wordCount:T,messagesIncluded:e.length,originalWordCount:s}}catch(c){console.error("❌ [ContextSummarization] Failed to generate summary:",c);const u=de.get("summarization")||0,p=3;if(u<p)return de.set("summarization",u+1),console.log(`🔄 [ContextSummarization] Retry ${u+1}/${p} in 1 second...`),await new Promise(T=>setTimeout(T,1e3)),this.summarizeMessages(e,t,r);console.error("❌ [ContextSummarization] Max retries reached"),de.delete("summarization");const f=e.slice(0,5).map(T=>T.content.substring(0,100)).join(" ... ").substring(0,this.MAX_WORDS*6);return{summary:`[Previous conversation context] ${f}`,wordCount:this.countWords(f),messagesIncluded:e.length,originalWordCount:s}}}async applyContextSummarization(e){if(!this.shouldSummarize(e))return e;const{toSummarize:t,toKeep:r}=this.splitMessages(e.messages);if(t.length===0)return e;const s=await this.summarizeMessages(t,e.gameTitle,e.genre),n=[{id:"summary-"+Date.now(),role:"system",content:s.summary,timestamp:t[t.length-1].timestamp,metadata:{isSummary:!0,messagesIncluded:s.messagesIncluded,originalWordCount:s.originalWordCount,summaryWordCount:s.wordCount}},...r],i=s.summary.replace(/!\[.*?\]\(data:image\/.*?\)/g,""),c=i.split(/\s+/).filter(p=>p.length>0),u=c.length>500?c.slice(0,500).join(" ")+"...":i;return console.log(`✅ [ContextSummarization] Context optimized: ${e.messages.length} messages → ${n.length} (${s.originalWordCount} words → ${s.wordCount} + recent)`),{...e,messages:n,contextSummary:u,lastSummarizedAt:Date.now(),updatedAt:Date.now()}}async getOptimizedContext(e){return this.shouldSummarize(e)?(await this.applyContextSummarization(e)).messages:e.messages}willTriggerSummarization(e){return this.getTotalWordCount(e.messages)>this.MAX_WORDS*3*.8}}const Hr=new Br,Is=Object.freeze(Object.defineProperty({__proto__:null,contextSummarizationService:Hr},Symbol.toStringTag,{value:"Module"}));export{vs as A,fs as B,Ss as C,es as D,ue as E,Xr as F,Qr as G,Os as H,Cs as I,Is as J,Ts as U,rs as W,ns as a,W as b,hs as c,U as d,ms as e,ds as f,gs as g,ps as h,X as i,_s as j,ys as k,ws as l,cs as m,ls as n,is as o,os as p,us as q,bs as r,Zr as s,N as t,Es as u,ss as v,as as w,As as x,ts as y,Nt as z};
