...
`)}return t.join(`
`)}function Bt(s){if(!s||s.scope==="off")return"";const e=[];if(s.previousTopics.length>0&&e.push(`
...
`)}
`)}return e.join(`
`)}async function ns(s,e){try{const t=await R.getAIPreferences(s);if(t.responseHistoryScope==="off")return{previousTopics:[],corrections:[],scope:"off"};const[r,a]=await Promise.all([R.getResponseTopics(s,e,t.responseHistoryScope),R.getActiveCorrections(s,e,!0)]);return{previousTopics:r,corrections:a,scope:t.responseHistoryScope}}catch(t){return console.error("[PromptSystem] Error fetching behavior context:",t),null}}const Fe=3e3,Ht=2e4,be=["story_so_far","quest_log","characters","boss_strategy","tips","hidden_secrets","points_of_interest"],He=`
...
`},Vt=(s,e,t,r)=>{const a=r||J.getDefaultProfile(),o=J.buildProfileContext(a),n=s.messages.slice(-10).map(i=>`${i.role==="user"?"User":"Otagon"}: ${i.content}`).join(`
`);return`
...
**Tag Definitions:**
${ye}
`},jt=(s,e,t,r)=>{const a=r||J.getDefaultProfile(),o=J.buildProfileContext(a),n=s.gameTitle?`
...
**Tag Definitions:**
${ye}
`},is=async(s,e,t,r,a,o,n,i,c)=>{const u=n?Bt(n):"",f=i?`
**User Timezone:** ${i}
...
*Content loading...*`}}const ls=new Xt,Zt=`You are a correction validator for OTAKON, an AI gaming companion.
