...
export const ProactiveInsightsPanel: React.FC<ProactiveInsightsPanelProps> = ({
  ...
  const [insights, setInsights] = useState<ProactiveInsightSuggestion[]>([]);
    ...
    const loadInsights = async () => {
        ...
        try {
            ...
            const allInsights = unifiedAIService().getProactiveInsights();
  ...
  const handleInsightAction = async (insight: ProactiveInsightSuggestion) => {
    try {
      const { unifiedAIService } = await import('../services/unifiedAIService');
      await unifiedAIService().markInsightAsRead(insight.id);
  ...
  const handleDeleteInsight = async (insightId: string) => {
    try {
      const { unifiedAIService } = await import('../services/unifiedAIService');
      await unifiedAIService().deleteInsight(insightId);
      loadInsights();
    } catch (error) {
