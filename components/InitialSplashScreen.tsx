...
const InitialSplashScreen: React.FC<InitialSplashScreenProps> = ({ onComplete }) => {
  ...
  return (
    <div className="h-screen bg-gradient-to-br from-[#111111] to-[#0A0A0A] text-[#F5F5F5] flex flex-col items-center justify-center font-inter px-8 pt-16 pb-20 text-center overflow-hidden">
      ...
      <h1 
        className="text-7xl md:text-8xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-[#FF4D4D] to-[#FFAB40] mt-8 mb-6 leading-tight"
      >
        Otakon
      </h1>

      <p 
        className="text-2xl md:text-3xl text-[#CFCFCF] mb-16 leading-relaxed"
      >
        Your Spoiler-Free Gaming Companion
      </p>

      <div 
        className="flex flex-col items-center justify-center gap-6 w-full max-w-lg"
      >
        <button
          onClick={onComplete}
          className="w-full bg-gradient-to-r from-[#E53A3A] to-[#D98C1F] hover:from-[#D42A2A] hover:to-[#C87A1A] text-white font-bold py-5 px-8 rounded-xl transition-all duration-300 hover:scale-105 hover:shadow-xl hover:shadow-[#E53A3A]/25 text-lg flex items-center justify-center"
        >
          Start the Adventure
        </button>
