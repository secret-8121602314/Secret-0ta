...
const LandingPage: React.FC<LandingPageProps> = ({ onGetStarted, onOpenAbout, onOpenPrivacy, onOpenRefund, onOpenContact }) => {
    ...
    return (
        <div className="text-white font-inter animate-fade-in overflow-x-hidden relative">
            {/* Header with Logo Click Functionality */}
            <header className="fixed top-0 left-0 right-0 z-50 bg-black/80 backdrop-blur-xl border-b border-neutral-800/50">
                <div className="container mx-auto px-6 py-4">
                    <div className="flex items-center justify-between">
                        <button
                            ...
                            aria-label="Get started with Otakon"
                        >
                            <div className="flex items-center gap-3">
                                <div className="group-hover:scale-110 transition-transform duration-200">
                                    <Logo className="h-8 w-8" />
                                </div>
                                <span className="text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-[#FF4D4D] to-[#FFAB40]">
                                    Otakon
                                </span>
                            </div>
                        </button>
                        ...
                        <nav className="hidden md:flex items-center gap-8">
                            <a href="#features" onClick={(e) => { e.preventDefault(); document.getElementById('features')?.scrollIntoView({ behavior: 'smooth' }); }} className="text-neutral-300 hover:text-white transition-colors">Features</a>
                            <a href="#pricing" onClick={(e) => { e.preventDefault(); document.getElementById('features')?.scrollIntoView({ behavior: 'smooth' }); }} className="text-neutral-300 hover:text-white transition-colors">Pricing</a>
                            <button onClick={onOpenAbout} className="text-neutral-300 hover:text-white transition-colors">About</button>
                            <button onClick={onOpenContact} className="text-neutral-300 hover:text-white transition-colors">Contact</button>
                        </nav>
                    </div>
                </div>
            </header>
                ...
                <div className="container mx-auto px-6 py-8">
                    <div className="flex flex-col md:flex-row justify-between items-center text-center md:text-left gap-6">
                        ...
                        <div className="flex items-center gap-6 text-sm font-medium text-neutral-400">
                           ...
                           <button onClick={onOpenAbout} className="hover:text-white transition-colors">About</button>
                           <button onClick={onOpenPrivacy} className="hover:text-white transition-colors">Privacy</button>
                           <button onClick={onOpenRefund} className="hover:text-white transition-colors">Refund Policy</button>
                           <button onClick={onOpenContact} className="hover:text-white transition-colors">Contact Us</button>
                        </div>
                    </div>
                </div>
            </footer>
