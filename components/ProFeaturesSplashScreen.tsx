...
const ProFeature: React.FC<{ title: string; description: string; children: React.ReactNode; }> = ({ title, description, children }) => (
    <div className="flex items-start gap-3 sm:gap-4 md:gap-6 p-3 sm:p-4 md:p-6 bg-gradient-to-r from-[#1C1C1C]/80 to-[#0A0A0A]/80 backdrop-blur-xl rounded-2xl border-2 border-neutral-800/60 hover:border-neutral-700/80 hover:scale-105 transition-all duration-500 feature-card">
...
const ProFeaturesSplashScreen: React.FC<ProFeaturesSplashScreenProps> = ({ onComplete, onUpgrade, onUpgradeToVanguard }) => {
    ...
    return (
        <div className="h-screen bg-gradient-to-br from-[#111111] to-[#0A0A0A] text-[#F5F5F5] flex flex-col font-inter pro-features-mobile">
            {/* Sticky Header */}
            <header className="sticky top-0 bg-gradient-to-b from-[#111111]/95 to-[#0A0A0A]/95 backdrop-blur-sm border-b border-[#424242]/20 px-4 sm:px-6 md:px-8 pt-8 sm:pt-12 md:pt-16 pb-4 sm:pb-6 md:pb-8 text-center z-10">
                <div className="max-w-2xl w-full mx-auto">
                    ...
                    <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-[#FF4D4D] to-[#FFAB40] mb-3 sm:mb-4 md:mb-6 leading-tight">Supercharge with Otakon Pro</h1>
                    <p className="text-base sm:text-lg md:text-xl text-neutral-300 mb-6 sm:mb-8 md:mb-10 leading-relaxed">Unlock exclusive features for the ultimate gaming companion.</p>

                    <div className="w-full max-w-md mx-auto p-1 bg-gradient-to-r from-[#2E2E2E]/60 to-[#1A1A1A]/60 backdrop-blur-sm rounded-2xl flex items-center gap-1 mb-4 sm:mb-6">
